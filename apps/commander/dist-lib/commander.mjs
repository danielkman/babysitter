...
], ee = "#c39b4e", te = "#e3c87f", R = "#6d5120", ne = /* @__PURE__ */ new Map();
function z(e) {
	let t = `${e.kind}|${e.entityId}|${e.adapter ?? ""}|${e.taskKind ?? ""}`, n = ne.get(t);
	if (n !== void 0) return n;
	let r = e.kind === "unit" ? ae(e) : le(e);
	return ne.set(t, r), r;
}
function re() {
...
function ae(e) {
	let t = F[e.adapter ?? ""] ?? I, [n, r, i, a, o] = t, s = P(`unit:${e.entityId}`), c = P(`cog:${e.entityId}`), l = s % 5, u = (s >>> 3) % 4, d = (s >>> 6) % 4, f = (s >>> 9) % 3, p = (s >>> 12) % 3, m = (s >>> 15) % 2, h = (s >>> 17) % 2 == 0, g = (s >>> 18) % 2 == 0, _ = 7 + c % 4, v = (c >>> 3) % 5 - 2, y = 23 + (c >>> 6) % 18, b = 34 + v, x = b + 8, S = `<ellipse cx="32" cy="57.5" rx="13" ry="2.4" fill="${a}" opacity="0.3"/>`, C = "";
	...
	let w = "";
	w = p === 0 ? `<ellipse cx="14.5" cy="27.5" rx="10.5" ry="3.9" transform="rotate(-32 14.5 27.5)" fill="${n}" opacity="0.62" stroke="${a}" stroke-width="0.9"/><ellipse cx="49.5" cy="27.5" rx="10.5" ry="3.9" transform="rotate(32 49.5 27.5)" fill="${n}" opacity="0.62" stroke="${a}" stroke-width="0.9"/><ellipse cx="15.5" cy="33.5" rx="8.5" ry="3.2" transform="rotate(-12 15.5 33.5)" fill="${n}" opacity="0.42" stroke="${a}" stroke-width="0.8"/><ellipse cx="48.5" cy="33.5" rx="8.5" ry="3.2" transform="rotate(12 48.5 33.5)" fill="${n}" opacity="0.42" stroke="${a}" stroke-width="0.8"/>` : p === 1 ? ie(13.5, 36, 4.2, r, a) + ie(50.5, 36, 4.2, r, a) : `<path d="M16 30 L8.5 26 L12 38 Z" fill="${n}" opacity="0.72" stroke="${a}" stroke-width="0.9" stroke-linejoin="round"/><path d="M48 30 L55.5 26 L52 38 Z" fill="${n}" opacity="0.72" stroke="${a}" stroke-width="0.9" stroke-linejoin="round"/>`;
	let T = "";
	switch (u) {
		case 0:
	...
	let N = h ? `<circle cx="${A - 2.5}" cy="${b + 5.5}" r="1.8" fill="${o}" opacity="0.55"/><circle cx="${j + 2.5}" cy="${b + 5.5}" r="1.8" fill="${o}" opacity="0.55"/>` : "", L = g ? `<circle cx="32" cy="${x + 7.5}" r="2" fill="${n}" stroke="${a}" stroke-width="0.8" opacity="0.9"/>` : "";
	return {
		svg: re() + S + C + w + T + E + D + O + k + N + M + L + "</svg>",
		palette: [...t]
	};
}
function oe(e, t, n, r) {
...
function le(e) {
	let t = e.taskKind ?? "task", n = L[P(`kind:${t}`) % L.length] ?? L[0], [r, i, a] = n, o = P(`task:${e.entityId}`), s = se[o % se.length] ?? se[0], c = ce[(o >>> 4) % ce.length] ?? ce[0];
	return {
		svg: re() + `<circle cx="32" cy="32" r="26" fill="none" stroke="${te}" stroke-width="2.6" stroke-dasharray="4.2 3.4"/><circle cx="32" cy="32" r="23" fill="${ee}" stroke="${R}" stroke-width="1.6"/><circle cx="32" cy="11.5" r="1.2" fill="${R}"/><circle cx="52.5" cy="32" r="1.2" fill="${R}"/><circle cx="32" cy="52.5" r="1.2" fill="${R}"/><circle cx="11.5" cy="32" r="1.2" fill="${R}"/><circle cx="${c[0]}" cy="${c[1]}" r="1" fill="${te}" opacity="0.9"/><circle cx="${s[0]}" cy="${s[1]}" r="2.6" fill="${i}"/><circle cx="32" cy="32" r="17.5" fill="${i}" stroke="${R}" stroke-width="0.8"/><ellipse cx="26" cy="25.5" rx="6.5" ry="4" fill="${a}" opacity="0.18"/><circle cx="32" cy="32" r="13.5" fill="none" stroke="${a}" stroke-width="1" opacity="0.3"/>` + oe(t, r, i, a) + "</svg>",
		palette: [...n]
	};
}
//#endregion
...
function pe(e) {
	let t = `${e.id}|${e.caption}|${e.tone ?? "normal"}`, n = fe.get(t);
	if (n !== void 0) return n;
	let r = e.tone === "danger" ? A : k, i = `${e.id} ${e.caption}`.toLowerCase(), a = ue.find((e) => e.match.test(i)), o;
	if (a !== void 0) o = a.paths;
	else {
		let t = P(`opt:${e.id}`);
		o = `${de[t % de.length] ?? de[0]}<circle cx="${(3 + (t >>> 4) % 15).toFixed(0)}" cy="${(16 + (t >>> 8) % 3).toFixed(0)}" r="0.7"/>`;
	}
	let s = j(o, r);
	return fe.set(t, s), s;
}
...
var ye = [
	...
	review: [
		B("approve-review", "Approve Review", "Approve the review and record the verdict", "Approve the review under way"),
		B("review-request-changes", "Request Changes", "Request changes on the reviewed work", "Send the work back with change requests", "danger"),
		B("add-comment", "Add Comment", "Add a review comment to the open thread", "Annotate the review thread"),
		{
			id: "open-diff",
			label: "Open Diff",
	...
	fix: [
		B("run-tests", "Run Tests", "Run the test suite against the patch (vitest)", "Execute the test suite now"),
		B("root-cause", "Root-Cause", "Trace the defect to its root cause before patching", "Drive the diagnosis to the root cause"),
		B("apply-patch", "Apply Patch", "Apply the prepared patch to the workspace", "Apply the prepared patch"),
		B("rollback", "Rollback", "Roll the workspace back to the last good state", "Revert to the last good state", "danger")
	],
	"root-cause-analysis": [
		B("hypothesize", "Hypothesize", "Form the next failure hypothesis", "Record a fresh failure hypothesis"),
		B("bisect", "Bisect", "Bisect the history to isolate the breaking change", "Binary-search the breaking change"),
		B("instrument", "Instrument", "Add instrumentation around the suspect path", "Wire probes around the suspect path"),
		B("conclude", "Conclude", "Conclude the analysis and write up the finding", "Close the analysis with a finding")
	],
	polish: [
		B("capture-plates", "Capture Plates", "Capture fresh screenshot plates of the surfaces", "Capture screenshot plates"),
		B("score", "Score", "Score the current polish pass against the rubric", "Score the polish pass"),
		B("apply-findings", "Apply Findings", "Apply the scored findings to the surfaces", "Apply the scored findings")
	],
	implement: [
		B("run-tests", "Run Tests", "Run the test suite against the implementation (vitest)", "Execute the test suite now"),
		{
			id: "open-diff",
			label: "Open Diff",
			intent: { kind: "open-diff" },
		...
		B("checkpoint", "Checkpoint", "Checkpoint the working state before continuing", "Snapshot the working state")
	],
	"test-coverage": [
		B("run-suite", "Run Suite", "Run the full coverage suite", "Execute the full suite"),
		B("coverage-report", "Coverage Report", "Produce a coverage report for the touched modules", "Report coverage on touched modules"),
		B("add-cases", "Add Cases", "Add test cases for the uncovered branches", "Author cases for uncovered branches")
	],
	docs: [
		B("preview", "Preview", "Render a preview of the drafted pages", "Preview the drafted pages"),
		B("spell-gauge", "Spell-Gauge", "Run the spell-gauge over the prose", "Measure the prose with the spell-gauge"),
		B("publish-draft", "Publish Draft", "Publish the current draft for review", "Publish the draft")
	],
	deploy: [
		B("dry-run", "Dry Run", "Execute a deployment dry run", "Rehearse the deployment"),
		B("ship-it", "Ship It", "Ship the release into the cold void", "Execute the deployment for real", "danger"),
		B("hold-the-line", "Hold the Line", "Hold the deployment — keep the current release pinned", "Freeze the rollout where it stands")
	],
	research: [
		B("summarize", "Summarize", "Summarize the findings so far", "Condense the findings"),
		B("cite-sources", "Cite Sources", "Attach citations to every claim", "Pin citations to the claims"),
		B("archive-to-brain", "Archive to Brain", "Archive the findings to the company brain (§V2-3)", "Write the findings back to memory")
	],
	migrate: [
		B("plan-steps", "Plan Steps", "Plan the migration steps end to end", "Lay out the migration steps"),
		B("execute-step", "Execute Step", "Execute the next migration step", "Run the next migration step"),
		B("verify-parity", "Verify Parity", "Verify data parity between old and new paths", "Check parity across both paths")
	]
}, we = {
	id: "edit-card",
	label: "Edit Card",
...
function Fe(e) {
	...
	if (r.length > me.length) {
		let e = r.filter((e) => i.some((t) => t.id === e.id));
		return [...r.filter((e) => !i.some((t) => t.id === e.id)).slice(0, me.length - e.length), ...e];
	}
	return r;
}
function Ie(e, t) {
	switch (e) {
		case "backlog": return De;
		case "do": return Fe(t);
		case "ai-review": return Oe;
		case "human-review": return ke;
		case "approved": return t.every((e) => e.merged) ? je : Ae;
		case "merged": return Me;
		case "in-production": return Ne;
	}
}
function Le(e) {
	let t = [...new Set(e.map((e) => e.column))];
	if (t.length === 1) return Ie(t[0], e);
	let n = e.map((e) => Ie(e.column, [e])), r = (n[0] ?? []).filter((e) => n.every((t) => t.some((t) => t.id === e.id)));
	return r.length > 0 ? r : Se;
}
function Re(e) {
	let t = e.selection.states;
	if (t.length === 1 && t[0] === "awaiting_approval") return xe;
	if (t.every((e) => Pe.has(e) || e === "awaiting_approval")) {
		let t = e.cards.filter((e) => e.column === "do");
		return t.length > 0 ? Fe(t) : [...be, Te];
	}
	return Se;
}
function ze(e) {
	let t;
	return t = e.selection.count === 0 ? ye : e.selection.kinds.length === 1 && e.selection.kinds[0] === "task" && e.cards.length > 0 ? Le(e.cards) : e.selection.kinds.length === 1 && e.selection.kinds[0] === "unit" ? Re(e) : Se, t.slice(0, me.length).map((t, n) => ve(t, e, me[n])).map((t) => Be(t, e));
}
function Be(e, t) {
	return e.id === "toggle-sim" ? t.fleet.simPaused ? {
		...e,
		label: "Resume Sim",
		icon: ge["resume-sim"] ?? e.icon,
...
var Ve = {
	generateCommands: ze,
	generateIcon: (e) => z(e),
	generateOptionIcon: (e) => pe(e),
	suggestCompletion: (e) => O(e)
}, He = me.length, Ue = new Set(me);
function We(e) {
	let t = /^Key([A-Z])$/.exec(e)?.[1];
	return t !== void 0 && Ue.has(t) ? t : null;
}
function Ge(e) {
	return Ve.generateCommands(h(e)).slice(0, He);
}
function Ke(e, t) {
	return Ge(e).find((e) => e.hotkey === t);
}
function qe(e) {
...
function Ye(e, t, n) {
	let r = t.getState(), { tasks: i } = d(r), a = qe(r);
	switch (e.kind) {
		case "steer":
			r.openSteer();
			return;
...
function Xe(e, t, n) {
	let r = Ke(t.getState(), e);
	return r === void 0 ? !1 : (r.enabled && Ye(r.intent, t, n), !0);
}
//#endregion
...
function $e({ store: e, orders: t }) {
	let n = (n) => {
		if (Ze(n.target)) return;
		switch (n.code) {
			case "Escape":
				e.getState().escape();
				return;
		...
		if (r === null) return;
		let i = e.getState();
		i.meta.foundryOpen || i.meta.archiveOpen || i.meta.steerOpen || i.meta.cardEditorTaskId !== null || i.meta.ideTaskId !== null || (n.repeat || Xe(r, e, t)) && n.preventDefault();
	};
...
var it = "kradle.a5c.ai/v1alpha1", at = 177e10, ot = [
	"claude-code",
	"codex",
	"gemini-cli",
	"pi"
...
], ft = Object.fromEntries(st.map((e) => [e, dt.find((t) => t.stack.spec.adapter === ct[e]).stackRef])), pt = {
...
function vt(e, t) {
	...
	for (let t of gt) for (let a of t.stems) n.push({
		...
		attributes: {
			...
			updatedAt: (/* @__PURE__ */ new Date(at - e.int(1, 90) * 864e5)).toISOString()
		}
	});
	for (let t of n) {
		let r = e.int(1, 3), i = {};
...
function yt(e) {
	...
	})), i = [], a = 0, o = (e) => (a += 1, `adr-${String(a).padStart(2, "0")}-${e}`), s = t.shuffle(st), c = t.int(6, 8);
	for (let e = 0; e < c; e += 1) {
...
var Ut = class {
	...
	constructor(e) {
		...
		for (let e of st) this.templates.set(e, {
			revision: 1,
			phases: [...wt[e]]
		});
		let t = [
	...
	createTask(e) {
		if (!st.includes(e.taskKind)) return this.emit({
			type: "error",
	...
	updateTask(e, t) {
		...
		if (t.taskKind !== void 0 && !st.includes(t.taskKind)) return this.emit({
			type: "error",
			code: "invalid_task_kind",
	...
	upsertStack(e) {
		...
		let i = {
			...
			spec: {
				...
				model: n.model ?? U[this.adapterOfStackSpec(n)][0],
				prompt: {
	...
	rosterAgentView(e) {
		let t = this.stacks.get(e.stackRef), n = t ? this.adapterOfStackSpec(t.stack.spec) : "claude-code", r = t?.stack.spec.model || U[n][0];
		return {
			agentId: e.agentId,
			name: e.name,
			stackRef: e.stackRef,
	...
	enterAiReview(e) {
		let t = e.reviewerAgentId ? this.rosterAgents.get(e.reviewerAgentId) : null, n = e.workerAdapter ?? ct[e.taskKind], r = t ? 1 : this.rng.int(1, 2), i = ot.filter((e) => e !== n), a = this.latestSession((t) => t.taskId === e.taskId && t.role === "worker");
		for (let n = 0; n < r; n += 1) {
	...
	enterApproved(e) {
		let t = this.rng.pick(ot), n = (e.approvingReviewSessionId === null ? void 0 : this.sessions.get(e.approvingReviewSessionId)) ?? this.latestSession((t) => t.taskId === e.taskId && t.role === "worker");
	...
	spawnAgent(e, t, n, r, i) {
		...
		this.agentCounter += 1;
		let o = this.now(), s = `agt-${String(this.agentCounter).padStart(3, "0")}-${t}`, c = a.stack.spec.model || U[e][0], l = this.rng.int(2, 5), u = this.cards.get(n), d = this.createSession({
			sessionId: s,
			role: t,
			coordination: !1,
			taskId: n,
	...
	openCoordinationSession(e) {
		...
		let i = this.createSession({
			...
			taskId: e.taskId,
			agent: r,
			model: n.stack.spec.model || U[r][0],
			stackRef: t,
	...
	adapterOfStackSpec(e) {
		...
		for (let e of t) if (ot.includes(e)) return e;
		return "claude-code";
	}
	unseal(e) {
...
function pn({ agentIds: e, agents: t, store: r }) {
	...
	return /* @__PURE__ */ u("span", {
		...
		children: [
			d.map((e) => {
				let n = t[e], i = n?.agent ?? "claude-code", a = z({
					entityId: e,
					kind: "unit",
					adapter: i
				});
				return /* @__PURE__ */ l("button", {
					...
					className: H("wr-card-agent", `wr-card-agent--${i}`, "is-spawning"),
		...
		children: [
			/* @__PURE__ */ u("div", {
				...
				children: [/* @__PURE__ */ l("button", {
					type: "button",
					className: H("wr-assign-chip", "wr-assign-chip--worker", c !== null && "is-set"),
					...
					children: [
						...
						a.map((t) => /* @__PURE__ */ u("button", {
							type: "button",
							className: H("wr-assign-pop-opt", e.workerAgentId === t.agentId && "is-selected", t.status === "assigned" && t.agentId !== e.workerAgentId && "is-busy"),
							onClick: (r) => {
			...
			/* @__PURE__ */ u("div", {
				...
				children: [/* @__PURE__ */ l("button", {
					type: "button",
					className: H("wr-assign-chip", "wr-assign-chip--reviewer", d !== null && "is-set"),
					...
					children: [
						...
						s.map((t) => /* @__PURE__ */ u("button", {
							type: "button",
							className: H("wr-assign-pop-opt", e.reviewerAgentId === t.agentId && "is-selected", t.status === "assigned" && t.agentId !== e.reviewerAgentId && "is-busy"),
							onClick: (r) => {
			...
			/* @__PURE__ */ l("div", {
				...
				children: /* @__PURE__ */ l("button", {
					type: "button",
					className: H("wr-assign-chip", "wr-assign-chip--human", f && "is-set"),
...
function hn({ card: e, orders: t, mini: n }) {
	...
	return /* @__PURE__ */ u("div", {
		className: H("wr-card-body", n && "wr-card-body--mini"),
		children: [
			/* @__PURE__ */ l("span", {
			...
			/* @__PURE__ */ u("span", {
				...
				children: [/* @__PURE__ */ l("span", {
					...
					children: [
						...
						!n && (e.costUsd > 0 || e.durationMs !== null || e.tokensBurned > 0) && /* @__PURE__ */ u("span", {
							className: "wr-card-cost",
							title: `${e.tokensBurned.toLocaleString()} tokens${e.costUsd > 0 ? ` · $${e.costUsd.toFixed(4)}` : ""}${e.durationMs === null ? "" : ` · ${Math.round(e.durationMs / 1e3)}s`}`,
							children: [
								e.durationMs !== null && `${Math.round(e.durationMs / 1e3)}s`,
								e.costUsd > 0 && `${e.durationMs === null ? "" : " · "}$${e.costUsd.toFixed(2)}`,
								e.costUsd <= 0 && e.durationMs === null && e.tokensBurned > 0 && `${e.tokensBurned.toLocaleString()}tok`
							]
						}),
			...
			/* @__PURE__ */ l("button", {
				...
				className: H("wr-card-yolo", e.yolo && "is-on"),
				"aria-pressed": e.yolo,
...
function gn({ card: e, allCards: t, agents: r, rosterAgents: i, store: c, orders: d, selected: f, onHoverLane: p }) {
	...
	return /* @__PURE__ */ u("div", {
		...
		role: "button",
		tabIndex: 0,
		className: H("wr-card", T && "wr-card--stack", e.merged && "wr-card--merged", e.compacted && "wr-card--compact", f.has(e.taskId) && "is-selected", C && "is-draggable", A && "is-dragging", x && "is-snapback"),
		style: M,
		onPointerDown: D,
		onClick: O,
		onDoubleClick: k,
		...
		children: [
			...
			T && /* @__PURE__ */ l("div", {
				...
				children: w.map((e, t) => /* @__PURE__ */ u("div", {
					...
					role: "button",
					tabIndex: 0,
					className: H("wr-card wr-card--mini", f.has(e.taskId) && "is-selected"),
...
function _n({ store: e, orders: t }) {
	let i = V(e, (e) => e.board), s = V(e, (e) => e.selection.ids), c = V(e, (e) => e.meta.movingCards), d = V(e, (e) => e.meta.reviewTaskId), f = a(null), p = a(/* @__PURE__ */ new Map()), m = a(/* @__PURE__ */ new Map()), h = a(/* @__PURE__ */ new Map()), [g, _] = o(null), v = i.cardIds.map((e) => i.cards[e]?.view).filter((e) => e !== void 0), y = new Set(s);
	r(() => {
		let t = f.current;
		if (t === null) return;
	...
	return /* @__PURE__ */ l("div", {
		...
		children: Ct.map((n) => {
			...
			return /* @__PURE__ */ u("section", {
				...
				className: H("wr-lane", `wr-lane--${n}`, g !== null && s && "is-drop-legal", o && s && "is-drop-target", o && !s && "is-drop-illegal"),
				"aria-label": Kt[n],
				children: [
					/* @__PURE__ */ u("header", {
	...
	return /* @__PURE__ */ l("div", {
		...
		children: e.options.map((r) => {
			...
			return /* @__PURE__ */ u("button", {
				...
				className: H("wr-inq-opt", r.tone === "danger" && "wr-inq-opt--danger", r.tone === "primary" && "wr-inq-opt--primary"),
...
function Sn({ store: e, orders: t }) {
	let r = V(e, (e) => e.board.inquiries), i = V(e, (e) => e.board.agents), s = V(e, (e) => e.meta.dockPulse), c = a(null), [d, f] = o([]), [p, m] = o(!1);
	n(() => {
		if (s === 0) return;
	...
	return /* @__PURE__ */ u("div", {
		ref: c,
		className: H("wr-dock", p && "is-pulsing", _ && "wr-dock--empty"),
		...
		children: [
			...
			/* @__PURE__ */ u("div", {
				...
				children: [d.map((e) => /* @__PURE__ */ u("div", {
					...
					let n = i[e.unitId]?.agent ?? "claude-code", r = z({
						entityId: e.unitId,
						kind: "unit",
						adapter: n
					});
...
function Cn({ store: e, orders: t }) {
	let n = Ge({
		world: V(e, (e) => e.world),
		board: V(e, (e) => e.board),
		selection: V(e, (e) => e.selection),
		alerts: V(e, (e) => e.alerts),
		meta: V(e, (e) => e.meta)
	}), r = Math.max(0, He - n.length), i = (n) => {
		n.enabled && Ye(n.intent, e, t);
	};
	return /* @__PURE__ */ u("section", {
		...
		children: [/* @__PURE__ */ l("div", {
			...
			children: [n.map((e) => /* @__PURE__ */ u("button", {
				...
				className: H("wr-cmd", e.severity === "danger" && "wr-cmd--danger", e.severity === "urgent" && "wr-cmd--urgent"),
				disabled: !e.enabled,
...
function En({ store: e }) {
	let t = V(e, (e) => e.events), n = V(e, (e) => e.meta.simStartMs), [r, i] = o(null), a = t, s = r ?? a, c = (t) => {
	...
	return /* @__PURE__ */ u("section", {
		...
		children: [/* @__PURE__ */ u("div", {
			...
			children: [s.map((e) => /* @__PURE__ */ l("li", { children: /* @__PURE__ */ u("button", {
				...
				className: H("wr-ticker-item", `wr-ticker-item--${e.severity}`, e.entityId !== void 0 && "wr-ticker-item--linked"),
				onClick: () => c(e),
				children: [
					/* @__PURE__ */ l("span", {
	...
	return tt((e, t) => ({
		...
		commitTick(t) {
			e((e) => {
				...
				let F = 0, I = 0, L = 0;
				for (let e of t.units) Bn.has(e.state) && (F += 1), I += Vn(e), L += e.cost.totalUsd;
				let ee = t.tasks.filter((e) => e.state === "done").length, te = {
					unitCount: t.units.length,
					busyCount: F,
					idleCount: t.units.filter((e) => e.state === "idle").length,
					tokensBurned: I,
					costUsd: Math.round(L * 100) / 100,
					tasksDone: ee,
					tasksTotal: t.tasks.length,
					alertCount: w.length
				}, R = d || s || !p || !l || a.runToUnit !== n.runToUnit ? {
					units: u,
					unitIds: p ? n.unitIds : f,
					tasks: o,
					taskIds: l ? n.taskIds : c,
					runToUnit: a.runToUnit
				} : n, ne = t.rosterAgents, z = ne !== r.rosterAgents && (ne.length !== r.rosterAgents.length || ne.some((e, t) => e !== r.rosterAgents[t])), re = t.personas.length !== r.personas.length || t.personas.some((e, t) => e !== r.personas[t]), ie = re ? t.personas : r.personas, ae = t.definitions.length !== r.definitions.length || t.definitions.some((e, t) => e !== r.definitions[t]), oe = ae ? t.definitions : r.definitions;
				return {
					world: R,
					board: h || y || !_ || !x || S !== r.inquiries || A !== r.heldByCard || z || re || ae ? {
						cards: m,
...
function ir({ turnCount: e }) {
	...
	return /* @__PURE__ */ l("span", {
		...
		"aria-label": `rank ${t}`,
		children: Array.from({ length: er }, (e, n) => /* @__PURE__ */ l("span", { className: H("wr-rank-chevron", n < t && "is-earned") }, n))
	});
}
function ar({ store: e, unit: t }) {
	let n = t.view.taskId, r = V(e, (e) => n === null ? void 0 : e.world.tasks[n]);
	return /* @__PURE__ */ u("div", {
...
function sr({ store: e, views: t, task: n }) {
	let r = V(e, (e) => e.world.units), i = V(e, (e) => e.board.cards[n.id]?.runStage ?? null);
	V(e, (e) => e.meta.tickIndex);
...
function dr({ store: e, orders: t }) {
	let n = V(e, (e) => e.meta), r = n.resources;
...
function vr({ store: e, orders: t, views: n, card: r }) {
	let i = V(e, (e) => e.board.cards), a = V(e, (e) => e.board.cardIds), [s, c] = o(() => fr(r.view)), d = a.map((e) => i[e]?.view).filter((e) => e !== void 0), f = mr(r.id, d), p = hr(d), m = n.listStacks(), h = pr(r.view), g = () => e.getState().closeCardEditor();
	return /* @__PURE__ */ l("div", {
			...
			children: [
				...
				/* @__PURE__ */ u("div", {
					...
					children: [/* @__PURE__ */ l("span", {
						...
						role: "switch",
						"aria-checked": s.yolo,
						className: H("wr-card-editor-yolo", s.yolo && "is-on"),
						onClick: () => c({
							...s,
							yolo: !s.yolo
						}),
...
function yr({ store: e, orders: t, views: n }) {
	let r = V(e, (e) => e.meta.cardEditorTaskId), i = V(e, (e) => e.meta.cardEditorTaskId === null ? void 0 : e.board.cards[e.meta.cardEditorTaskId]);
...
function Cr(e) {
	let t = e?.requests?.cpu;
	if (t === void 0) return "";
	for (let e of [
		"small",
		"medium",
		"large"
	]) if (Sr[e].requests?.cpu === t) return e;
	return "";
}
function wr(e) {
...
function Dr(e) {
	...
	return {
		...
		mcpServerRefs: Tr(t.externalTools?.mcpServerRefs),
		cliToolRefs: Tr(t.externalTools?.cliToolRefs),
		skillRefs: Tr(t.skillRefs),
		subagentRefs: Tr(t.subagentRefs),
		contextLabelRefs: Tr(t.contextLabelRefs),
		workspacePolicyRef: t.workspacePolicyRef ?? "",
		runnerPool: t.runnerPool ?? "",
		roleBindings: Tr(t.permissionRefs?.roleBindings),
		secretGrants: Tr(t.permissionRefs?.secretGrants),
		configGrants: Tr(t.permissionRefs?.configGrants),
		memoryRepositoryRefs: Tr(t.memoryRepositoryRefs),
		displayName: t.displayName ?? "",
		agentRole: Tr(t.agentRole?.refs),
		size: Cr(t.resources)
	};
}
function Or(e) {
...
function Ar(e, t) {
	let n = ot.includes(t) ? t : null;
	return {
		...e,
		adapter: t,
		baseAgent: t,
		model: n === null ? e.model : U[n][0]
	};
}
function jr(e) {
	let t = e.name.trim();
	if (t === "") return null;
	let n = wr(e.mcpServerRefs), r = wr(e.cliToolRefs), i = wr(e.skillRefs), a = wr(e.subagentRefs), o = wr(e.contextLabelRefs), s = wr(e.memoryRepositoryRefs), c = wr(e.roleBindings), l = wr(e.secretGrants), u = wr(e.configGrants), d = wr(e.agentRole), f = {
...
var Pr = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 20 20\" width=\"100%\" height=\"100%\" aria-hidden=\"true\"><path d=\"M3 6.5 H17 C16.4 9.2 14.2 10.6 11.6 10.9 L11.6 13.2 H13.6 C14.3 13.2 14.8 13.7 14.8 14.4 V15.4 H5.2 V14.4 C5.2 13.7 5.7 13.2 6.4 13.2 H8.4 L8.4 10.9 C6.6 10.7 5.4 9.9 4.8 8.6 L3 8.2 Z\" fill=\"currentColor\"/></svg>";
function Fr({ store: e, orders: t }) {
	let n = V(e, (e) => e.board.cardIds), r = V(e, (e) => e.board.cards), [i, a] = o("implement"), [s, d] = o(""), [f, p] = o(""), m = n.filter((e) => r[e]?.view.parentId === null), h = pt[i], g = () => {
		t.createTask({
			taskKind: i,
			...s.trim() === "" ? {} : { title: s.trim() },
			...f === "" ? {} : { parentId: f }
...
function Ir({ stack: e, onForgeFrom: t, onEdit: n }) {
	let r = e.stack.spec;
...
function Lr({ orders: e, views: t }) {
	let [n, r] = o(Er), [, i] = o(0), a = t.listStacks(), s = xr.includes(n.approvalMode) ? xr : [n.approvalMode, ...xr];
	return /* @__PURE__ */ u("div", {
		className: "wr-foundry-stacks-body",
		children: [/* @__PURE__ */ l("ul", {
			className: "wr-stack-roster",
			"aria-label": "Agent stack roster",
			children: a.map((e) => /* @__PURE__ */ l(Ir, {
				stack: e,
				onForgeFrom: () => r(Or(e)),
				onEdit: () => r(kr(e))
			}, e.stackRef))
				...
				/* @__PURE__ */ u("div", {
					...
					children: [
						/* @__PURE__ */ u("label", {
							...
							children: [
								...
								/* @__PURE__ */ l("select", {
									className: "wr-foundry-input",
									value: n.adapter,
									onChange: (e) => r(Ar(n, e.target.value)),
									children: ot.map((e) => /* @__PURE__ */ l("option", {
										value: e,
										children: e
									}, e))
								}),
								/* @__PURE__ */ l(br, {
									...
									onSelect: (e) => r(Ar(n, e.displayName || e.id))
								})
							]
						}),
						/* @__PURE__ */ u("label", {
						...
						/* @__PURE__ */ u("label", {
							...
							children: [/* @__PURE__ */ l("span", {
								className: "wr-foundry-label",
								children: "size (agent Job resources)"
							}), /* @__PURE__ */ u("select", {
								className: "wr-foundry-input",
								value: n.size,
								onChange: (e) => r({
								...
								children: [
									/* @__PURE__ */ l("option", {
										value: "",
										children: "default (deployment floor)"
									}),
									/* @__PURE__ */ l("option", {
										value: "small",
										children: "small — 25m / 512Mi (fits busy clusters)"
									}),
									/* @__PURE__ */ l("option", {
										value: "medium",
										children: "medium — 250m / 1Gi"
									}),
									/* @__PURE__ */ l("option", {
										value: "large",
										children: "large — 1000m / 2Gi"
									})
								]
							})]
						}),
						...
						/* @__PURE__ */ u("label", {
							...
							children: [
								...
								/* @__PURE__ */ l(br, {
									...
									onSelect: (e) => r({
										...n,
										mcpServerRefs: Nr(n.mcpServerRefs, e.displayName || e.id)
									})
								})
							]
						}),
						/* @__PURE__ */ u("label", {
							...
							children: [
								...
								/* @__PURE__ */ l(br, {
									...
									onSelect: (e) => r({
										...n,
										cliToolRefs: Nr(n.cliToolRefs, e.displayName || e.id)
									})
								})
							]
						}),
						/* @__PURE__ */ u("label", {
							...
							children: [
								...
								/* @__PURE__ */ l(br, {
									...
									onSelect: (e) => r({
										...n,
										skillRefs: Nr(n.skillRefs, e.displayName || e.id)
									})
								})
							]
						}),
						...
						/* @__PURE__ */ u("label", {
							...
							children: [
								...
								/* @__PURE__ */ l(br, {
									...
									onSelect: (e) => r({
										...n,
										agentRole: Nr(n.agentRole, e.displayName || e.id)
									})
								})
							]
						}),
...
function Vr({ orders: e, views: t }) {
	let n = t.listDefinitions(), r = t.listPersonas(), i = t.listStacks(), [, a] = o(0), [s, c] = o(""), [d, f] = o(r[0]?.name ?? ""), [p, m] = o(i[0]?.stackRef ?? ""), [h, g] = o(""), [_, v] = o(""), [y, b] = o(""), [x, S] = o(""), [C, w] = o(""), [T, E] = o("direct"), [D, O] = o("professional"), [k, A] = o(""), [j, M] = o(""), [N, P] = o("nova"), [F, I] = o(""), L = s.trim() !== "" && d !== "" && p !== "", ee = () => {
		L && e.upsertDefinition({
			name: s.trim(),
			personaRef: d,
			stackRef: p,
	...
	return /* @__PURE__ */ u("div", {
		...
		children: [
			/* @__PURE__ */ u("ul", {
				...
				}), n.map((e) => /* @__PURE__ */ l(zr, { def: e }, e.name))]
			}),
			/* @__PURE__ */ u("div", {
				...
				children: [/* @__PURE__ */ l("div", {
					...
					}), r.map((e) => /* @__PURE__ */ l(Br, { persona: e }, e.name))]
				})]
			}),
			/* @__PURE__ */ u("div", {
				className: "wr-stack-editor",
							...
							/* @__PURE__ */ u("label", {
								...
								children: [
									...
									/* @__PURE__ */ l(br, {
										...
										onSelect: (e) => I(Nr(F, e.displayName || e.id))
									})
								]
							})
						]
...
function Hr({ store: e, orders: t, views: r }) {
	let i = V(e, (e) => e.meta.foundryOpen), a = V(e, (e) => e.meta.foundryTab);
	V(e, (e) => e.meta.tickIndex);
	...
	return n(() => {
		...
		"data-testid": "foundry",
		children: /* @__PURE__ */ u("div", {
			className: H("wr-modal wr-foundry", (s === "stacks" || s === "agents") && "wr-foundry--wide"),
			role: "dialog",
				...
				/* @__PURE__ */ u("div", {
					...
					children: [
						/* @__PURE__ */ l("div", {
							...
							className: H("wr-foundry-tab", s === "commission" && "is-active"),
						...
						/* @__PURE__ */ l("div", {
							...
							className: H("wr-foundry-tab", s === "stacks" && "is-active"),
							onClick: () => c("stacks"),
							onKeyDown: (e) => {
						...
						/* @__PURE__ */ l("div", {
							...
							className: H("wr-foundry-tab", s === "agents" && "is-active"),
							onClick: () => c("agents"),
							onKeyDown: (e) => {
...
var Jr = "const|let|var|function|return|if|else|for|while|do|import|export|from|new|class|extends|implements|interface|type|enum|async|await|switch|case|break|continue|typeof|instanceof|in|of|this|null|undefined|true|false|throw|try|catch|finally|readonly|public|private|protected|static|void|never|unknown|number|string|boolean|default|as|satisfies|keyof|yield|delete|get|set", Yr = {
	ts: {
		re: new RegExp([
			String.raw`(\/\/.*$|\/\*[\s\S]*?\*\/)`,
			String.raw`('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|` + "`(?:[^`\\\\]|\\\\.)*`)",
...
var Zr = 400, Qr = 80;
function $r({ node: e, depth: t, changed: n, collapsed: r, onToggleDir: i, onOpenFile: a, activePath: o }) {
	...
	if (e.type === "dir") {
		...
		}), d && (e.children ?? []).map((e) => /* @__PURE__ */ l($r, {
			node: e,
			depth: t + 1,
			changed: n,
			collapsed: r,
	...
	let d = n.get(e.path);
	return /* @__PURE__ */ u("div", {
		className: H("wr-ide-entry wr-ide-entry--file", o === e.path && "is-open"),
		style: s,
		role: "treeitem",
		onClick: () => a(e.path),
		children: [/* @__PURE__ */ l("span", {
...
function ei({ store: e, orders: t, views: r }) {
	let i = V(e, (e) => e.meta.ideTaskId);
	V(e, (e) => e.meta.tickIndex);
	...
	let A = d === null ? "" : p[d] ?? "";
	if (n(() => (E.current !== null && window.clearTimeout(E.current), E.current = window.setTimeout(() => w(A), Qr), () => {
	...
	let P = (e) => {
		...
		if (d !== null) {
			if (n.key === "Tab" && y !== null && y.path === d) {
				n.preventDefault();
				let e = Gr(p[d] ?? "", n.currentTarget.selectionStart, y);
				m((t) => ({
					...t,
					[d]: e.text
				})), g((e) => ({
	...
	}, z = d === null ? "plain" : qr(d), re = Wr(A, x).lineIndex, ie = C.split("\n"), ae = ie.length > 400, oe = ae ? ie.slice(0, 400) : ie;
	return /* @__PURE__ */ u("div", {
		className: "wr-ide",
		...
		children: [/* @__PURE__ */ u("header", {
			...
			children: [/* @__PURE__ */ u("nav", {
				...
				}), j !== null && (j.children ?? []).map((e) => /* @__PURE__ */ l($r, {
					node: e,
					depth: 0,
					changed: N,
					collapsed: _,
				...
				children: [/* @__PURE__ */ u("div", {
					...
					children: [s.map((e) => /* @__PURE__ */ u("span", {
						"data-testid": `ide-tab-${Ur(e)}`,
						role: "tab",
						"aria-selected": e === d,
						className: H("wr-ide-tab", e === d && "is-active", h[e] === !0 && "is-dirty"),
						onClick: () => {
							e !== d && F(e);
						},
						children: [
					...
					children: [/* @__PURE__ */ u("div", {
						...
						children: [/* @__PURE__ */ l("div", {
							...
							children: oe.map((e, t) => /* @__PURE__ */ l("div", {
								className: H("wr-ide-lineno", t === re && "is-lit"),
								children: t + 1
							}, t))
						}), /* @__PURE__ */ l("pre", {
							className: "wr-ide-highlight",
							children: /* @__PURE__ */ u("code", { children: [oe.map((e, t) => /* @__PURE__ */ u("div", {
								className: H("wr-ide-line", t === re && "is-lit"),
								children: [
									Xr(e, z).map((e, t) => e.cls === "plain" ? /* @__PURE__ */ l("span", { children: e.text }, t) : /* @__PURE__ */ l("span", {
										className: `tok-${e.cls}`,
										children: e.text
									}, t)),
...
var G = {
	width: 1200,
	height: 760
}, oi = 34, si = 30, ci = 2.399963229728653;
function li(e) {
	return e.replace(/:/g, "-");
}
var ui = [
...
function pi(e) {
	let t = e <= 1 ? 1 : e <= 4 ? 2 : Math.ceil(Math.sqrt(e)), n = Math.ceil(e / t), r = (G.width - oi * (t + 1)) / t, i = (G.height - oi * (n + 1)) / n, a = [];
	for (let n = 0; n < e; n += 1) {
...
function mi(e, t = []) {
	...
	let c = pi(i.length), l = [], u = /* @__PURE__ */ new Map(), d = [];
	i.forEach((e, t) => {
		let n = c[t];
		e !== "" && d.push({
			silo: e,
		...
		let r = o.get(e) ?? [], i = n.x + n.width / 2, a = n.y + (n.height + si) / 2, s = Math.min(n.width, n.height - si) / 2 - 26, f = Math.sqrt(Math.max(1, r.length));
		r.forEach((t, n) => {
			let r = Math.sqrt(n + .55) / f * s, o = n * ci, c = {
				id: t.id,
				nodeKind: t.nodeKind,
				silo: e,
				x: K(i + Math.cos(o) * r),
...
function gi({ store: e, views: t, taskId: n, unitId: r }) {
	let i = V(e, (e) => e.board.memory.silos), a = ti(n, r);
	if (a === null) return /* @__PURE__ */ l("div", {
		className: "wr-inspector-body wr-tab-empty",
		children: "no memory traffic — nothing to inspect"
	});
	let o = t.getMemoryIO(a), s = new Map(o.read.map((e) => [e.recordId, e.kind])), d = ri(o.read).map((e) => ({
		key: e,
		hue: di(s.get(e) ?? "")
	})), f = ii(o.written).map((e) => ({
		key: e,
		hue: di("proposal")
	})), p = (t) => {
		...
		children: [/* @__PURE__ */ u("section", {
					...
					children: o.read.map((e, t) => /* @__PURE__ */ u("li", {
						...
						children: [
							...
							/* @__PURE__ */ l("span", {
								...
								style: { "--mem-hue": String(di(e.kind)) },
								children: e.kind
							}),
							" ",
							/* @__PURE__ */ l("span", {
			...
			children: [
				...
				o.written.length === 0 ? /* @__PURE__ */ l("div", {
					...
					children: [ai(o.written), " proposed changes"]
				}), /* @__PURE__ */ l("ul", {
					className: "wr-memio-list",
					children: o.written.map((e, t) => /* @__PURE__ */ u("li", {
						...
						onClick: () => p(ni(i, e.silo)),
						onKeyDown: (t) => {
							(t.key === "Enter" || t.key === " ") && p(ni(i, e.silo));
						},
						children: [
							/* @__PURE__ */ l("span", {
								className: "wr-memio-id",
...
function _i({ session: e }) {
	return /* @__PURE__ */ u(c, { children: [
		/* @__PURE__ */ l("span", {
			className: H("wr-sess-role", `wr-sess-role--${e.role}`),
			children: e.role
		}),
...
function vi({ session: e }) {
	return /* @__PURE__ */ l("span", {
		className: H("wr-sess-status", `wr-sess-status--${e.status}`),
		children: e.status
	});
}
function yi({ session: e }) {
	return /* @__PURE__ */ u(c, { children: [/* @__PURE__ */ l(_i, { session: e }), /* @__PURE__ */ l(vi, { session: e })] });
}
function bi(e) {
	return `T${_(e.startedTick)} → ${e.endedTick === null ? "now" : `T${_(e.endedTick)}`}`;
}
function xi({ node: e, views: t, onOpen: n }) {
	let r = e.session, i = z({
	...
	return /* @__PURE__ */ u(c, { children: [/* @__PURE__ */ u("div", {
			...
			/* @__PURE__ */ u("span", {
				...
				children: [
					...
					/* @__PURE__ */ l("span", {
						...
						children: /* @__PURE__ */ l(_i, { session: r })
					}),
					/* @__PURE__ */ u("span", {
						className: "wr-sess-chiprow wr-sess-chiprow--line",
						children: [/* @__PURE__ */ l(vi, { session: r }), a !== void 0 && /* @__PURE__ */ u("button", {
							type: "button",
		...
		children: e.children.map((e) => /* @__PURE__ */ l(xi, {
			node: e,
			views: t,
			onOpen: n
		}, e.session.sessionId))
...
function Si({ sessions: e, views: n, onOpen: r }) {
	let i = jn(e);
		...
		case "tool_result": return /* @__PURE__ */ u("li", {
			...
			children: [
				/* @__PURE__ */ l("span", {
					...
					dangerouslySetInnerHTML: { __html: qi }
				}),
				/* @__PURE__ */ l("span", {
...
function wi({ sessionId: e, views: t, onOpen: r, onBack: i, hideParentLink: o = !1 }) {
	...
	return /* @__PURE__ */ u("div", {
		...
		children: [
				...
				children: [
					...
					/* @__PURE__ */ u("div", {
						...
						children: [/* @__PURE__ */ l("div", {
							...
							children: /* @__PURE__ */ l(yi, { session: p })
						})]
					}),
					/* @__PURE__ */ u("span", {
						className: "wr-sess-meta",
			...
			/* @__PURE__ */ u("ol", {
				...
				}), s.transcript.map((e) => /* @__PURE__ */ l(Ci, { entry: e }, e.seq))]
			})
		]
	});
}
function Ti({ views: e, taskId: t, childIds: r }) {
	let [i, a] = o(Mn);
	if (n(() => {
		a(Mn);
	...
	let s = (e) => a(Nn(e));
	return i.mode === "transcript" ? /* @__PURE__ */ l(wi, {
...
var Ei = "the cogitator does not know this incantation";
function Di(e) {
	return {
		lines: e,
		clear: !1
...
function Li(e, t) {
	...
	switch (r) {
		case "help": return Di(Ai());
		case "pwd": {
			let e = t.getWorkspaceView(t.taskId);
			return Di([`/workspaces/${t.workspaceId}/${ki(e)}`]);
		}
		case "ls": return Di(ji(t, a));
		case "cat": return Di(Mi(t, a));
		case "git": {
			let [e = "", ...n] = i;
			return Di(e === "status" ? Ni(t) : e === "diff" ? Pi(t, n.join(" ")) : e === "log" ? Fi(t) : [`${Ei}: git ${e}`.trimEnd()]);
		}
		case "npm": return i[0] === "test" ? Di(Ii(t)) : Di([`${Ei}: ${n}`]);
		case "clear": return {
			lines: [],
			clear: !0
		};
		default: return Di([`${Ei}: ${n}`]);
	}
}
function Ri(e, t, n) {
	if (e.length === 0) return {
...
var zi = ["AEGIS COGITATOR — remote shell consecrated", "speak `help` for the litany of incantations"];
function Bi({ taskId: e, workspaceId: t, views: r }) {
	let [i, s] = o(() => zi.map((e, t) => ({
		id: t,
		kind: "out",
		text: e
	}))), [c, d] = o(""), [f, p] = o([]), [m, h] = o(-1), g = a(zi.length), _ = a(null), v = a(null);
	if (n(() => {
		s(zi.map((e, t) => ({
			id: t,
			kind: "out",
			text: e
		}))), d(""), p([]), h(-1), g.current = zi.length;
	}, [e]), n(() => {
		let e = _.current;
				...
				/* @__PURE__ */ l("input", {
					...
					onKeyDown: (e) => {
						...
						if (e.key === "ArrowUp" || e.key === "ArrowDown") {
							e.preventDefault();
							let t = Ri(f, m, e.key === "ArrowUp" ? -1 : 1);
							h(t.index), d(t.text);
						}
					}
				}),
...
function Wi({ ws: e }) {
	...
	return /* @__PURE__ */ u("div", {
		...
		children: [
			...
			/* @__PURE__ */ u("span", {
				className: H("wr-ws-evidence", `wr-ws-evidence--${e.testEvidence.status}`),
...
function Gi({ diff: e }) {
	return /* @__PURE__ */ l("div", {
		...
		children: /* @__PURE__ */ l("div", {
			...
			children: Hi(e).map((e, t) => /* @__PURE__ */ u("div", {
				className: H("wr-diff-row", e.kind === "add" && "wr-diff-add", e.kind === "del" && "wr-diff-del", e.kind === "meta" && "wr-diff-meta"),
				children: [
					/* @__PURE__ */ l("span", {
...
function Ki({ files: e, openIndex: t, onToggle: n }) {
	return e.length === 0 ? /* @__PURE__ */ l("div", {
		...
		children: e.map((e, r) => /* @__PURE__ */ u("div", {
			...
			children: [/* @__PURE__ */ u("button", {
				...
				className: H("wr-ws-file", t === r && "is-open"),
				onClick: () => n(r),
				title: `${e.path} — click to ${t === r ? "close" : "open"} the diff plate`,
				children: [
					/* @__PURE__ */ l("span", {
						className: H("wr-ws-status", `wr-ws-status--${e.status}`),
						children: e.status
					}),
					/* @__PURE__ */ l("span", {
			...
			}), t === r && /* @__PURE__ */ l(Gi, { diff: e.diff })]
		}, e.path))
	});
}
//#endregion
//#region src/components/panels/Inspector.tsx
var qi = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 16 16\" width=\"100%\" height=\"100%\" aria-hidden=\"true\"><path d=\"M8 5.2 A2.8 2.8 0 1 0 8 10.8 A2.8 2.8 0 1 0 8 5.2 Z M8 1.5 V3.5 M8 12.5 V14.5 M1.5 8 H3.5 M12.5 8 H14.5 M3.4 3.4 L4.8 4.8 M11.2 11.2 L12.6 12.6 M12.6 3.4 L11.2 4.8 M4.8 11.2 L3.4 12.6\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.4\" stroke-linecap=\"round\"/></svg>", Ji = 28, Yi = [
	{
		id: "transcript",
		label: "Transcript"
	},
...
function Xi({ entry: e, live: t }) {
	switch (e.kind) {
		...
		case "tool": return /* @__PURE__ */ u("li", {
			className: H("wr-tr wr-tr--tool", e.toolStatus === "failed" && "wr-tr--tool-failed", e.toolStatus === "running" && "wr-tr--tool-running"),
			children: [
				/* @__PURE__ */ l("span", {
					className: "wr-tr-tool-glyph",
					dangerouslySetInnerHTML: { __html: qi }
		...
		case "thinking": return /* @__PURE__ */ u("li", {
			className: H("wr-tr wr-tr--thinking", t && "is-live"),
			children: [/* @__PURE__ */ l("span", {
				className: "wr-tr-role",
				children: "THINKING"
			}), e.text]
...
function Qi({ unit: e, inquiries: t, resolved: r, onChoose: i }) {
	...
	return /* @__PURE__ */ u("div", {
		...
		children: [/* @__PURE__ */ u("ol", {
			...
			onScroll: (e) => {
				let t = e.currentTarget, n = t.scrollHeight - t.scrollTop - t.clientHeight < Ji;
			...
			children: [
				...
				e.transcript.map((t) => /* @__PURE__ */ l(Xi, {
					entry: t,
					live: t === m && e.view.state === "thinking"
				}, t.id)),
				r.map((e) => /* @__PURE__ */ l(Zi, { resolved: e }, `res-${e.hookRequestId}`)),
...
function $i({ observation: e, simStartMs: t }) {
		...
		children: [
			...
			/* @__PURE__ */ l("div", {
				...
				children: e.phases.map((e) => /* @__PURE__ */ u("span", {
					className: H("wr-stage", e.status === "done" && "wr-stage--done", e.status === "current" && "wr-stage--now", e.status === "pending" && "wr-stage--pending"),
			...
			f.length > 0 && /* @__PURE__ */ l("div", {
				...
				children: f.map(([e, t]) => /* @__PURE__ */ u("span", {
					className: H("wr-effect-chip", e === "breakpoint" && t > 0 && "wr-effect-chip--pulse"),
					children: [
						e,
						" ×",
						t
...
function ea({ taskId: e, views: t }) {
	...
	return i === null ? /* @__PURE__ */ l("div", {
		...
		children: [/* @__PURE__ */ l(Wi, { ws: i }), /* @__PURE__ */ l("div", {
...
function na({ store: e, orders: t, views: n }) {
	let r = V(e, (e) => e.meta.inspectorUnitId), i = V(e, (e) => e.meta.inspectorTaskId), a = V(e, (e) => e.meta.inspectorTab), o = V(e, (e) => e.world.units), s = V(e, (e) => e.board), d = V(e, (e) => e.meta.simStartMs);
	V(e, (e) => e.meta.tickIndex);
	...
	return /* @__PURE__ */ u("aside", {
		...
		children: [
			...
			/* @__PURE__ */ l("nav", {
				...
				children: Yi.map(({ id: t, label: n }) => /* @__PURE__ */ l("button", {
					type: "button",
					"data-testid": `inspector-tab-${t}`,
					className: H("wr-inspector-tab", a === t && "is-active"),
			...
			a === "sessions" && /* @__PURE__ */ l(Ti, {
				views: n,
				taskId: p,
				childIds: m?.view.childIds ?? []
			}),
			a === "process" && /* @__PURE__ */ l($i, {
				observation: h,
				simStartMs: d
			}),
			a === "workspace" && /* @__PURE__ */ l(ea, {
				taskId: p,
				views: n
			}),
			a === "memory" && /* @__PURE__ */ l(gi, {
				store: e,
				views: n,
				taskId: p,
				unitId: f?.id ?? null
			}),
			a === "terminal" && /* @__PURE__ */ l(Bi, {
				taskId: p,
				workspaceId: m?.view.workspaceId ?? f?.view.workspaceId ?? "",
				views: n
			}),
			f !== void 0 && /* @__PURE__ */ l(ta, { unit: f })
		]
	});
}
//#endregion
...
function oa(e) {
	return Math.min(ia, Math.max(ra, e));
}
function sa(e) {
	return 1.25 ** (-e / 120);
}
...
function la(e, t, n, r) {
	if (e.k <= 1) return e;
	let i = (t.minX + t.maxX) / 2, a = (t.minY + t.maxY) / 2, o = e.tx + e.k * i, s = e.ty + e.k * a, c = Math.min(1, (e.k - 1) / (ia - 1)), l = n * .35 * c, u = r * .35 * c, d = Math.min(n - l, Math.max(l, o)), f = Math.min(r - u, Math.max(u, s));
	return d === o && f === s ? e : {
		k: e.k,
		tx: e.tx + (d - o),
		ty: e.ty + (f - s)
...
function ma(e, t) {
	let n = pa(t.trim()), r = /* @__PURE__ */ new Set();
	if (n.length === 0) return r;
	for (let t of e) (pa(t.id).includes(n) || pa(t.attributes.title).includes(n)) && r.add(t.id);
	return r;
}
function ha(e, t, n, r, i, a = 32) {
	let o = e.tx + e.k * t, s = e.ty + e.k * n;
	return o >= -a && o <= r + a && s >= -a && s <= i + a;
}
function ga(e, t, n, r, i, a, o = !0, s = !0) {
...
function _a({ store: e }) {
	let t = V(e, (e) => e.meta.archiveOpen), r = V(e, (e) => e.meta.archiveFocusId), s = V(e, (e) => e.board.memory), c = V(e, (e) => e.board.agents), d = V(e, (e) => e.selection.ids), f = V(e, (e) => e.meta.memoryPulses), p = V(e, (e) => e.board.heldByCard), [m, h] = o(null), [g, _] = o(null), [v, y] = o(null), [b, x] = o(aa), [S, C] = o(""), w = a(null), T = a(null), E = i(() => mi(s.records, s.silos), [s.records, s.silos]), D = i(() => {
		let e = /* @__PURE__ */ new Map();
	...
	if (n(() => {
		...
		let n = (t) => {
			t.preventDefault();
			let n = fa(e.getBoundingClientRect(), t.clientX, t.clientY, G.width, G.height), r = sa(t.deltaY);
			x((e) => {
				let t = r > 1 ? ua(n, D.map((t) => ({
					x: e.tx + e.k * t.x,
					y: e.ty + e.k * t.y
				}))) : n, i = ca(e, r, t);
				return t === n ? la(i, O, G.width, G.height) : i;
			});
		};
	...
	let k = new Map(s.records.map((e) => [e.id, e])), A = g === null ? void 0 : k.get(g), j = new Map(E.nodes.map((e) => [e.id, e.silo])), M = new Map(E.nodes.map((e) => [e.id, {
		x: e.x,
		y: e.y
	}])), N = new Map(E.nodes.map((e) => [e.id, ha(b, e.x, e.y, G.width, G.height)])), P = 1 / Math.max(1, b.k), F = ma(s.records, S), I = S.trim().length > 0, L = /* @__PURE__ */ new Set(), ee = !1;
	for (let e of d) {
		let t = c[e];
		if (t !== void 0) {
			ee = !0;
			for (let e of t.heldPieces) L.add(e);
		}
		let n = p[e];
		if (n !== void 0) {
			ee = !0;
			for (let e of n) L.add(e);
		}
	}
	let te = new Set(f.flatMap((e) => e.recordIds)), R = new Set(f.map((e) => e.silo)), ne = (e) => {
		...
		let { scale: r } = fa(n.getBoundingClientRect(), e.clientX, e.clientY, G.width, G.height), i = (e.clientX - t.lastX) / r, a = (e.clientY - t.lastY) / r;
		t.lastX = e.clientX, t.lastY = e.clientY, x((e) => da(e, i, a));
	}, re = () => {
		T.current = null;
	};
	return /* @__PURE__ */ l("div", {
			...
			children: [/* @__PURE__ */ u("header", {
				...
				children: [
					...
					/* @__PURE__ */ u("span", {
						...
						children: [/* @__PURE__ */ l("button", {
							type: "button",
							className: H("wr-mem-chip", m === null && "is-active"),
							onClick: () => h(null),
							children: "all"
						}), E.kinds.map((e) => /* @__PURE__ */ l("button", {
							type: "button",
							"data-testid": `memory-filter-${e}`,
							className: H("wr-mem-chip", m === e && "is-active"),
							style: { "--mem-hue": String(di(e)) },
				...
				children: [/* @__PURE__ */ l("aside", {
					...
					children: s.silos.map((e) => /* @__PURE__ */ u("div", {
						...
						className: H("wr-mem-silo", R.has(e.name) && "is-pulsing"),
						children: [
							/* @__PURE__ */ l("div", {
					...
					children: [/* @__PURE__ */ l("svg", {
						ref: w,
						className: H("wr-memory-graph", T.current !== null && "is-panning"),
							...
							children: [
								...
								/* @__PURE__ */ l("g", {
									...
									children: E.edges.map((e) => {
										let t = v ?? g;
										if (!ga(b.k, j.get(e.src) ?? "", j.get(e.dst) ?? "", e.src, e.dst, [v, g], N.get(e.src) ?? !0, N.get(e.dst) ?? !0)) return null;
										...
										return /* @__PURE__ */ l("path", {
											className: H("wr-mem-edge", t !== null && "is-faded"),
											d: e.d,
											fill: "none"
										}, e.key);
									})
								...
								/* @__PURE__ */ l("g", {
									...
									children: E.nodes.map((e) => {
										...
										let t = L.has(e.id), n = te.has(e.id), r = I && F.has(e.id), i = b.k >= 1 || v === e.id || g === e.id;
										return /* @__PURE__ */ u("g", {
											"data-testid": `memory-node-${li(e.id)}`,
											className: H("wr-mem-node", t && "is-held", ee && L.size > 0 && !t && "is-dimmed", n && "is-pulsing", g === e.id && "is-focused", r && "is-match", I && !r && "is-filtered"),
											transform: `translate(${e.x} ${e.y})`,
											style: { "--mem-hue": String(di(e.nodeKind)) },
											onClick: (t) => {
...
function xa(e, t) {
	return e.current.tab === t && e.current.detailId === null && e.trail.length === 0 ? e : ya(t);
}
function Sa(e, t, n) {
...
function Ca(e) {
	...
	return t === void 0 ? e.current.detailId === null ? e : ya(e.current.tab) : {
		current: t,
...
function Pa({ stackRef: e, store: t, views: n, nav: r }) {
	let i = Da(n.listStacks(), e);
	...
	let a = Ea(n.listSessions(), e), o = i.stack.spec.prompt;
	return /* @__PURE__ */ u("div", {
		...
		children: [
			/* @__PURE__ */ u("div", {
				...
				children: [
					...
					/* @__PURE__ */ l("span", {
						...
						children: /* @__PURE__ */ l(Ma, { stack: i })
					}),
					/* @__PURE__ */ l("button", {
						type: "button",
			...
			/* @__PURE__ */ l("ul", {
				...
				children: a.map((e) => /* @__PURE__ */ u("li", {
					className: H("wr-reg-subrow", `wr-reg-subrow--${e.status}`),
					role: "button",
					tabIndex: 0,
...
function Fa({ views: e, cards: t, nav: n }) {
	...
	return r.length === 0 ? /* @__PURE__ */ l("div", {
		...
		children: r.map((e) => {
			...
			return /* @__PURE__ */ u("li", {
				...
				className: H("wr-reg-row", "wr-reg-row--agent", `wr-reg-row--${e.status}`),
				role: "button",
				tabIndex: 0,
...
function Ia({ session: e, cards: t, views: n, nav: r }) {
...
function Ra({ cards: e, views: t, nav: n }) {
	let r = t.listStacks();
		...
		children: e.map((e) => /* @__PURE__ */ u("li", {
			...
			children: [
				...
				/* @__PURE__ */ l("button", {
					...
					children: Da(r, e.stackRef)?.name ?? e.stackRef
				}),
...
function za({ taskId: e, cards: t, views: n, nav: r }) {
		...
		children: [
			...
			d === null ? /* @__PURE__ */ l("div", {
				...
				children: [
					...
					/* @__PURE__ */ l("span", {
						className: "wr-reg-mono",
						children: ja(d.gitStatus.headSha)
					}),
					/* @__PURE__ */ l("span", {
						className: H("wr-reg-dirty", d.gitStatus.dirty && "is-dirty"),
		...
		children: e.listWorkspaces().map((e) => /* @__PURE__ */ u("li", {
			...
			children: [/* @__PURE__ */ u("span", {
				...
				children: [
					...
					/* @__PURE__ */ l("span", {
						className: H("wr-reg-dirty", e.dirty && "is-dirty"),
					...
					e.gitStatus !== null && e.gitStatus.branch !== "" && /* @__PURE__ */ u(c, { children: [
						...
						/* @__PURE__ */ l("span", {
							className: "wr-reg-mono",
							children: ja(e.gitStatus.headSha)
						})
					] }),
					/* @__PURE__ */ u("span", {
						className: "wr-reg-dim",
...
function Va({ workspaceId: e, cards: t, views: n, nav: r }) {
		...
		children: [
			/* @__PURE__ */ u("div", {
				...
				children: [
					...
					/* @__PURE__ */ l("span", {
						className: H("wr-reg-dirty", i.dirty && "is-dirty"),
...
function Ha({ store: e, views: t }) {
	let r = V(e, (e) => e.meta.registryOpen), i = V(e, (e) => e.meta.registryStackRef);
	V(e, (e) => e.meta.tickIndex);
	let [a, s] = o(ya());
	if (n(() => {
		r && s(i === null ? ya() : ba(i));
	...
	let c = {
		openDetail: (e, t) => s((n) => Sa(n, e, t)),
		openRun: (t) => e.getState().openRunsAt(t)
	}, d = () => s((e) => Ca(e)), f = t.listCardViews(), { tab: p, detailId: m } = a.current, h;
	return h = m === null ? p === "stacks" ? /* @__PURE__ */ l(Na, {
		views: t,
		nav: c
		...
		views: t,
		nav: c
	}) : p === "stacks" ? /* @__PURE__ */ l(Pa, {
		stackRef: m,
		store: e,
		views: t,
		nav: c
	}) : p === "agents" ? /* @__PURE__ */ l(La, {
		sessionId: m,
		cards: f,
		views: t,
		nav: c,
		onBack: d
	}) : p === "tasks" ? /* @__PURE__ */ l(za, {
		taskId: m,
		cards: f,
		views: t,
		nav: c
				...
				/* @__PURE__ */ u("header", {
					...
					children: [
						...
						/* @__PURE__ */ l("div", {
							...
							children: va.map((e) => /* @__PURE__ */ l("div", {
								role: "tab",
								tabIndex: 0,
								"data-testid": `registry-tab-${e}`,
								"aria-selected": p === e,
								className: H("wr-foundry-tab", p === e && "is-active"),
								onClick: () => s((t) => xa(t, e)),
								onKeyDown: (t) => {
									(t.key === "Enter" || t.key === " ") && s((t) => xa(t, e));
								},
								children: ka[e]
							}, e))
						}),
						wa(a) && /* @__PURE__ */ l("button", {
							type: "button",
...
function Ga({ store: e, orders: t, views: r }) {
	let i = V(e, (e) => e.meta.reviewTaskId), a = V(e, (e) => i === null ? void 0 : e.board.cards[i]);
	V(e, (e) => e.meta.tickIndex);
...
var Ka = "The cogitator refuses the amendment — a rite requires at least two phases.", qa = "The cogitator cannot inscribe an empty phase — name every step of the rite.", Ja = "Amendments bind future runs only — rites already underway keep their pinned revision.";
function Ya(e, t, n) {
...
function $a(e) {
	return e.length < 2 ? Ka : e.some((e) => e.trim().length === 0) ? qa : null;
}
//#endregion
...
function ro({ phases: e }) {
	return /* @__PURE__ */ l("span", {
		className: "wr-runs-pipeline",
		"aria-label": "phase pipeline",
		children: e.map((e) => /* @__PURE__ */ l("i", {
			className: H("wr-runs-stage", e.status === "done" && "is-done", e.status === "current" && "is-now"),
			title: e.label
		}, e.label))
	});
}
...
function ao({ run: e, title: t, simStartMs: n, onOpen: r }) {
	return /* @__PURE__ */ u("li", {
...
function oo({ run: e, title: t, views: n, simStartMs: r, onBack: i }) {
	let a = eo(e, n.getRunObservation(e.taskId)), o = {
		runId: e.runId,
		taskId: e.taskId,
...
function so({ template: e, orders: t, onClose: n }) {
				...
				children: r.map((e, t) => /* @__PURE__ */ u("li", {
					...
					children: [
						/* @__PURE__ */ l("input", {
							...
							onChange: (e) => {
								s(null), i(Ya(r, t, e.target.value));
							}
						}),
						/* @__PURE__ */ l("button", {
							type: "button",
	...
	return /* @__PURE__ */ u("div", {
		...
		children: [/* @__PURE__ */ l("ul", {
			...
			children: i.map((e) => /* @__PURE__ */ u("li", {
				className: H("wr-process-card", n === e.kind && "is-open"),
				role: "button",
				tabIndex: 0,
				onClick: () => r(e.kind),
				onKeyDown: (t) => {
...
function lo({ store: e, orders: t, views: r }) {
	let i = V(e, (e) => e.meta.runsOpen), a = V(e, (e) => e.meta.runsFocusRunId), s = V(e, (e) => e.board.cards), c = V(e, (e) => e.meta.simStartMs);
	V(e, (e) => e.meta.tickIndex);
					...
					children: [
						...
						/* @__PURE__ */ u("div", {
							...
							children: [/* @__PURE__ */ l("div", {
								...
								className: H("wr-foundry-tab", d === "runs" && "is-active"),
								onClick: () => f("runs"),
								onKeyDown: (e) => {
								...
								className: H("wr-foundry-tab", d === "processes" && "is-active"),
								onClick: () => f("processes"),
								onKeyDown: (e) => {
...
function uo({ store: e, orders: t }) {
	let n = V(e, (e) => e.meta.steerOpen), r = V(e, (e) => e.world), i = V(e, (e) => e.selection), [a, s] = o("");
	if (!n) return null;
	let { units: c } = d({
		world: r,
		selection: i
...
function fo({ store: e, orders: t, views: r }) {
	return n(() => $e({
		store: e,
		orders: t
	}), [e, t]), /* @__PURE__ */ u("div", {
...
}, mo = "1", ho = 15e3, go = 1e4, _o = 500, vo = 3e4, yo = 256, bo = class extends Error {
	status;
	constructor(e, t, n) {
...
function To(e) {
	...
	let t = e.type;
	return typeof t != "string" || !wo.has(t) ? null : e;
}
var Eo = class {
	config;
	gatewayUrl;
	...
	constructor(e, t = {}) {
		...
		this.config = e, this.gatewayUrl = e.gatewayUrl, this.token = e.token, this.restBaseUrl = Co(e.gatewayUrl), this.webSocketFactory = t.webSocketFactory ?? xo, this.fetchImpl = t.fetch ?? So(), this.pingIntervalMs = e.pingIntervalMs ?? ho, this.pongTimeoutMs = e.pongTimeoutMs ?? go, this.baseDelayMs = e.baseDelayMs ?? _o, this.maxDelayMs = e.maxDelayMs ?? vo, this.maxReconnectAttempts = e.maxReconnectAttempts;
	}
	connect() {
	...
	handleRawMessage(e) {
		...
		if (n !== null) {
			if (n.type === "hello" && this.state === "authenticating") {
				if (!n.protocolVersions.includes(mo)) {
					let e = /* @__PURE__ */ Error(`gateway does not support protocol v${mo} (got ${n.protocolVersions.join(",")})`);
	...
	send(e) {
		...
		t || (this.outboundBuffer.push(e), this.outboundBuffer.length > yo && this.outboundBuffer.shift());
	}
	trackSubscription(e) {
		switch (e.type) {
			case "subscribe": {
	...
	async getJson(e) {
		...
		if (!t.ok) {
			...
			throw new bo(`/api/v1/${e}`, t.status, n);
		}
		return await t.json();
	}
	listAgents() {
...
function Oo(e, t) {
	return e.mode === "real" ? e.gatewayUrl !== void 0 && e.token !== void 0 ? new Eo(e, t) : new Do() : new po({ seed: e.seed });
}
//#endregion
//#region src/backend/kradle/controllerClient.ts
var ko = 5e3, Ao = 3e4, jo = "X-Kradle-Request", Mo = "commander", No = "default", Po = 500, Fo = class extends Error {
	status;
	endpoint;
	bodyExcerpt;
	constructor(e, t, n) {
...
function Bo(e, t = {}) {
	let n = zo((e.kradleApiUrl ?? "").trim());
	if (!n) throw Error("KradleControllerClient: kradleApiUrl is required");
	let r = (e.kradleOrg ?? "").trim() || No, i = e.kradleToken?.trim() || void 0, a = typeof e.requestTimeoutMs == "number" && e.requestTimeoutMs > 0 ? e.requestTimeoutMs : Ao, o = t.fetch ?? Lo(), s = t.eventSourceFactory ?? Ro();
	function c() {
	...
	async function l(e, t, r) {
		...
		s ? (l["Content-Type"] = "application/json", l[jo] = Mo, r !== void 0 && (u.body = JSON.stringify(r))) : u.cache = "no-store";
		let d = new AbortController();
		u.signal = d.signal;
		let f = setTimeout(() => d.abort(), s ? ko : a), p;
		try {
			p = await o(`${n}${e}`, u);
		} finally {
			clearTimeout(f);
		...
		if (!p.ok) {
			let t = "";
			try {
				t = (await p.text()).slice(0, Po);
			} catch {
				t = "";
			}
			throw new Fo(p.status, e, t);
		}
		return await p.json();
	}
	return {
		...
		resumeRun(e, t) {
			return Promise.reject(new Io("runs/<run>/resume"));
		},
		forkRun(e, t) {
			return Promise.reject(new Io("runs/<run>/fork"));
		},
		continueRun(e, t) {
			return Promise.reject(new Io("runs/<run>/continue"));
		},
		queryMemory(e) {
...
var Vo = "kradle.a5c.ai/origin", Ho = "commander.a5c.ai/default-for", Uo = "commander.a5c.ai/coordination", Wo = 15e3, Go = {
	totalUsd: 0,
	inputTokens: 0,
	outputTokens: 0,
	thinkingTokens: 0,
...
function Qo(e) {
	return Zo(e.metadata.creationTimestamp);
}
var $o = new Set(ot), es = new Set(st);
function ts(e, t = "claude-code") {
	return e !== void 0 && $o.has(e) ? e : t;
}
function ns(e) {
	if (e !== void 0 && es.has(e)) return e;
	switch (e) {
		case "ci-repair":
...
function is(e, t) {
	let n = [];
	for (let r of e.resources ?? []) if (r.kind === t && Array.isArray(r.items)) for (let e of r.items) qo(e) && qo(e.metadata) && n.push(e);
	return n;
}
function as(e) {
	let t = X(e), n = q(t.adapter) ?? "claude-code", r = q(t.baseAgent) ?? n, i = U[ts(n)] ?? [], a = q(t.model) ?? i[0] ?? "", o = Y(t.prompt) ?? Y(t.promptTemplates), s = q(o?.system) ?? q(t.systemPrompt) ?? "", c = q(o?.developer) ?? q(t.developerPrompt), l = (e) => {
		let t = Yo(e);
...
function os(e) {
	let t = q(X(e).displayName) ?? e.metadata.name, n = Xo(e)[Vo] === "foundry";
	return {
...
function ss(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of is(e, "AgentDefinition")) t.set(n.metadata.name, os(n));
	for (let n of $(e, "stacks")) t.set(n.metadata.name, os(n));
	return [...t.values()].sort((e, t) => e.name.localeCompare(t.name));
}
function cs(e) {
	let t = $(e, "personas");
	return t.length > 0 ? t : is(e, "AgentPersona");
}
function ls(e) {
	let t = $(e, "definitions");
	return t.length > 0 ? t : is(e, "AgentDefinition");
}
function us(e) {
	let t = $(e, "processTemplates");
	return t.length > 0 ? t : is(e, "AgentProcessTemplate");
}
function ds(e) {
	let t = /* @__PURE__ */ new Map(), n = [...us(e)].sort((e, t) => e.metadata.name.localeCompare(t.metadata.name));
	for (let e of n) {
		let n = X(e), r = q(n.taskKind);
		if (r === void 0 || !es.has(r)) continue;
...
function ps(e) {
	let t = $(e, "appearances");
	return t.length > 0 ? t : is(e, "AgentAppearance");
}
function ms(e) {
	let t = $(e, "voiceProfiles");
	return t.length > 0 ? t : is(e, "AgentVoiceProfile");
}
function hs(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of cs(e)) t.set(n.metadata.name, n);
	let n = /* @__PURE__ */ new Map();
	for (let t of ps(e)) {
...
function _s(e) {
	let t = hs(e);
	return [...t.personaByName.values()].map((e) => gs(e, t)).sort((e, t) => e.displayName.localeCompare(t.displayName));
}
function vs(e) {
	let t = hs(e), n = [];
	for (let r of ls(e)) {
...
function xs(e) {
	...
	return t !== void 0 && bs.has(t) ? t : void 0;
}
function Ss(e) {
...
function Ts(e) {
	return e !== void 0 && ws.has(e);
}
function Es(e) {
	return q(X(e).agentDispatchRun) ?? q(X(e).dispatchRun);
}
function Ds(e) {
	let t = is(e, "AgentDispatchAttempt");
	return t.length > 0 ? t : [];
}
function Os(e) {
	...
	for (let [e, n] of t) {
		n.sort((e, t) => (Qo(e.item) ?? e.seq) - (Qo(t.item) ?? t.seq) || e.seq - t.seq);
		let t = n.map((e) => e.item);
		r.set(e, t);
		let a;
		for (let e = t.length - 1; e >= 0; --e) if (!Ts(Q(t[e]))) {
			a = t[e];
			break;
		}
...
function Ms(e) {
	let t = /* @__PURE__ */ new Map(), n = /* @__PURE__ */ new Map(), r = $(e, "approvals"), i = rs(e, "approvals");
	for (let e of r) {
		let n = ks(e);
		n !== void 0 && (t.get(n) ?? t.set(n, []).get(n)).push(e);
	}
	let a = i.length > 0 ? i : r.filter(js);
	for (let e of a) {
		let t = ks(e);
...
var Ns = new Set(["review", "ai-review"]), Ps = new Set([
	"write-back",
	"release",
	"tool-use"
]);
...
function Ks(e) {
	let t = $(e, "runs"), n = Ms(e), r = Ws(e), i = Rs(e), a = Os(e), o = zs(e), s = /* @__PURE__ */ new Map();
	for (let e of t) {
		let t = q(X(e).parentRunRef) ?? Xo(e)["commander.a5c.ai/parent"];
		t !== void 0 && (s.get(t) ?? s.set(t, []).get(t)).push(e.metadata.name);
	}
	let c = [], l = 0;
	for (let e of t) {
		let t = X(e), u = Xo(e), d = e.metadata.name, f = ns(q(t.taskKind)), p = Q(e), m = Z(e), h = Y(m.tokenUsage), g = (J(h?.totalTokens) ?? (J(h?.inputTokens) ?? 0) + (J(h?.outputTokens) ?? 0)) || 0, _ = Zo(m.startedAt) ?? Zo(m.queuedAt), v = Zo(m.completedAt) ?? Zo(m.failedAt), y = _ !== void 0 && v !== void 0 ? v - _ : null, b = n.pendingByRun.get(d) ?? [], x = b.some((e) => Ns.has(As(e) ?? "")), S = b.some((e) => Ps.has(As(e) ?? "")), C = (n.byRun.get(d) ?? []).some((e) => Ps.has(As(e) ?? "") && (Q(e) === "Approved" || q(Z(e).decision) === "approved")), w = q(Z(e).mergedAt) !== void 0 || u["commander.a5c.ai/merged"] === "true", T = q(Z(e).releasedAt) !== void 0 || u["commander.a5c.ai/release-id"] !== void 0, E = xs(e) ?? Cs(p, {
			taskKind: f,
			hasPendingReviewApproval: x,
			hasPendingWriteBackApproval: S,
			hasApprovedWriteBack: C,
			merged: w,
			released: T
		}), D = E === "merged" || E === "in-production" || E === "approved" ? E : x ? "ai-review" : S ? "human-review" : E, O = q(t.workspaceRef) ?? "", k = O === "" ? void 0 : r.get(O), A = k === void 0 ? 0 : Us(k).uncommittedCount ?? 0, j = q(Y(t.sourceRefs)?.pullRequest), M = q(t.repository) ?? "", N = q(t.title) ?? j ?? (M === "" ? d : `${M}:${f}`), P = Y(t.sourceEvent), F = q(t.description) ?? q(P?.name) ?? "", I = (n.byRun.get(d) ?? []).find((e) => q(Z(e).feedback) !== void 0), L = I ? q(Z(I).feedback) ?? null : null, ee = Bs(d, a, i, o).map((e) => e.metadata.name), te = (a.byRun.get(d) ?? []).length || 1, R = {
			taskId: d,
			taskKind: f,
			title: N,
			repository: M,
			...
			column: D,
			order: 0,
			yolo: Jo(Y(t.approvalPolicy)?.autoApprove) ?? u["commander.a5c.ai/yolo"] === "true",
			merged: D === "merged" || D === "in-production",
			progress: Ss(e) ?? Gs(p),
...
function Js(e) {
	return e === void 0 ? "pending" : qs[e] ?? e;
}
var Ys = {
	pending: "Pending",
	queued: "Queued",
...
function Qs(e) {
	let t = Xs(e);
	return t !== void 0 && t in Zs ? Zs[t] : "created";
}
function $s(e, t, n) {
	let r = n?.get(e), i = r === void 0 ? wt[e] : r.phases, a = Gs(t);
...
function nc(e) {
	let t = $(e, "runs"), n = ds(e);
	return t.map((t) => {
		let r = X(t), i = Z(t), a = ns(q(r.taskKind)), o = Q(t), s = Zo(i.queuedAt) ?? Qo(t) ?? 0, c = Zo(i.completedAt) ?? Zo(i.failedAt);
		return {
			...
			observedState: Qs(o),
			phases: $s(a, o, n),
			pendingEffectsByKind: ec(e, t.metadata.name),
			tokens: { ...Ko },
			costUsd: J(i.cost) ?? 0,
			startedAt: s,
			endedAt: c ?? null
...
function rc(e, t) {
	...
	let r = ns(q(X(n).taskKind)), i = Q(n), a = ds(e);
	return {
		runId: n.metadata.name,
		taskId: n.metadata.name,
		observedState: Qs(i),
		pendingEffectsByKind: ec(e, t),
		phases: $s(r, i, a),
		journal: []
	};
}
...
function ac(e) {
	return e !== void 0 && e in ic ? ic[e] : "active";
}
function oc(e) {
	let t = /* @__PURE__ */ new Map();
...
function sc(e) {
	...
	let t = X(e), n = Y(t.cost), r = Yo(t.messages) ?? [], i = J(n?.inputTokens) ?? 0, a = J(n?.outputTokens) ?? 0, o = J(n?.thinkingTokens) ?? 0, s = J(n?.cachedTokens) ?? 0, c = J(n?.totalUsd) ?? 0;
	return {
		tokenUsage: {
			inputTokens: i,
			outputTokens: a,
...
function lc(e, t, n, r) {
	...
	return ys(n, r, q(i.agentDefinition) ?? q(s.agentDefinition), q(i.personaRef) ?? q(s.personaRef));
}
function uc(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of ls(e)) t.set(n.metadata.name, n);
	return t;
}
function dc(e, t, n, r, i, a) {
	let o = X(t), s = Xo(t), c = t.metadata.name, l = q(o.dispatchRun), u = lc(e, t, i, a)?.displayName ?? s["commander.a5c.ai/creature"] ?? c, d = ts(q(o.adapter)), f = q(o.model) ?? U[d][0] ?? "", p = s["kradle.a5c.ai/agent-role"] === "reviewer" ? "reviewer" : s["kradle.a5c.ai/agent-role"] === "integration" ? "integration" : "worker", m = ac(Q(t)), h = cc(e, l), g = sc(n.bySession.get(c)), _ = q(o.title) ?? `${u} — ${p}`, v = m === "active" ? null : 0;
	return {
		sessionId: c,
		title: _,
		creatureName: u,
...
function fc(e, t) {
	let n = oc(e), r = new Map(ss(e).map((e) => [e.stackRef, e.name])), i = hs(e), a = uc(e), o = $(e, "sessions"), s = [], c = 0;
	for (let l of o) {
		if (t !== void 0 && Fs(l) !== t) {
			c += 1;
			continue;
...
function mc(e, t) {
	...
	if (n === void 0) return null;
	let r = oc(e), i = dc(e, n, r, new Map(ss(e).map((e) => [e.stackRef, e.name])), hs(e), uc(e)), a = r.bySession.get(t);
	return {
		record: i,
		transcript: (a === void 0 ? [] : Yo(X(a).messages) ?? []).map((e, t) => {
			let n = Y(e) ?? {}, r = pc[q(n.role) ?? "assistant"] ?? "message", i = {
				seq: t,
				tick: 0,
				timestamp: Zo(n.timestamp) ?? 0,
				kind: r,
				text: q(n.content) ?? ""
			}, a = q(n.toolName);
...
function hc(e, t) {
	let n = $(e, "runs").find((e) => e.metadata.name === t);
	if (n === void 0) return null;
	let r = q(X(n).workspaceRef), i = r === void 0 ? void 0 : Ws(e).get(r), a = (Ms(e).byRun.get(t) ?? []).map((e) => q(Z(e).feedback)).filter((e) => e !== void 0);
	return {
		taskId: t,
		phase: i === void 0 ? "missing" : Hs(Q(i)),
		gitStatus: i === void 0 ? {
			branch: "main",
			headSha: "",
			dirty: !1
...
function gc(e) {
	let t = Ks(e), n = $(e, "workspaces"), r = Rs(e);
	return n.map((e) => {
		let n = e.metadata.name, i = Us(e), a = Hs(Q(e)), o = t.filter((e) => e.workspaceId === n), s = o.map((e) => e.taskId), c = o.map((e) => ({
			taskId: e.taskId,
			title: e.title,
			branch: i.branch,
			headSha: i.headSha,
...
function vc(e, t, n) {
	let r = (n?.matches ?? []).map((e) => ({
		...
		tick: 0,
		unitId: t
	})), i = [], a = [...$(e, "memoryImports"), ...is(e, "AgentMemoryUpdate")];
	for (let e of a) {
		let n = X(e);
		if (q(n.sourceRun) !== t) continue;
		let r = (Yo(n.changes) ?? []).map((e) => {
			let t = Y(e) ?? {};
			return {
				path: q(t.path) ?? "",
...
function bc(e, t, n, r, i, a, o) {
	let s = X(t), c = Xo(t), l = q(s.dispatchRun), u = ts(q(s.adapter)), d = q(s.model) ?? U[u][0] ?? "", f = cc(e, l), p = c["kradle.a5c.ai/agent-role"] === "reviewer" ? "reviewer" : c["kradle.a5c.ai/agent-role"] === "integration" ? "integration" : "worker", m = sc(r.bySession.get(t.metadata.name)), h = lc(e, t, i, a);
	return {
...
function Sc(e) {
	let t = Xs(e);
	return t !== void 0 && t in xc ? xc[t] : "queued";
}
function Cc(e) {
	let t = X(e), n = Q(e), r = ns(q(t.taskKind)), i = n === "Succeeded" || n === "succeeded" ? "Ready" : n === "Failed" || n === "failed" || n === "Cancelled" || n === "cancelled" ? "Error" : "Pending";
	return {
		taskId: e.metadata.name,
		taskKind: r,
...
function wc(e, t, n) {
	let r = X(e), i = q(r.dispatchRun), a = ts(q(r.adapter)), o = q(r.model) ?? U[a][0] ?? "", s = sc(t.bySession.get(e.metadata.name));
	return {
...
function Dc(e, t) {
	let n = Ks(e), r = oc(e), i = new Map(ss(e).map((e) => [e.stackRef, e.name])), a = hs(e), o = uc(e), s = $(e, "sessions").filter(Ls), c = s.map((n) => bc(e, n, i, r, a, o, t)), l = s.map((e) => wc(e, r, t)), u = $(e, "runs").map((e) => Cc(e)), d = Tc(e, t), f = Ec(e, t), p = {};
	for (let t of n) {
...
function Nc(e) {
	...
	return Array.isArray(t) ? t : (e?.agents?.approvals?.items ?? []).filter((e) => {
		let t = Oc(e.status) ? kc(e.status.phase) : void 0;
		return t === void 0 || t === "Pending";
	});
}
function Pc(e) {
...
function Lc(e) {
	if (e !== void 0) {
		for (let t of Object.keys(Ic)) if (e === t || e.includes(t)) return t;
		return e;
	}
}
function Rc(e, t) {
	...
	let r = n.find((e) => jc(e)[Ho] === t);
	if (r !== void 0) return r.metadata.name;
	let i = ct[t], a = n.find((e) => Lc(kc(Ac(e).adapter)) === i);
	return a === void 0 ? n[0].metadata.name : a.metadata.name;
}
function zc(e, t, n) {
	let r = Fc(e);
	return n != null && n !== "" ? r.find((e) => e.metadata.name === n)?.metadata.name ?? n : r.find((e) => jc(e)["commander.a5c.ai/default-for"] === t)?.metadata.name ?? null;
}
var Bc = "default";
function Vc(e, t) {
	let n = e.spec, r = e.metadata.name, i = { ...e.metadata.labels ?? {} }, a = n.runtimeIdentity?.serviceAccountRef ?? Bc, o = {
	...
	return n.provider !== void 0 && (o.provider = n.provider), n.toolProfileRef !== void 0 && (o.toolProfileRef = n.toolProfileRef), n.externalTools !== void 0 && (o.externalTools = n.externalTools), n.skillRefs !== void 0 && (o.skillRefs = n.skillRefs), n.subagentRefs !== void 0 && (o.subagentRefs = n.subagentRefs), n.contextLabelRefs !== void 0 && (o.contextLabelRefs = n.contextLabelRefs), n.workspacePolicyRef !== void 0 && (o.workspacePolicyRef = n.workspacePolicyRef), n.runnerPool !== void 0 && (o.runnerPool = n.runnerPool), n.permissionRefs !== void 0 && (o.permissionRefs = n.permissionRefs), n.memoryRepositoryRefs !== void 0 && (o.memoryRepositoryRefs = n.memoryRepositoryRefs), n.displayName !== void 0 && (o.displayName = n.displayName), n.agentRole !== void 0 && (o.agentRole = n.agentRole), n.resources !== void 0 && (o.resources = n.resources), {
		apiVersion: e.apiVersion ?? "kradle.a5c.ai/v1alpha1",
		kind: "AgentStack",
		metadata: {
			name: r,
...
function qc(e, t) {
	...
	return e.displayName !== void 0 && e.displayName !== "" && (n.displayName = e.displayName), {
		...
		metadata: { name: e.name ?? Kc(e.taskKind) },
		spec: n
	};
}
...
function Yc(e, t) {
	return Nc(e).some((e) => e.metadata.name === t);
}
function Xc(e, t) {
...
function Zc(e, t) {
	...
	return {
		abort(t) {
			let r = n();
			for (let n of t) Xc(r, n) ? l("abort/cancelRun", () => e.cancelRun(n)) : i ? i.abort([n]) : c("abort");
		},
		steer(e, t) {
		...
		upsertDefinition(t) {
			...
			}]), l("upsertDefinition/applyDefinition", () => Uc(e, {
				name: t.name,
				spec: {
					personaRef: t.personaRef,
					stackRef: t.stackRef,
		...
		createAgentIdentity(t) {
			...
			return l("createAgentIdentity/applyAgentIdentity", () => Gc(e, t)), t.name;
		},
		createRosterAgent() {
		...
		updateProcessTemplate(t, n) {
			...
			return r.length === 0 ? null : (l("updateProcessTemplate/applyProcessTemplate", () => Jc(e, {
				taskKind: t,
				phases: r
			})), 1);
		},
...
var el = 5e3, tl = 500, nl = class {
	...
	views() {
		return {
			getWorkspaceView: (e) => this.snapshot ? hc(this.snapshot, e) : null,
			getRunObservation: (e) => this.snapshot ? rc(this.snapshot, e) : null,
			listStacks: () => this.snapshot ? ss(this.snapshot) : [],
			listRosterAgents: () => [],
			listPersonas: () => this.snapshot ? _s(this.snapshot) : [],
			listDefinitions: () => this.snapshot ? vs(this.snapshot) : [],
			listRuns: () => this.snapshot ? nc(this.snapshot) : [],
			listProcessTemplates: () => this.snapshot ? yc(this.snapshot) : [],
			getMemoryIO: (e) => {
				...
				return t === void 0 && this.fetchMemory(e), vc(this.snapshot, e, t);
			},
			getWorkspaceTree: () => null,
			getFileContent: () => null,
			getGitLog: () => [],
			listSessions: (e) => this.snapshot ? fc(this.snapshot, e) : [],
			getSession: (e) => this.snapshot ? mc(this.snapshot, e) : null,
			listCardViews: () => this.snapshot ? Dc(this.snapshot, this.deps.now()).cards : [],
			listWorkspaces: () => this.snapshot ? gc(this.snapshot) : []
		};
	}
	start() {
		this.refresh(), this.pollHandle = this.deps.setTimer(() => this.tickPoll(), el), this.streamUnsub = this.deps.client.openEventStream(() => this.scheduleRefresh());
	}
	scheduleRefresh() {
	...
	tickPoll() {
		this.disposed || (this.refresh(), this.pollHandle = this.deps.setTimer(() => this.tickPoll(), el));
	}
	refresh() {
...
function rl(e, t, n, r = {}) {
	...
	let m = r.createClient === void 0 ? Bo({
		...
		setTimer: l,
		clearTimer: u
	}), g = n.gatewayUrl !== void 0 && n.gatewayUrl !== "", _ = Zc(m, {
...
function al(e) {
	let t = il(e), n = Oo(t), r = Qn(), i, a, o = !1;
	if (n instanceof po) i = $n(r, n), a = n.sim, e.mock === !0 && (window.__commander = {
		store: r,
		version: Fn
	}, o = !0);
	else {
...
//#endregion
export { ol as CommanderRoot, ol as default };
