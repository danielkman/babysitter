#!/usr/bin/env node
// Generate all atlas target plugins from the unified atlas source.
// Dedicated pipeline — separate from the babysitter generator (scripts/generate-plugins.mjs).
// Usage: node scripts/generate-atlas-plugins.mjs [--output <dir>] [--marketplace <path>] [--compare <dir>]
...
const UNIFIED_SOURCE = join(ROOT, 'plugins/atlas-unified');
const DEFAULT_OUTPUT = join(ROOT, 'artifacts', 'generated-atlas-plugins');
const COMPILER_PKG = join(ROOT, 'packages/adapters/extensions');
...
function resolveUnifiedPluginVersion() {
  ...
  throw new Error('Unable to resolve a unified atlas plugin version.');
}
...
function syncUnifiedPluginVersion() {
  ...
  console.log(`[generate-atlas] Synced plugins/atlas-unified/plugin.json version -> ${nextVersion}`);
  return nextVersion;
}
...
if (!existsSync(distCli)) {
  console.log('[generate-atlas] Building adapters-extensions compiler...');
...
console.log(`[generate-atlas] Source: ${relative(ROOT, UNIFIED_SOURCE)}`);
console.log(`[generate-atlas] Output: ${relative(ROOT, outputDir) || '.'}`);
console.log(`[generate-atlas] Unified atlas plugin version: ${unifiedVersion}`);

if (!compareOnly) {
  resetOutputDir(outputDir);

  // Compile each target declared in the atlas plugin.json `targets` map, emitting
  // one output subdir named by the logical target key (e.g. `gemini`, `genty`).
  // We drive per-target compilation rather than `--target all` so the output
  // directory names match the plugin.json target keys exactly (the compiler's
  // catalog-driven `all` would emit `gemini-cli`/`hermes` and skip `genty`).
  const atlasManifest = JSON.parse(readFileSync(join(UNIFIED_SOURCE, 'plugin.json'), 'utf8'));
  const targets = Object.keys(atlasManifest.targets || {});
  if (targets.length === 0) {
    console.error('[generate-atlas] plugin.json declares no targets');
    process.exit(1);
  }

  let failed = 0;
  for (const target of targets) {
    const targetOutput = join(outputDir, target);
    const cliArgs = [
      'compile',
      '--target', target,
    ...
    try {
      ...
      console.error(`[generate-atlas] Compilation failed for target: ${target}`);
      failed += 1;
    }
  }

  if (failed > 0) {
    console.error(`[generate-atlas] ${failed} target(s) failed to compile`);
    process.exit(1);
  }

  console.log('[generate-atlas] Compilation complete.');
