#!/usr/bin/env node
/**
 * Unified Context Hook for Pi
 * Routes through hooks-proxy for all hook execution.
 *
 * Fires when Pi requests context injection before the agent processes
 * a turn. Delegates to `babysitter hook:run --hook-type user-prompt-submit`
 * via hooks-proxy.
 *
 * This hook injects babysitter orchestration context (active run state,
 * pending effects, iteration instructions) into the agent's context window.
 *
...
const LOG_FILE = path.join(LOG_DIR, "babysitter-pi-context-hook.log");
...
function main() {
  ...
  const handler = `babysitter hook:run --harness unified --hook-type user-prompt-submit --state-dir ${STATE_DIR} --json`;
