...
adapters emits a `cost` event whenever an adapter reports usage. You can
...
adapters sessions list --agent claude-code --json \
  | jq -r '.[].sessionId' \
  | while read sid; do
      adapters sessions read --agent claude-code --id "$sid" --json \
