...
### 7.1 adapters Own Paths

| Path Purpose | Resolution | Override |
|---|---|---|
| Global config dir | `os.homedir()/.adapters/` | `createClient({ configDir })` or `AGENT_MUX_CONFIG_DIR` env var |
| Project config dir | `<projectRoot>/.adapters/` | `createClient({ projectConfigDir })` or `--project-dir` CLI flag |
| Run temp dir | `os.tmpdir()/adapters-<runId>/` | Not overridable |
...
All paths exposed through adapters API surfaces are normalized to **forward slashes** regardless of platform:
...
```typescript
/**
 * Remove all orphaned adapters temp directories.
 * 
 * Scans os.tmpdir() for directories matching 'adapters-*' that have
...
| Session files | Read-only by adapters | No locking (agent-owned) |
...
2. **Not filtering parent env:** adapters does not strip or filter inherited variables, as agents may legitimately need access to `PATH`, `HOME`, `LANG`, `TERM`, and other system variables.
...
- adapters does not filter the parent environment because agents may legitimately need system variables.
...
- Config file corruption is possible if external tools write to agent config files while adapters holds a lock.
