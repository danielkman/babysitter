...
| CLI `adapters run` | `10-cli-reference.md` | 1 |
...
interface RunHandle extends AsyncIterable<AgentEvent> {
   ...
   * and the entry written to `.adapters/run-index.jsonl`.
   */
...
| `running` | `killed` | Process receives external signal (not from adapters) |
...
`node-pty` is a native module with platform-specific compilation requirements. It is an optional peer dependency of `@a5c-ai/comm-adapter`:
...
- Temp dirs created under `$TMPDIR/adapters-<runId>/` (macOS) or `/tmp/adapters-<runId>/` (Linux).
- Config paths follow XDG conventions on Linux (`$XDG_CONFIG_HOME` or `~/.config/`). macOS uses `~/Library/Application Support/` for agents that follow macOS conventions, but adapters's own config always uses `~/.adapters/`.
...
- Temp dirs created under `%TEMP%\adapters-<runId>\`.
