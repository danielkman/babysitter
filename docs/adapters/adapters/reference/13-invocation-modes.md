...
export interface SshInvocation {
  ...
  readonly autoInstall?: boolean;    // default true (used by `adapters remote`)
}
...
## 6. `adapters remote` bootstrap

`adapters remote install|update <host>` ( `packages/cli/src/commands/remote.ts`) composes a four-step self-install pipeline through `buildInvocationCommand()`:

1. **Probe** — `adapters --version` on the target. Skipped steps if already installed (unless `--force` or subcommand is `update`).
2. **Install adapters** — `npm install -g @a5c-ai/adapters-cli` (or `npm update` for the update verb).
3. **Install harness** — `adapters install <harness> [--force]` (or `adapters update <harness>`).
4. **Verify** — `adapters detect --all --json`.
