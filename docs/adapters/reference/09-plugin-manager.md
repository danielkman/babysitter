...
6. **No shadowing.** adapters does not maintain its own plugin registry or database. It reads the agent's native plugin state on every call. There is no cached or shadow plugin list.
...
**list:** Reads `~/.claude/settings.json` to enumerate configured MCP servers. Scans directories specified in prior `--add-dir` invocations (tracked via adapters's run index). Each MCP server entry becomes an `InstalledPlugin` with `format: 'mcp-server'`. Skill directories produce entries with `format: 'skill-directory'`.
...
- **Used by:** OpenCode (`opencode-*` prefix), Pi (`@mariozechner/pi-*`), omp (`@oh-my-pi/*`), OpenClaw (any npm package with adapters plugin metadata).
...
The CLI exposes plugin operations under the `adapters plugins` namespace. All commands respect capability gating and produce the same errors as the SDK methods.

### 15.1 `adapters plugins list`
...
### 15.3 `adapters plugins uninstall`
...
```
$ adapters plugins browse opencode --category coding --sort downloads
...
```
$ adapters plugins info @openclaw/browser-skill --agent openclaw
