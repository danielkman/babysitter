...
### Working Directory
```bash
adapters run amp --cwd /path/to/project --prompt "Analyze code complexity"
```
...
```bash
adapters run amp --model oracle --prompt "Complex architectural decision"
```

### Librarian
Code search and analysis specialist powered by Sourcegraph's code graph:
```bash
adapters run amp --prompt "Find all implementations of the authentication interface"
```
...
```bash
# List sessions
adapters sessions amp

# Read session
adapters sessions amp read <session-id>

# Resume session
adapters run amp --session-id <session-id>

# Fork session
adapters sessions amp fork <session-id> --name "experimental-branch"
```
...
adapters plugins list amp

# Install MCP server
adapters plugins install amp <mcp-server>

# List installed plugins
adapters plugins list amp --installed
```
...
adapters run amp --model oracle --prompt "
...
adapters sessions amp read <session-id> --validate
```
...
1. **Use Oracle routing** for complex, multi-step tasks:
   ```bash
   adapters run amp --model oracle --prompt "Full-stack feature implementation"
   ```

2. **Leverage Librarian** for code discovery:
   ```bash
   adapters run amp --prompt "Find all error handling patterns in the codebase"
   ```
...
4. **Use auto-approve carefully** - only in trusted environments:
   ```bash
   adapters run amp --tool-approval-mode yolo --prompt "Format and lint all files"
   ```

5. **Monitor costs** with cost tracking:
   ```bash
   adapters run amp --prompt "Complex refactoring task" | adapters cost
   ```
