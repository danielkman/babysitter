...
This specification defines the adapter system: the primary extension and abstraction layer that allows adapters to drive any CLI-based AI coding agent through a uniform interface. The adapter system consists of three core pieces:
 ...
 * everything adapters needs to know about a specific CLI-based agent.
 *
...
interface AgentAdapter {
  ...
  /**
   * Structured manifest of what this agent can do. Used by adapters
...
interface AdapterRegistry {
   ...
   * Used by `adapters install <agent>` and by consumers to guide users
...
6. Write the run record to `.adapters/run-index.jsonl`.
...
- Package name: `adapters-adapter-<agent-name>` (e.g., `adapters-adapter-aider`).
- Default export: an `AgentAdapter` instance or a factory function `() => AgentAdapter`.
- Peer dependency: `@a5c-ai/comm-adapter` (to get type definitions).

```typescript
// adapters-adapter-aider/src/index.ts
import { BaseAgentAdapter } from '@a5c-ai/comm-adapter';
...
If a significant fraction of lines are returning `null` (the adapter cannot parse most output), this indicates an adapter bug or an agent version mismatch. adapters does not detect this automatically -- adapter authors should ensure comprehensive parsing for all known output formats of their agent.
...
`AgentAdapter` exposes three optional methods consumed by `adapters install|update|detect`:
...
`@a5c-ai/adapters-codecs` now ships 11 adapters. The 11th is `adapters-remote`; see `docs/12-built-in-adapters.md`.
