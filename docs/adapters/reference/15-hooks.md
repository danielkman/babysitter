...
`adapters` provides a harness-agnostic hook system that lets one
...
- **Hook type** — a harness-specific event name (e.g. `PreToolUse`,
  ...
  `@a5c-ai/comm-adapter` for the per-harness list.
- **Registration** — a record in `.adapters/hooks.json` (project) or
  `~/.adapters/hooks.json` (global) that maps
...
```
adapters hooks <agent> discover
adapters hooks <agent> list
adapters hooks <agent> add <hookType> [--handler builtin|command|script]
                                  [--target <id-or-cmd>]
                                  [--id <id>] [--priority N] [--global]
adapters hooks <agent> remove <id> [--global|--project]
adapters hooks <agent> set <id> [--priority N] [--enabled true|false] [--target ...]
adapters hooks <agent> handle <hookType>   # reads JSON payload on stdin
```
...
stdin and `adapters` dispatches all matching registrations.

## Built-in programmatic hooks

| ID                       | Description                                             |
|--------------------------|---------------------------------------------------------|
| `log`                    | Append payload to `~/.adapters/hook-log.jsonl`              |
