...
**Specification v1.0** | `@a5c-ai/comm-adapter`
...
|---|---|
| npm scope | `@a5c-ai` |
| Core runtime package | `@a5c-ai/comm-adapter` |
...
|---|---|---|
| `@a5c-ai/comm-adapter` | `AgentMuxClient`, normalized events, type definitions, provider/hook/workspace helpers, and stream/runtime primitives | `@a5c-ai/atlas`, `@a5c-ai/adapters-observability` |
| `@a5c-ai/adapters-codecs` | All built-in adapter implementations (claude, codex, gemini, copilot, cursor, opencode, pi, omp, openclaw, hermes) | `@a5c-ai/comm-adapter` |
| `@a5c-ai/adapters-cli` | CLI binary (`adapters`) | `@a5c-ai/comm-adapter`, `@a5c-ai/adapters-codecs` |
...
import { classifyTool } from '@a5c-ai/comm-adapter/browser';
...
(`await import('@a5c-ai/comm-adapter')`) rather than assuming a CJS shim.
...
An **Agent** is a local CLI-based AI coding agent that can be spawned as a subprocess. Agents are external programs installed on the user's system. adapters does not embed, bundle, or proxy any agent -- it drives them through their CLI interface.

Each agent is identified by a unique `AgentName` string. adapters provides adapters for ten built-in agents (see Section 1.4). Additional agents can be registered via the plugin adapter contract.

### 2.2 Adapter

An **Adapter** is adapters's internal representation of a specific agent. Each adapter encapsulates complete knowledge of a single agent:
...
When a `RunOptions` field requires a capability that the agent does not have, adapters throws a `CapabilityError` before spawning the subprocess.
...
- **Global profiles**: `~/.adapters/profiles/<name>.json` -- shared across all projects.
- **Project profiles**: `.adapters/profiles/<name>.json` -- project-specific.
...
adapters provides a unified `PluginManager` interface that delegates to each agent's native plugin system. Calling any `PluginManager` method on an agent where `supportsPlugins` is `false` throws a `CapabilityError`.
...
adapters defines a hierarchy of typed error classes. All errors extend the base `AgentMuxError`.
...
adapters uses two directory trees. **Project-level configuration overrides global configuration** via deep merge at the field level.

### 4.1 Global Directory: `~/.adapters/`
...
|---|---|---|
| macOS | `$HOME` (typically `/Users/<user>`) | `/Users/<user>/.adapters/` |
| Linux | `$HOME` (typically `/home/<user>`) | `/home/<user>/.adapters/` |
| Windows | `%USERPROFILE%` (typically `C:\Users\<user>`) | `C:\Users\<user>\.adapters\` |
...
### 4.2 Project Directory: `.adapters/`

Located in the project root (the working directory or the nearest ancestor containing a `.adapters/` directory).
...
4. If not found, use `path.join(process.cwd(), '.adapters')` as the default (created lazily on first write).
...
The only persistent data adapters writes. An append-only JSONL (JSON Lines) file where each line is a self-contained JSON object recording one run.
...
For reference, the per-agent native file locations. These are the files adapters reads through the `SessionManager` and `ConfigManager`.
...
5. **Built-in defaults** -- hardcoded in adapters (e.g., `approvalMode: 'prompt'`, `stream: 'auto'`).
...
interface AgentMuxClient {
  ...
  /**
   * Config manager -- read/write agent-native and adapters config files.
   */
...
adapters normalizes all file paths using `path.resolve()` and `path.join()` from the Node.js `path` module, which automatically handles platform-specific separators (`/` on POSIX, `\` on Windows). All paths stored in configuration files and `run-index.jsonl` use the OS-native separator.
...
- **XDG compliance**: Some agents store config under XDG directories (e.g., Gemini CLI at `~/.config/gemini/`, OpenCode at `~/.config/opencode/`). adapters reads from the actual paths used by each agent, respecting `XDG_CONFIG_HOME` and `XDG_DATA_HOME` when the agent does.
...
adapters does **not** store, manage, or transmit credentials. Authentication is handled entirely by each agent's native mechanisms (API keys in environment variables, OAuth tokens managed by the agent's own CLI, etc.). adapters only observes the result of authentication checks through each adapter's `detectAuth()` method.
...
When spawning agent subprocesses, adapters passes through the current `process.env` merged with any `env` overrides from `RunOptions`. Consumers must take care not to pass sensitive values in `RunOptions.env` that should not be visible to the agent subprocess. adapters does not filter or sanitize environment variables.

### 7.5 Config File Integrity

adapters does not sign or checksum its configuration files. If a config file is tampered with, adapters will use the tampered values. Consumers in security-sensitive environments should use file system permissions and integrity monitoring tools to protect the `.adapters/` and `~/.adapters/` directories.
...
The adapters package exports an 11th built-in adapter, `adapters-remote`, for nested adapters execution over any invocation mode. See `docs/12-built-in-adapters.md`.
