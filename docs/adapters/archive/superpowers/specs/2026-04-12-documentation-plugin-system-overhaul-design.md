...
This design addresses multiple documentation quality issues and implements a proper separation between plugin management and MCP server management in the adapters CLI system.

## Problems Being Solved

1. **Outdated Installation Instructions**: Agent docs show direct npm installation instead of `adapters install`
...
5. **Command Structure Confusion**: `adapters plugins` actually manages MCPs, not true plugins
...
- **`adapters mcp`** - Manage Model Context Protocol servers per agent
- **`adapters plugin`** - Manage full plugins through agent-native systems

**MCP Commands (Rename existing `adapters plugins`):**

```bash
# Global MCP management
adapters mcp list <agent>                    # List installed MCPs for agent
adapters mcp install <agent> <mcp-server>    # Install MCP server for agent
adapters mcp uninstall <agent> <mcp-server>  # Remove MCP server for agent

# Project-level MCP management (new)
adapters mcp list <agent> --project         # List project-specific MCPs
adapters mcp install <agent> <mcp-server> --project  # Install to project scope
```
...
```bash
adapters plugin list <agent>                 # Calls: claude plugins list, gemini extensions list, etc.
adapters plugin install <agent> <plugin>     # Calls: claude plugins install <plugin>, etc.
adapters plugin enable <agent> <plugin>      # Calls: claude plugins enable <plugin>, etc.  
adapters plugin disable <agent> <plugin>     # Calls: claude plugins disable <plugin>, etc.
adapters plugin marketplace <agent> [cmd]    # Calls: claude plugins marketplace [cmd], etc.
```
...
**Error Handling:**
For agents without native plugin systems, return clear error: "Plugin management not supported for `{agent}`. Use 'adapters mcp' for MCP servers."
...
adapters plugin marketplace <agent>

# For MCP servers (all agents):
adapters mcp install <agent> <mcp-server>
...
- Document `adapters mcp` command API with all configuration nuances
- Document `adapters plugin` command delegation patterns per agent
...
**Unit Tests:**
- Test `adapters mcp` command variations (global/project scopes, configurations)
- Test `adapters plugin` delegation to each agent's native commands
...
- Test that all installation examples use `adapters install` format
...
- **No backward compatibility** - existing `adapters plugins` commands will be replaced entirely
