...
Survey of comparable open-source projects and parity / gap analysis vs adapters.
...
paperclip's 7 adapters: claude-local, codex-local, cursor-local, gemini-local, opencode-local, openclaw-gateway, pi-local. **We match** all 7 by name and add: hermes, omp, copilot, adapters-remote (11 total). **We are a superset.**
...
- No reference we saw exposes a CLI surface as broad as ours (`adapters` has run, sessions, hooks, plugins, detect, doctor, config).

## Conclusion

adapters is a **structural superset** of every reference project surveyed. The actionable gaps are narrow and mostly in the **claude streaming flags** and **cost attribution granularity**. Filed as followups in the issue tracker.
...
| Cost Tracking | [Tokscale](https://github.com/junhoyeo/tokscale) | TS | Multi-harness CLI | Similar scope to adapters |
...
- **Pure Open Source**: TokenBudget, LiteLLM, adapters
- **Open Source + Enterprise**: Langfuse, Arize Phoenix  
- **SaaS with markup**: OpenRouter (5% markup)
- **Subscription SaaS**: Portkey ($49/month+)

**Insight**: The pure open-source positioning of adapters aligns with developer-focused tools rather than enterprise platforms.
