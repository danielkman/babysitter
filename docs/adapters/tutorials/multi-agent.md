...
adapters lets you fan out a prompt to multiple adapters and merge the result stream. This is useful for voting, consensus, or comparison.
...
```bash
adapters run claude,gemini,opencode --prompt "Explain the PR" --json | \
  jq -s 'group_by(.agent)'
```
