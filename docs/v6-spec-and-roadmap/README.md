...
- `packages/babysitter` and `packages/genty/platform` provide the CLI and agent runtime.
...
The currently selected first executable V6 slice is the `@a5c-ai/genty-platform` seam contract. It keeps the existing package boundary intact while making runtime-domain ownership, subpath exports, and validation gates explicit.
