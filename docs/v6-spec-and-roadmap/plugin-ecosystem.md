...
- on legacy non-Babysitter agents, `@a5c-ai/extensions-adapter` is the compiler and distribution path for the concrete plugin outputs those metaplugins need,
...
- compiler validation and per-target emission through `@a5c-ai/extensions-adapter`,
...
`@a5c-ai/extensions-adapter` is the current compiler for harness-specific plugin outputs. The public surface is:
