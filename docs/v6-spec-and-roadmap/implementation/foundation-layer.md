...
- Clarify which runtime concerns already belong to `packages/genty/platform`
...
- Validate `extensions-adapter` and related packaging flows against real plugin manifests and generated outputs
...
- ADR-backed first executable slice for the accepted `@a5c-ai/genty-platform` seam contract
...
This phase does not create `@a5c-ai/genty-runtime` or `@a5c-ai/genty-platform`. Those names remain deferred vocabulary unless later decision records promote a specific slice into scope.
