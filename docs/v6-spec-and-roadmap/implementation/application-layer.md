...
- Clarify whether governance work belongs in existing plugin bundles, SDK modules, or `packages/genty/platform` seams → [Security Architecture](../security-architecture.md)
...
- Keep `packages/genty/platform` as the current orchestration package unless a later ADR approves a narrow rename or extraction
...
This phase does not create a replacement `@a5c-ai/genty-platform`, does not assume an `agent-platform` package underneath it, and does not require all major functionality to be converted into plugins. Those remain deferred until separately promoted by decision record.
