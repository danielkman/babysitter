...
- Clarify session ownership across `packages/genty/platform`, `packages/babysitter-sdk`, and plugin integration points
...
If a future standalone package is ever pursued here, it should host proved composition logic rather than replace concrete plugin packaging. The current repository evidence still treats unified plugins and per-harness bundles as the install surfaces, with `extensions-adapter` compiling those outputs.
...
**Deferred Vocabulary Guardrail**: `@a5c-ai/genty-platform`, `@a5c-ai/genty-platform-meta-plugins`, and `@a5c-ai/genty-platform-orchestration-plugin` remain non-deliverables in this phase
