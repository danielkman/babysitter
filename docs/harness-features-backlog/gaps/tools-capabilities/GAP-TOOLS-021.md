...
## Current State
Runs are user-initiated only. The hook system (`packages/babysitter-sdk/src/hooks/`) fires hooks during run lifecycle but does not receive external events. No webhook endpoint, filesystem watcher, or event listener exists. The MCP server could theoretically receive external calls but has no event-to-run mapping.
