...
## Recommendation
Phase 3 implementation. Define typed interaction schemas in `packages/babysitter-sdk/src/interaction/types.ts`. Extend ProcessContext with `ctx.ask()` intrinsic. Implement interaction renderers in each harness adapter. Ensure journal tracking for replay.
