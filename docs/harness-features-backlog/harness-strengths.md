...
**What**: Every state change in a run is recorded as an immutable journal event. The replay engine (`packages/babysitter-sdk/src/runtime/replay/createReplayEngine.ts`) can replay any run from its journal, producing identical results.
...
**Key components**:
- Journal storage: `packages/babysitter-sdk/src/storage/` (appendEvent, loadJournal)
...
**Supported adapters** (`packages/babysitter-sdk/src/harness/`):
...
**Why it matters**: CC has built-in agents but no reusable, versioned process definitions. The harness's process library (`packages/babysitter-sdk/src/processLibrary/`) enables:
...
**Why it matters**: CC plugins have no migration path between versions. The harness's migration system (`packages/babysitter-sdk/src/plugins/migrations.ts`) can:
...
**Why it matters**: CC's permission model is per-tool-type. The harness's pattern language (`packages/babysitter-sdk/src/breakpoints/patterns.ts`) can express fine-grained policies like:
...
**Why it matters**: CC has hooks but they are tightly coupled to the CC runtime. The harness's hook system (`packages/babysitter-sdk/src/hooks/`) is harness-agnostic:
...
The 16 agentic tool definitions (`packages/babysitter-sdk/src/harness/agenticTools.ts`) are injectable into any Pi session:
...
`piSecureSandbox` (`packages/babysitter-sdk/src/harness/piSecureSandbox.ts`) provides Docker-based isolation for Pi bash execution, sandboxing command execution in containers. CC's sandbox is less granular.
