...
| Contract tests | Vitest + fixtures | Stable boundaries between SDK, hooks-adapter, adapters, transport-adapter, agent-core, and agent-platform, including transport-adapter route matrix and runtime env injection contracts | Every PR and push |
...
| Browser/UI E2E | Playwright + mock gateway | Adapters WebUI session flows, transcript rendering, model picker behavior, approvals, reconnect behavior | PRs touching WebUI/gateway/session code; staging before publish |
| CLI smoke tests | Node subprocess tests | `babysitter`, `adapters`, hooks-adapter CLI, package entrypoints, help output, dry-run paths | Every PR for touched packages; staging before publish |
...
| Hook mode | `none`, `hooks-adapter` | `hooks-adapter` lanes register an `adapters hooks` command bridge and assert the normalized hooks-adapter phase evidence |

Every no-model stack lane starts a local transport-adapter runtime with a fixture completion engine. The launched agent, including the local CI shim for real-agent lanes and the mock-harness path for `adapters-mocks`, must send a request through that transport-adapter runtime and attach the request count plus redacted evidence under `publish-no-model-stack-*`.

`Publish` also has an `agent_mux_hooks_mux_e2e` no-model/no-SDK job. It is intentionally separate from the live Babysitter plugin matrix: the GitHub matrix chooses `claude-code`, `codex`, and `pi`; the test only consumes that one selected lane, registers an `adapters hooks` command hook, bridges the native payload into `a5c-hooks-adapter invoke`, and asserts the hooks-adapter normalized phase evidence.
...
| Adapters plugin/session E2E | Provider secrets, installed external CLI, and plugin precondition where supported | `adapters run` or `createClient().run` starts a session, plugin command creates a Babysitter run, and hooks/process lifecycle are asserted | Scheduled, manual, staging gate |
| Babysitter-agent live orchestration | Preinstalled/mocked backend plus `OPENAI_API_KEY`, configured Foundry/OpenAI credentials, or configured cloud equivalents where needed | `@a5c-ai/genty-platform` can plan, execute, post task results, and close a run without executing harness installer commands | `publish.yml` staging/main preflight, manual |
| Adapters live adapters | Provider-specific credentials | Claude Code and Codex adapters produce protocol events that match the adapter contracts | Scheduled/manual first, then `publish.yml` release preflight after quarantine |
| Transport-adapter live transport | Local process ports plus provider/harness credentials | Transport-adapter carries real agent-core streams and adapters-launched external harness traffic through proxy routes with redacted launch/env/metrics artifacts | Scheduled/manual first, then `publish.yml` after quarantine |
...
## Transport-Adapter Lane Split

Transport-adapter scenarios must be split across both lanes instead of hidden inside broad adapter smoke tests.
...
| Adapters launch proxy decision | No-model | `resolveLaunchPlan` can prove proxy forced/if-needed/native/forbidden behavior with fixture provider configs |
| Agent-core through transport-adapter | Both | Fixture stream belongs in no-model; live provider stream belongs in model-backed when credentials exist |
...
| Model-backed runtime smoke | Runtime maintainer | Harness and adapter maintainers | Immediate for staging/release failures |
