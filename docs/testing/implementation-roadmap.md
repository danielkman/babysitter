...
| 1. Mock and fixture contracts | Next no-model implementation slice | Runtime maintainers | No-model | Mock Codex, Claude Code, agent-core, transport-adapter, and gateway transcripts are shared by unit, integration, and UI tests | PR/push CI |
...
| 3. Adapter integration coverage | Partly covered by current adapter validation; transport-adapter fixtures remain the largest gap | Adapters maintainers | No-model | Transport-adapter route/runtime/env/launch-plan coverage, adapters gateway, adapters, and WebUI run against compatible fixture sessions | PR/push CI and `publish.yml` validation |
...
| 5. Split live E2E smokes | Partly implemented; deeper path-specific jobs remain | Runtime and adapter maintainers | Model-backed | Adapters plugin/session smoke uses pipeline-selected plugin preconditions; transport-adapter bridge smoke proves agent-core and external-harness proxy paths; agent-platform runtime smoke uses runtime path with no installer steps | `publish.yml` staging/release preflight |
...
| --- | --- | --- | --- |
| `babysitter-plugin` | Generate plugin packages, install the target agent with `adapters install`, install the local Babysitter SDK, then install the Babysitter plugin for the target harness | `adapters launch <agent> <provider>` with a `/babysitter:call ...` prompt | Adapters IDs, Babysitter run/effect IDs, native hook evidence, hooks-adapter event evidence, transport trace, redacted provider trace |
| `vanilla` | Install the target agent with `adapters install` only | `adapters launch <agent> <provider>` with a normal non-Babysitter prompt | Adapters IDs, session ID, transport trace, redacted provider trace |

Both modes use adapters for external agent installation and launch. The test code reads the selected mode from workflow env and must not enumerate scenario matrices internally.
...
2. Add no-model transport-adapter local route/codec, env-injection, passthrough, metrics/cache, and launch-plan proxy-decision scenarios.
3. Add shared fixture transcript format and migrate one adapters or transport-adapter test to consume it.
4. Add no-model harness dry-run tests for Codex and Claude Code.
5. Add transport-adapter fixture and agent-core event stream replay scenarios.
6. Add credential-gated transport-adapter bridge smoke for agent-core and one external harness through `adapters launch --with-proxy*`.
