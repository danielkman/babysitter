...
| 3. Mux integration coverage | Partly covered by current mux validation; transport-mux fixtures remain the largest gap | Agent-mux maintainers | No-model | Transport-mux route/runtime/env/launch-plan coverage, adapters gateway, adapters, and WebUI run against compatible fixture sessions | PR/push CI and `publish.yml` validation |
...
3. Add shared fixture transcript format and migrate one adapters or transport-mux test to consume it.
