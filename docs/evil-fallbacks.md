...
**Journal append queue swallows previous errors** — `packages/sdk/src/storage/journal.ts:19-27`
`.catch(() => undefined)` on the previous operation means ALL prior journal write failures are silently swallowed. The journal — the system's audit trail and replay foundation — can silently lose events.

**Atomic write silently downgrades to non-atomic** — `packages/sdk/src/storage/atomic.ts:30-47`
On `ENOENT`, the atomic rename falls back to `fs.writeFile` (non-atomic). If the process crashes mid-write, journal events can be corrupted.

### High

**Cost computation silent failure** — `packages/sdk/src/runtime/orchestrateIteration.ts:135-136`
Three operations in one `try/catch { /* optional */ }`. Billing data silently disappears.
...
**Policy decision reporting — best effort** — `packages/sdk/src/runtime/intrinsics/task.ts:132-133`
Compliance decisions fail to record with no indication. Audit trail has gaps.

### Medium

**Lock acquisition retries 41 times silently** — `packages/sdk/src/storage/lock.ts:68-79`
40 attempts × 250ms = 10+ seconds silent waiting. No per-attempt logging.
...
**Require cache clear failures ignored** — `packages/sdk/src/runtime/orchestrateIteration.ts:200`
Stale module code may be reused on the next `require()`.

**Breakpoint auto-approval — non-critical** — `packages/sdk/src/runtime/intrinsics/task.ts:141-154`
Auto-approval fails silently → breakpoint becomes manual without explanation.

**Process library resolution — fall through** — `packages/sdk/src/processLibrary/active.ts:196-203`
Binding errors swallowed, falls to default. User doesn't know which library is active.

**Git marketplace — nested fallback** — `packages/sdk/src/plugins/marketplace.ts:134-139`
Stash errors assumed "nothing to stash." Force-checkout overwrites without understanding the failure.
...
**Endpoint resolution fallback chain** — `packages/agent-core/src/session.ts:38-89`
9 env vars checked in priority order. Model defaults to "gpt-4o" silently. Anthropic path silently converts GPT model names to Claude. Empty `apiKey` strings returned on some paths. Final fallback sends `amuxApiKey` to OpenAI endpoint — could be a Foundry token.

### High

**Tool error wrapping loses everything** — `packages/agent-core/src/agenticTools/shared/results.ts:37-58`
All tool execution errors (network, permission, timeout, bugs) converted to generic `errorResult`. Stack traces lost, error categories lost.

**Subagent invocation — message only** — `packages/agent-core/src/subagent/invoker.ts:65-76,116-118`
All subagent errors become `{ success: false, error: "message string" }`. No stack, no category, no context. Multi-agent debugging impossible.

### Medium

**AST-grep → ripgrep silent fallback** — `packages/agent-core/src/agenticTools/tools/code.ts:100-130`
If `sg` fails, silently falls back to `rg`. User never knows they got text search instead of AST search. Pattern might be valid AST but broken as regex.

**Mermaid rendering — "mmdc not available"** — `packages/agent-core/src/agenticTools/tools/code.ts:216-239`
mmdc failure reason lost. Diagram syntax errors indistinguishable from "mmdc not installed." Temp files left on disk.

**Puppeteer import — misleading message** — `packages/agent-core/src/agenticTools/browser/tool.ts:42-49`
Any import error produces "puppeteer is not installed" — could be module init error, version mismatch, etc.
...
**Directory walk silent failure** — `packages/agent-core/src/agenticTools/shared/paths.ts:61-65`
`readdirSync` error → returns silently. Incomplete file listings without warning.

**Spawn error — message only** — `packages/agent-core/src/agenticTools/shared/process.ts:94-99`
ENOENT, EACCES, ETIMEDOUT all become generic `stderr: error.message`. No error type distinction.
...
**Process module load retry — 3 silent attempts** — `packages/agent-platform/src/harness/internal/createRun/orchestration/effects.ts:632-678`
Module load failures retried 3 times with delays. Only verbose logging. If all fail, no retry context in error.
...
**Process definition extraction — wrong code blocks** — `packages/agent-platform/src/harness/internal/createRun/planProcess/recovery.ts:33-106`
If no valid process code block found, returns ANY code block as fallback. Could extract shell scripts or config as a "process definition."

**Journal scan silent stop** — `packages/agent-platform/src/storage/journalWatcher.ts:83-139`
readdir error → silent return. Corrupt journal files skipped. Without `onError` handler, failures invisible.

**MCP reconnection — error status not exception** — `packages/agent-platform/src/mcp/client/manager.ts:242-271`
Connection failures become `status: "error"` on the record. Callers checking status get falsy state; no exception thrown.
...
**Prompt retry — no history** — `packages/agent-platform/src/harness/internal/createRun/agent-core-loop.ts:118-164`
Retried up to 2 times (1s, 3s delays). Returns final result without indicating retries occurred.

**Effect execution retry — count swallowed** — `packages/agent-platform/src/harness/internal/createRun/orchestration/effects.ts:521-630`
Up to 3 retries with exponential backoff. "Failed after 3 retries" indistinguishable from "failed immediately."
...
**MCP config file — corrupt = empty** — `packages/agent-platform/src/mcp/client/config.ts:33-52`
Both ENOENT and JSON parse errors return empty server list. Misconfiguration invisible.
...
**MCP tool execution — exception → result object** — `packages/agent-platform/src/mcp/client/executor.ts:38-59`
Tool exceptions caught and converted to `success: false`. Stack traces lost. Callers may not check success flag.
...
**Provider resolution — 5-level model fallback** — `packages/agent-mux/core/src/provider-resolver.ts:94-96`
`input.model ?? process.env.AMUX_MODEL ?? profile?.model ?? fileDefaults?.model ?? defaults.defaultModel`. No logging of which source was chosen.
...
**Google → Vertex silent upgrade** — `packages/agent-mux/core/src/provider-resolver.ts:124-126`
If `GOOGLE_GENAI_USE_VERTEXAI=true` or project is set, silently switches from Google to Vertex. User may not realize which backend is active.
...
**Missing model config → foundry-gpt55** — `.github/workflows/live-stack.yml:231`
`models[entry.model] || models['foundry-gpt55']`. Typos absorbed silently.

**Unknown agent → synthetic config** — `.github/workflows/live-stack.yml:235-236`
`agents[entry.agent] || {agent: entry.agent, amux_agent: entry.agent}`. Unrecognized agents create invented test scenarios.

**Unknown mode → non-interactive** — `.github/workflows/live-stack.yml:251`
`modes[e.mode] || modes['ni']`. Interactive tests silently run non-interactively.

**Unknown install → vanilla** — `.github/workflows/live-stack.yml:252`
`installModes[e.install] || installModes['vanilla']`. Plugin mode silently becomes vanilla.

**Hooks "pending" cast to "passed"** — `packages/agent-mux/cli/tests/live-stack/primary-live-runner.ts:889-909`
Missing hooks evidence marked `'pending' as 'passed'` then retroactively upgraded to passed if any run completed.
...
**3-event journal = "completed"** — `packages/agent-mux/cli/tests/live-stack/primary-live-runner.ts:919-930`
`MIN_JOURNAL_EVENTS = 3`. RUN_CREATED + PROCESS_ASSIGNED + ITERATION_STARTED satisfies this. Run that never finished tasks passes.
...
**Graph catalog resolution silently falls back to hardcoded metadata** — `packages/sdk/src/harness/amuxFallbackMetadata.ts:179-220`
...
**Harness discovery falls back to legacy probing silently** — `packages/sdk/src/harness/discovery.ts:144-151`
...
**Tool call arguments JSON parse → empty object** — `packages/transport-mux/src/server.ts:1304`
...
**Hook bash execution → `{}\n`** — `packages/extension-mux/src/transformHelpers.ts:110,123-125`
Shell script execution catch returns `{}\n`. Caller thinks hook succeeded with empty result. Completely masks bash failures.

**Gemini CLI spawn errors → "not found"** — `packages/extension-mux/src/transformHelpers.ts:272-275`
Any spawn error (permissions, system issues) treated same as "CLI not found." Wrong fallback message.

**Extension uninstall — double empty catch with destructive fallback** — `packages/extension-mux/src/transformHelpers.ts:290,295`
Uninstall failures fall through to `fs.rmSync({ recursive: true, force: true })` without verification.

**Breakpoint answer poller — fabricated "expired" result** — `packages/tasks-mux/src/client/answer-poller.ts:220`
Any error fetching final breakpoint state → returns hardcoded minimal "expired" result. Misleads consumers.

**Response body nested catch** — `packages/tasks-mux/src/backends/server.ts:244`
If response isn't JSON AND reading text fails, body becomes `undefined`. Error reporting becomes lossy.

### High

**Hook execution best-effort** — `packages/extension-mux/src/transformHelpers.ts:247-254`
`try { execFileSync("bash", [...]); } catch { /* best-effort */ }`. Hook failures completely invisible.

**File overwrite on read error** — `packages/extension-mux/src/installSharedGenerator.ts:77-84`
Read errors (permissions, corrupt file) treated as "doesn't exist." May overwrite with wrong content.

**chmod failure ignored** — `packages/extension-mux/src/installSharedGenerator.ts:131-133`, `packages/cloud/src/sdk/providers.ts:128-130`
File permission changes fail silently. Scripts won't be executable; provider secrets may be world-readable.

**Cloud agent spawn — JSON/raw conflation** — `packages/cloud/src/sdk/agents.ts:96-110`
JSON parse failure converts structured output into "raw output" mode. Caller can't distinguish.

**Kubernetes resource parse → empty array** — `packages/cloud/src/sdk/deploy.ts:103-107`
...
**Config write serialization — previous error swallowed** — `packages/agent-mux/core/src/config-manager.ts:161`
`prev.catch(() => undefined).then(fn)`. Previous write error swallowed, next write proceeds. Race conditions possible.

### High

**PTY/subprocess kill failures — 8+ locations** — `packages/agent-mux/core/src/spawn-runner.ts:163`, `packages/agent-mux/launch/src/launch.ts` (multiple)
...
**Connection/server cleanup — resource leaks** — `packages/agent-mux/adapters/src/remote-adapter-base.ts:136,141`
`connection.close().catch(() => {})`, `stopServer?.().catch(() => {})`. Connections and servers leak.

**Ephemeral config cleanup** — `packages/agent-mux/adapters/src/claude-code/runtime-hooks/ephemeral-config.ts:112,117,130`
Three `fs.rm().catch(() => {})` calls. Filesystem leaks accumulate.

**Hook event chain failure** — `packages/agent-mux/core/src/spawn-runtime-hooks.ts:145`
`await eventChain.catch(() => {})`. Hook dispatch failures silently ignored.

**Adapter installation detection → null** — `packages/agent-mux/core/src/adapter-registry.ts:160`, `packages/agent-mux/core/src/spawn-runner-utils.ts:66`
...
**Gateway JSON parse → empty object** — `packages/agent-mux/gateway/src/kanban/routes.ts:1494`
`.catch(() => ({}))`. API response parse failure → empty object.

### Medium

**Auth detection → 'unknown'** — `packages/agent-mux/core/src/adapter-registry.ts:155-157`
Auth detection failure caught, marked 'unknown' instead of error. Silent degradation.
