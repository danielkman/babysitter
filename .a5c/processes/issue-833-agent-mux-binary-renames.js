  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Resolve duplicate agent-mux binary ownership: keep @a5c-ai/adapters as canonical owner and keep @a5c-ai/adapters-cli on legacy amux alias only if needed to avoid publishing the same canonical bin twice.',
...
const readArtifactsTask = defineTask('issue-833.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'git status --short',
      'git diff --stat',
      'git diff -- package.json scripts/check-binary-renames.cjs packages/adapters/cli packages/adapters/extensions packages/adapters/harness-mock packages/adapters/hooks/cli packages/adapters/sdk packages/adapters/tasks packages/adapters/transport packages/adapters/triggers packages/adapters/tui docs .github scripts tsconfig.json',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 120000,
  },
