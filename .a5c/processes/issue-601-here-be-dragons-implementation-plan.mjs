...
export const readIssueAndRepoContextTask = defineTask('issue-601.read-issue-and-repo-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'printf "%s\\n" "--- focused files ---"',
      'for path in packages/agent-core/src/agenticTools/index.ts packages/genty/platform/src/harness/piWrapper.ts packages/agent-core/src/agenticTools/tools/execution.ts packages/genty/platform/src/harness/agenticTools/tools/execution.ts packages/adapters/ui/src/screens/SessionDetailScreen.test.tsx tsconfig.json .a5c/processes/issue-601-remaining-dragons-plan.mjs; do if [ -f "$path" ]; then printf "\\n### %s\\n" "$path"; nl -ba "$path" | sed -n "1,220p"; fi; done',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 300000,
  },
