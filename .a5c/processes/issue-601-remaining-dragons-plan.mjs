...
const readArtifactsTask = defineTask('issue-601.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short --branch',
      'git diff -- docs/here-be-dragons.md tsconfig.json packages/agent-core packages/tula/platform packages/agent-mux .a5c/processes/issue-601-remaining-dragons-plan.mjs .a5c/processes/issue-601-remaining-dragons-plan.inputs.json',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 120000,
  },
