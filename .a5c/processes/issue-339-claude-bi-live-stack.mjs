...
const readContextTask = defineTask('issue-339.read-runtime-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'sed -n "206,260p" packages/adapters/launch/src/launch.ts',
      'sed -n "1080,1325p" packages/adapters/launch/src/launch.ts',
      'printf "\\n--- bridge hooks path ---\\n"',
      'sed -n "1,260p" packages/adapters/launch/src/bridge-hooks.ts',
      ...
      'sed -n "1,240p" packages/adapters/cli/tests/live-stack/primary-live-runner.test.ts',
      'sed -n "1,220p" packages/adapters/cli/tests/launch-bridge-interactive.test.ts',
      'sed -n "1,260p" packages/adapters/cli/tests/launch-runtime-integration.test.ts',
      'printf "\\n--- recent launch history ---\\n"',
      'git log --oneline -20 -- packages/adapters/launch packages/adapters/cli/tests packages/agent-catalog packages/sdk/src/harness .github/workflows/live-stack.yml',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 180000,
  },
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        '---',
        'Do not edit files.',
        'Trace the live execution path from Live Stack matrix generation to adapters launch, bridge-interactive PTY spawn, child_process fallback, prompt delivery, transport-mux proxy startup, hook emulation, and artifact/report validation.',
...
const redGateTask = defineTask('issue-339.red-test-gate', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'rg -n "issue #339|claude.*BI|bridged.*fallback|child_process.*fallback|prompt.*args|delayed stdin|Windows" packages/adapters/cli/tests packages/adapters/launch -g "*.ts"',
      'set +e',
      'npm exec --yes --package=vitest -- vitest run --config vitest.config.ts packages/adapters/cli/tests/launch-bridge-interactive.test.ts packages/adapters/cli/tests/launch-runtime-integration.test.ts packages/adapters/cli/tests/live-stack/primary-live-runner.test.ts',
      'status=$?',
      'set -e',
      'if [ "$status" -eq 0 ]; then',
...
const verifyFixTask = defineTask('issue-339.verify-fix', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'npm exec --yes --package=vitest -- vitest run --config vitest.config.ts packages/adapters/cli/tests/launch-bridge-interactive.test.ts packages/adapters/cli/tests/launch-runtime-integration.test.ts packages/adapters/cli/tests/live-stack/primary-live-runner.test.ts packages/agent-catalog/src/catalog.test.ts packages/sdk/src/harness/amuxFallbackMetadata.contract.test.ts',
...
const readArtifactsTask = defineTask('issue-339.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short',
      'git diff -- packages/adapters/launch packages/adapters/cli/tests packages/agent-catalog packages/sdk/src/harness .github/workflows/live-stack.yml',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 60000,
  },
