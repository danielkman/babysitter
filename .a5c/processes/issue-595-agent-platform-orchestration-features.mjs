...
const readContextTask = defineTask('issue-595.read-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'sed -n "1,180p" packages/genty/platform/src/harness/types.ts',
      'printf "\\n--- agent-platform orchestration index ---\\n"',
      'sed -n "1,240p" packages/genty/platform/src/harness/internal/createRun/orchestration/index.ts',
      'printf "\\n--- agent-platform orchestration effects ---\\n"',
      'sed -n "1,460p" packages/genty/platform/src/harness/internal/createRun/orchestration/effects.ts',
      'printf "\\n--- agent-platform internal phase ---\\n"',
      'sed -n "1,520p" packages/genty/platform/src/harness/internal/createRun/orchestration/internalPhase.ts',
      ...
      'printf "\\n--- existing tests ---\\n"',
      'rg -n "parallelGroupId|background|subprocess|processCodeHash|schemaVersion|checkpoint|rollback|dependsOn|maxConcurrentRuns" packages/sdk packages/genty/platform packages/agent-runtime -g "*.test.ts" -g "*.ts" | head -600',
      'printf "\\n--- package scripts ---\\n"',
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        '---',
        'Edit the repository directly, scoped to S../platform runtime, storage, and tests named by the contract.',
...
const readArtifactsTask = defineTask('issue-595.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'printf "\\n--- diff ---\\n"',
      'git diff -- packages/sdk packages/genty/platform packages/agent-runtime docs .a5c/processes',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 120000,
  },
...
const publishTask = defineTask('issue-595.publish', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'git status --short',
      'CHANGED="$(git diff --name-only -- packages/sdk packages/genty/platform packages/agent-runtime docs .a5c/processes)"',
