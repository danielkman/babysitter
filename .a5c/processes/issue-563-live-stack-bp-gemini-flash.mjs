...
const readContextTask = defineTask('issue-563.read-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'rg -n "buildPrompt|buildPrimaryLiveStackCommands|validateAgentBehavior|file-creation|processMode|gemini-cli|pi|babysitter-plugin|odyssey-live-test|summarize-translate-test|a5c-live-test|resolveLaunchMaxTurns" packages/adapters/cli/tests/live-stack packages/adapters/adapters packages/transport-mux plugins .github -g "*.ts" -g "*.mjs" -g "*.js" -g "*.yml" -g "*.md"',
      'printf "\\n--- live-stack fixture excerpts ---\\n"',
      'sed -n "1,360p" packages/adapters/cli/tests/live-stack/fixtures/summarize-translate-test.mjs',
      'sed -n "1,260p" packages/adapters/cli/tests/live-stack/fixtures/create-process-skeleton.mjs',
      'printf "\\n--- live-stack runner excerpts ---\\n"',
      'sed -n "1,280p" packages/adapters/cli/tests/live-stack/primary-live-runner.ts',
      'sed -n "490,560p" packages/adapters/cli/tests/live-stack/primary-live-runner.ts',
      'sed -n "740,870p" packages/adapters/cli/tests/live-stack/primary-live-runner.ts',
      'printf "\\n--- live-stack test excerpts ---\\n"',
      'sed -n "100,290p" packages/adapters/cli/tests/live-stack/primary-live-runner.test.ts',
      'sed -n "720,830p" packages/adapters/cli/tests/live-stack/primary-live-runner.test.ts',
      'printf "\\n--- recent git context ---\\n"',
      'git log --oneline -30 -- packages/adapters/cli/tests/live-stack packages/adapters/adapters packages/transport-mux plugins .github',
      'printf "\\n--- status ---\\n"',
...
const readArtifactsTask = defineTask('issue-563.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short --branch',
      'git diff -- packages/adapters/cli/tests/live-stack .a5c/processes/issue-563-live-stack-bp-gemini-flash.mjs',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 60000,
  },
