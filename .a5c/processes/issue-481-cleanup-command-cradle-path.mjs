...
const readIssueAndTraceTask = defineTask('issue-481.read-issue-and-trace', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'rg -n "cleanup-runs|skills[/\\\\\\\\]babysit[/\\\\\\\\]process[/\\\\\\\\]cradle|process-library:active|cradle/cleanup-runs" blueprints/babysitter-unified packages/sdk/src/prompts scripts package.json library -S',
      'printf "\\n--- cleanup command source ---\\n"',
      'sed -n "1,180p" blueprints/babysitter-unified/commands/cleanup.md',
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        '---',
        'Do not edit files.',
        'Trace how blueprints/babysitter-unified/commands/cleanup.md is propagated into packages/sdk/src/prompts/templates/commands/cleanup.md.',
...
const verifyTask = defineTask('issue-481.verify', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'git diff --check',
      'if rg -n "skills[/\\\\\\\\]babysit[/\\\\\\\\]process[/\\\\\\\\]cradle|cleanup-runs.js/processes/cleanup-runs.js" blueprints/babysitter-unified packages/sdk/src/prompts scripts; then',
      '  echo "obsolete cleanup cradle path still present" >&2',
      '  exit 1',
      'fi',
      'rg -n "process-library:active --json|cradle/cleanup-runs.js#process" blueprints/babysitter-unified/commands/cleanup.md packages/sdk/src/prompts/templates/commands/cleanup.md',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 900000,
  },
...
const readArtifactsTask = defineTask('issue-481.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short --branch',
      'git diff -- blueprints/babysitter-unified/commands/cleanup.md packages/sdk/src/prompts/templates/commands/cleanup.md scripts/sync-sdk-command-templates.cjs .a5c/processes/issue-481-cleanup-command-cradle-path.mjs .a5c/processes/issue-481-cleanup-command-cradle-path.inputs.json',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 60000,
  },
...
const deliverTask = defineTask('issue-481.deliver', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short --branch',
      'git add blueprints/babysitter-unified/commands/cleanup.md packages/sdk/src/prompts/templates/commands/cleanup.md scripts/sync-sdk-command-templates.cjs',
