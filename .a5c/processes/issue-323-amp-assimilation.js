...
const publishTask = defineTask('issue-323.publish', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      `PR_URL="$(gh pr list --head ${args.branchName} --json url --jq '.[0].url // empty')"`,
      `if [ -z "$PR_URL" ]; then PR_URL="$(gh pr create --base staging --head ${args.branchName} --title "Fix Amp CLI package metadata" --body "Closes #${args.issueNumber}" --json url --jq .url)"; fi`,
