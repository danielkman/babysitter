...
const readIssueContextTask = defineTask('issue-323.read-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'printf "\\n--- stale refs ---\\n"',
      'rg -n "@sourcegraph/amp-cli|@ampcode/cli|npm install -g .*amp" docs/agent-mux/reference/agents/amp.md packages/adapters/adapters/src/amp-adapter.ts packages/adapters/adapters/tests/amp-adapter.test.ts packages/atlas/graph/agent-stack/versions/amp-current.yaml packages/atlas/graph/extensions/plugin-artifacts/plugin-target-amp.yaml || true',
      'printf "\\n--- npm target package ---\\n"',
...
const verifyAmpAssimilationTask = defineTask('issue-323.verify-assimilation', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'if rg -n "@sourcegraph/amp-cli" packages/adapters/adapters/src/amp-adapter.ts packages/adapters/adapters/tests/amp-adapter.test.ts docs/agent-mux/reference/agents/amp.md; then exit 1; fi',
      'rg -n "@ampcode/cli" packages/adapters/adapters/src/amp-adapter.ts packages/adapters/adapters/tests/amp-adapter.test.ts docs/agent-mux/reference/agents/amp.md packages/atlas/graph/agent-stack/versions/amp-current.yaml',
...
const readArtifactsTask = defineTask('issue-323.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short',
      'git diff -- packages/adapters/adapters/src/amp-adapter.ts packages/adapters/adapters/tests/amp-adapter.test.ts docs/agent-mux/reference/agents/amp.md packages/atlas/graph/agent-stack/versions/amp-current.yaml',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 30000,
  },
...
const publishTask = defineTask('issue-323.publish', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      'set -euo pipefail',
      'git add packages/adapters/adapters/src/amp-adapter.ts packages/adapters/adapters/tests/amp-adapter.test.ts docs/agent-mux/reference/agents/amp.md',
