 ...
 * @description Implement Jitsi Helm subchart integration for the Kradle chart.
...
const readContextTask = defineTask('issue-623.read-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'cat packages/kradle/docs/jitsi/02-helm-deployment.md',
      'printf "\\n--- Jitsi architecture spec ---\\n"',
      'cat packages/kradle/docs/jitsi/01-architecture.md',
      'printf "\\n--- Jitsi CRD/controller spec if present ---\\n"',
      'test ! -f packages/kradle/docs/jitsi/03-crds-and-controllers.md || cat packages/kradle/docs/jitsi/03-crds-and-controllers.md',
      'printf "\\n--- chart surfaces ---\\n"',
      'sed -n "1,220p" packages/kradle/charts/Chart.yaml',
      'sed -n "1,360p" packages/kradle/charts/values.yaml',
      'sed -n "1,220p" packages/kradle/charts/templates/networkpolicy.yaml',
      'find packages/kradle/charts/templates -maxdepth 1 -type f -print | sort',
      'find packages/kradle/charts/crds -maxdepth 1 -type f -print | sort',
      'printf "\\n--- existing Jitsi chart/core references ---\\n"',
      'rg -n "jitsi|Jitsi|JVB|10000|jwt|webhookSecret|JitsiMeetProvider|JitsiMeetingTemplate|JitsiMeeting|JitsiRecording" packages/kradle/charts packages/kradle/core/tests packages/kradle/core/scripts packages/kradle/docs/jitsi -g "*.yaml" -g "*.yml" -g "*.js" -g "*.mjs" -g "*.md" | head -600',
      'printf "\\n--- package commands ---\\n"',
...
const implementTask = defineTask('issue-623.implement', (args, taskCtx) => ({
  ...
  labels: ['issue-623', 'kradle', 'jitsi', 'helm', 'implementation'],
  agent: {
    name: 'kradle-helm-implementer',
    prompt: {
      ...
      instructions: [
        ...
        'Keep changes scoped to packages/kradle chart/deployment artifacts and tests needed to verify them.',
...
const verifyTask = defineTask('issue-623.verify', (args, taskCtx) => ({
  ...
  labels: ['issue-623', 'kradle', 'jitsi', 'helm', 'verification'],
  shell: {
    command: [
      'set -euo pipefail',
      'git diff --check -- packages/kradle/charts packages/kradle/core',
      '(cd packages/kradle/core && node scripts/validate-package.mjs)',
      'node --test packages/kradle/core/tests/jitsi-helm-integration.test.js',
      'if command -v helm >/dev/null 2>&1; then helm repo add kubevela https://kubevela.github.io/charts >/dev/null 2>&1 || true; helm repo add kyverno https://kyverno.github.io/kyverno/ >/dev/null 2>&1 || true; helm repo add jitsi-contrib https://jitsi-contrib.github.io/jitsi-helm/ >/dev/null 2>&1 || true; helm dependency build packages/kradle/charts && helm lint packages/kradle/charts --set jitsi.install=true --set jitsi.prosody.auth.jwt.appSecret=test-secret --set jitsi-subchart.prosody.secretEnvs.JWT_APP_SECRET=test-secret --set jitsi.kradle.webhookSecret=test-webhook && helm template kradle packages/kradle/charts --set jitsi.install=true --set jitsi.prosody.auth.jwt.appSecret=test-secret --set jitsi-subchart.prosody.secretEnvs.JWT_APP_SECRET=test-secret --set jitsi.kradle.webhookSecret=test-webhook >/tmp/kradle-jitsi-install.yaml && grep -q "JWT_APP_SECRET: dGVzdC1zZWNyZXQ=" /tmp/kradle-jitsi-install.yaml && helm template kradle packages/kradle/charts --set jitsi.install=false --set jitsi.external.enabled=true --set jitsi.external.url=https://meet.example.com --set jitsi.external.jwtAppSecret=test-secret >/tmp/kradle-jitsi-external.yaml; else echo "helm not installed; package and node guardrail tests completed"; fi',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 900000,
  },
...
const readArtifactsTask = defineTask('issue-623.read-artifacts', (args, taskCtx) => ({
  ...
  labels: ['issue-623', 'kradle', 'jitsi', 'helm', 'artifacts'],
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short',
      'git diff -- packages/kradle/charts packages/kradle/core .a5c/processes/issue-623-jitsi-helm-integration.js .a5c/processes/issue-623-jitsi-helm-integration.inputs.json',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 60000,
  },
...
const reviewTask = defineTask('issue-623.review', (args, taskCtx) => ({
  ...
  labels: ['issue-623', 'kradle', 'jitsi', 'helm', 'review'],
  agent: {
    name: 'kradle-helm-reviewer',
    prompt: {
...
const publishTask = defineTask('issue-623.publish', (args, taskCtx) => ({
  ...
  labels: ['issue-623', 'kradle', 'jitsi', 'helm', 'publish'],
  shell: {
    command: [
      'set -euo pipefail',
      'git status --short',
      'git add -f .a5c/processes/issue-623-jitsi-helm-integration.js .a5c/processes/issue-623-jitsi-helm-integration.inputs.json',
      'git add packages/kradle/charts/Chart.yaml packages/kradle/charts/values.yaml packages/kradle/charts/templates/networkpolicy.yaml packages/kradle/charts/templates/jitsi-secrets.yaml packages/kradle/charts/crds/external-resources.yaml packages/kradle/charts/crds/agent-resources.yaml packages/kradle/charts/crds/aggregated-resources.yaml packages/kradle/charts/crds/jitsi-resources.yaml packages/kradle/core/scripts/validate-package.mjs packages/kradle/core/tests/jitsi-helm-integration.test.js',
      'git diff --cached --name-only',
      'if ! git diff --cached --quiet; then GIT_AUTHOR_NAME="a5c automation" GIT_AUTHOR_EMAIL="actions@users.noreply.github.com" GIT_COMMITTER_NAME="a5c automation" GIT_COMMITTER_EMAIL="actions@users.noreply.github.com" git commit -m "feat(kradle): add Jitsi Helm chart integration"; fi',
      ...
      `if [ -z "$PR_URL" ]; then PR_URL="$(gh pr create --base ${args.baseBranch} --head ${args.branchName} --title "Add Kradle Jitsi Helm integration" --body "Closes #${args.issueNumber}\\n\\nImplements the Jitsi Helm subchart integration with internal/external values, Jitsi chart CRDs, JWT/webhook secret handling, JVB UDP media policy, and package guardrail tests.")"; fi`,
      `COMMENT_BODY="$(mktemp)"`,
      `cat > "$COMMENT_BODY" <<'COMMENT'`,
      `Implemented issue #623 with source changes under packages/.`,
      ``,
      `Summary: added the jitsi-meet Helm dependency, expanded Kradle Jitsi values for in-cluster and external modes, added chart-level Jitsi CRDs, JWT/webhook secret templates, JVB UDP 10000 network-policy handling, and package guardrail tests.`,
      ``,
      `Verification: cd packages/kradle/core && node scripts/validate-package.mjs; node --test packages/kradle/core/tests/jitsi-helm-integration.test.js; helm dependency build/lint/template for install and external modes; npm run verify:metadata from a clean committed worktree.`,
      `COMMENT`,
      `printf '\\nPR: %s\\n' "$PR_URL" >> "$COMMENT_BODY"`,
