...
const readSpecTask = defineTask('issue-618.read-spec', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'sed -n "1,170p" docs/adapters-babysitter-integrations/plugin-mode.md',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 180000,
  },
...
const traceRuntimeTask = defineTask('issue-618.trace-runtime', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'rg -n "subprocessSupport|runSubprocessIntrinsic|ctx\\.subprocess|kind: \\"subprocess\\"|onCleanup|cleanupCallbacks|flushProcessCleanup" packages/sdk packages/genty/platform docs -g "*.ts" -g "*.md"',
      ...
      'sed -n "632,646p" packages/genty/platform/src/harness/internal/createRun/orchestration/effects.ts',
      'sed -n "140,153p" packages/genty/platform/src/api/runs.ts',
...
const readArtifactsTask = defineTask('issue-618.read-artifacts', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'printf "\\n--- changed files ---\\n"',
      'git diff --name-only -- packages/sdk packages/genty/platform docs plugins .a5c',
      'printf "\\n--- implementation diff ---\\n"',
      'git diff -- packages/sdk/src/runtime packages/sdk/src/harness packages/sdk/src/testing packages/genty/platform docs/adapters-babysitter-integrations/plugin-mode.md',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 180000,
  },
