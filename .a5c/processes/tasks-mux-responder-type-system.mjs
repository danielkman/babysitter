...
const reuseAuditTask = defineTask('issue-630/reuse-audit', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'rg -n "ResponderType|responderType|TaskRouter|routeTask|ResponderProfile|ResponderMatcher|BreakpointRouter|adapter|model|trackerBackend|capabilities" packages/tasks-mux docs/adapters-babysitter-integrations -g "*.ts" -g "*.md" || true',
...
const readSpecTask = defineTask('issue-630/read-spec', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'printf "%s\\n" "ISSUE #630 JSON:"',
      'cat "$tmp"',
      'printf "\\n%s\\n" "DESIGN DOC: docs/adapters-babysitter-integrations/tasks-mux-routing.md"',
      'cat docs/adapters-babysitter-integrations/tasks-mux-routing.md',
      'printf "\\n%s\\n" "OVERVIEW DOC: docs/adapters-babysitter-integrations/overview.md"',
      'cat docs/adapters-babysitter-integrations/overview.md',
      'rm -f "$tmp"',
    ].join('\n'),
    cwd: args.projectRoot,
    expectedExitCode: 0,
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Keep adapters and tracker backend execution out of scope unless the spec explicitly requires it for this issue; this issue should unblock those later integrations through types and routing contracts.',
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'Do not implement adapters execution backends, tracker API clients, SDK task intrinsic changes, or agent-platform effect resolution unless the spec-verbatim block below proves they are required for issue #630.',
