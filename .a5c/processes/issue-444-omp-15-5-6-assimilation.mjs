...
const readContextTask = defineTask('issue-444.read-context', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'rg -n "agentVersion:omp:ge-15-5-6|omp-15-5-6|PI_CODEX_WEBSOCKET_MAX_IDLE_REUSE_MS|@oh-my-pi/pi-coding-agent|URL and directory selectors|stale response frames" packages/atlas/graph packages/agent-catalog packages/adapters/adapters docs/adapters/reference -g "*.yaml" -g "*.ts" -g "*.md" | head -800',
    ].join('\n'),
    expectedExitCode: 0,
    timeout: 120000,
  },
