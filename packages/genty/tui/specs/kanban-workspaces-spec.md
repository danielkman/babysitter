...
This document defines a brownfield feature slice for adding `kanban` and `workspaces` views, navigation, and management actions to [`packages/adapters/tui`](../README.md).
...
- TUI view and command-surface additions in `packages/adapters/tui`
- reuse of shared kanban types from `packages/adapters/core/src/kanban.ts`
...
- shared kanban and workspace-facing types in `packages/adapters/core/src/kanban.ts`
...
- view registration: `packages/adapters/tui/src/plugin.ts`
- plugin registry/bootstrap: `packages/adapters/tui/src/registry.ts`
...
- issue and board state: `packages/adapters/core/src/kanban.ts`
- repository lifecycle and review state: `packages/adapters/core/src/kanban.ts`
- workspace links and permissions: `packages/adapters/core/src/kanban.ts`
...
The first implementation phase should add a package-local integration layer inside `packages/adapters/tui` with responsibilities such as:
...
- `packages/adapters/core/src/kanban.ts` for stable shared types
- `packages/kanban/src/mcp/tools/backlog.ts`
- `packages/kanban/src/mcp/tools/workspaces.ts`

If gaps in the shared type contract are discovered, the work should extend `packages/adapters/core` first instead of leaking `packages/kanban` private service shapes into the TUI.
...
- `packages/adapters/tui/src/app.tsx`
- `packages/adapters/tui/src/plugin.ts`
- `packages/adapters/tui/src/command-palette.tsx`
- new plugin files under `packages/adapters/tui/src/plugins/`
- new tests under `packages/adapters/tui/tests/`
...
- `packages/adapters/core/tests/kanban.test.ts`
...
- `@a5c-ai/genty-tui` remains part of the `build:adapters` and `test:adapters` surface
...
Because this spec is intentionally part of the package planning surface, `packages/adapters/tui/package.json` should ship `specs/` alongside the README.
...
- Ink remains the React-based TUI layer used by `packages/adapters/tui`, so new views should follow the existing plugin/view registration model.
...
- Node TTY resize behavior remains relevant to the existing viewport/layout code in `packages/adapters/tui/src/app.tsx`, so any dense board/workspace rendering needs to respect the current viewport logic instead of bypassing it.
