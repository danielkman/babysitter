...
const binaryPath = path.resolve(__dirname, '../dist/bin/adapters-tui.js');
const fixturePluginPath = path.resolve(__dirname, 'fixtures/adapters-tui-e2e-plugin.mjs');
...
describeBuiltBinary('real adapters-tui binary e2e', () => {
  ...
  it('loads external plugins and drives session detail, resume, and prompt dispatch through the real binary', async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapters-tui-home-'));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapters-tui-state-'));
  ...
  it('honors --user-plugins-dir for binary-level plugin discovery', async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapters-tui-home-'));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapters-tui-state-'));
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapters-tui-plugins-'));
