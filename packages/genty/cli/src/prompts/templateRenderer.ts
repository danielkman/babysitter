/**
 * Local Mustache-like template renderer for command prompt templates.
 *
 * Ported from @a5c-ai/babysitter-sdk/prompts/templateRenderer to remove
 * the SDK barrel dependency.  When the prompt composition layer is
 * migrated to @a5c-ai/genty-platform, this module will be replaced by
 * the platform's renderer.
 *
 * Supports:
 *   {{key}}                        — replaced with ctx[key] or extras[key] value
...
/**
 * Minimal PromptContext shape covering the fields used by the template
 * renderer.  Matches the full SDK PromptContext interface but only declares
 * the subset the renderer inspects.
 */
...
const KNOWN_STRING_KEYS: ReadonlyArray<keyof TemplateContext> = [
...
export function renderTemplateString(
  ...
  // Handle optional string sections
