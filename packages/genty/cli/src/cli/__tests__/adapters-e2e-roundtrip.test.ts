 ...
 *
 *   genty CLI (invokeViaAgentMux via agent-platform)
 ...
 *     -> parseable by adapters's babysitter adapter
 *
 * @module harnes./adapters/__tests__/e2e-roundtrip
 */
...
describe("E2E: genty <-> adapters round-trip", () => {
  it("full invocation round-trip with text, tool use, and cost", async () => {
    ...
    // Step 1: genty invokes via the agent-platform adapters bridge
    ...
    // Step 3: Format as adapters-events JSONL (what adapters babysitter adapter would parse)
  ...
  it("Pi rejection -- pi harness cannot use adapters", async () => {
    const client = createMockClient([]);
    await expect(
