 ...
 * actual genty CLI with `--output-format adapters-events` and parsing
 * its JSONL output.
 *
 * These tests are skipped when the genty CLI is not available on
 * PATH, so they are safe to run in any environment (CI or local).
 *
 * @module harnes./adapters/__tests__/live-integration
 */
...
const CLI_AVAILABLE = (() => {
  try {
    execSync("genty version", { stdio: "pipe", timeout: 10000 });
    return true;
  } catch {
    return false;
  }
...
/**
 * Spawn genty with the given args and collect stdout lines.
...
describe(
  "Live integration: genty CLI adapters-events output",
  { timeout: 30000 },
  () => {
    describe.skipIf(!CLI_AVAILABLE)("with genty CLI", () => {
      it("invoke with --output-format adapters-events produces valid JSONL", async () => {
        ...
        // genty should still emit session_start/session_end.
        //
