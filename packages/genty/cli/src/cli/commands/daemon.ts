...
import { startDaemon, stopDaemon, getDaemonStatus, loadDaemonConfig, runDaemonLoop } from "@a5c-ai/genty-platform/daemon";
import * as path from "node:path";
import * as os from "node:os";
import { executeAutomationTrigger } from "@a5c-ai/genty-platform/daemon";
import { isAutomationTriggerEvent } from "@a5c-ai/genty-platform/daemon";
...
export async function handleDaemonRun(args: {
  ...
  let config: import("@a5c-ai/genty-platform/daemon").DaemonConfig;
  try {
    const { promises: fsPromises } = await import("node:fs");
    const raw = await fsPromises.readFile(metadataPath, "utf-8");
    const parsed = JSON.parse(raw) as import("@a5c-ai/genty-platform/daemon").DaemonConfig;
    config = parsed;
  } catch {
