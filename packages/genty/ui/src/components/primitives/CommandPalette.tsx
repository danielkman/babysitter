...
function getRecentCommands(): string[] {
  try {
    return JSON.parse(localStorage.getItem(RECENT_COMMANDS_KEY) || '[]') as string[];
  } catch (err) {
    console.warn('[command-palette] Failed to read recent commands from localStorage:', (err as Error).message || err);
    return [];
  }
}
...
function addRecentCommand(id: string): void {
  try {
    ...
    console.warn('[command-palette] Failed to write recent commands to localStorage:', (err as Error).message || err);
  }
}
...
function buildCommands(org: string): Command[] {
  const prefix = (path: string) => '/orgs/' + org + (path === '/' ? '' : path);
  return [
    // Navigate
...
export interface CommandPaletteProps {
  /** The org slug used to build navigation paths */
  org: string;
  ...
  onClose: () => void;
  /**
   * Called when the user selects a command with an href.
   * Replaces next/navigation useRouter().push().
   */
  onNavigate: (path: string) => void;
}

export function CommandPalette({ org, isOpen, onClose, onNavigate }: CommandPaletteProps): JSX.Element | null {
  ...
  const recentCommands = showRecent
    ? recentCommandIds.map((id) => allCommands.find((c) => c.id === id)).filter((c): c is Command => Boolean(c))
    : [];
  ...
  function renderCommandItem(cmd: Command, absoluteIdx: number) {
...
export function CommandPaletteWrapper({ org, onNavigate }: CommandPaletteWrapperProps): JSX.Element {
  ...
  return <CommandPalette org={org} isOpen={isOpen} onClose={() => setIsOpen(false)} onNavigate={onNavigate} />;
}
