...
interface EventData {
  ...
  agentIdentity?: { displayName?: string };
  spec?: { agentDefinition?: string; agentStack?: string; action?: string };
  name?: string;
  status?: string;
  action?: string;
...
function mapEventToNotification(event: EventData): NotificationItem {
  let type = 'system';
...
export interface NotificationBellProps {
  /** The org slug used to build the SSE stream URL */
  org: string;
}

export function NotificationBell({ org }: NotificationBellProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      es.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data) as EventData & { type?: string };
