...
export interface AgentProfileOption {
  type: 'agentDefinition' | 'agentStack';
  value: string;
  label: string;
  hint?: string;
  stackRef?: string;
  stack?: AgentStackOption;
}

export interface AgentStackOption {
...
export interface MeetingOption {
  metadata?: { name?: string };
  spec?: { displayName?: string; roomId?: string };
  status?: { phase?: string };
}

export interface DispatchButtonProps {
  org: string;
  stacks?: AgentStackOption[];
...
function agentIdentityOptions(
  ...
  // Agents are already in option format when passed from outside
  const stackOptions: AgentProfileOption[] = stacks
    .flatMap((stack) => {
      const name = stack.metadata?.name;
      if (!name) return [];
      return [{
        type: 'agentStack' as const,
        value: name,
        label: name,
        hint: 'Legacy stack',
        stack,
  ...
  return [...agents, ...stackOptions];
}

export function DispatchButton({ org, stacks = [], agents = [], meetings = [] }: DispatchButtonProps) {
  const [status, setStatus] = useState<'idle' | 'selecting' | 'dispatching' | 'success' | 'error'>('idle');
  ...
  const selectedStack =
    stacks.find(
      (stack) => typeof stack !== 'string' && stack.metadata?.name === selectedStackName
    ) ||
  ...
  async function handleDispatch() {
    ...
    try {
      const res = await fetch(`/api/orgs/${org}/agents/dispatch`, {
        ...
        body: JSON.stringify({
          ...(selectedOption?.type === 'agentDefinition'
            ? { agentDefinition: selectedTarget }
            : { agentStack: selectedTarget }),
      ...
      setStatus('error');
      setMessage((err instanceof Error ? err.message : null) || 'Network error');
    }
  }
  ...
  const primaryStyle: React.CSSProperties = { ...buttonStyle, backgroundColor: '#2563eb', color: '#fff' };
  ...
  if (status === 'idle') {
    return (
      <button onClick={() => setStatus('selecting')} style={primaryStyle}>
        Dispatch Agent
      </button>
    );
  }
  ...
  if (status === 'selecting') {
    return (
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0', flexWrap: 'wrap' }}>
        <label
          htmlFor="dispatch-stack-select"
          style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' }}
        >
          Agent stack
        </label>
        <select
          id="dispatch-stack-select"
        ...
        {canJoinMeeting ? (
          <>
            <label
              htmlFor="dispatch-meeting-select"
              style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' }}
            >
              Meeting
            </label>
            <select
              id="dispatch-meeting-select"
              value={meetingRef}
            ...
            >
              ...
              {activeMeetings.map((meeting) => (
                <option
                  key={meeting.metadata?.name || meeting.spec?.roomId}
                  value={meeting.metadata?.name || meeting.spec?.roomId}
                >
                  {meeting.spec?.displayName || meeting.metadata?.name || meeting.spec?.roomId}
                </option>
              ))}
            </select>
          </>
        ...
        <label
          htmlFor="dispatch-repo-input"
          style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' }}
        >
          Repository
        </label>
        <input
          id="dispatch-repo-input"
          placeholder="Repository (optional)"
          value={repository}
          onChange={(e) => setRepository(e.target.value)}
          style={{ padding: '6px 12px', borderRadius: 4, border: '1px solid #d1d5db', fontSize: 13, width: 180 }}
          aria-label="Repository (optional)"
        />
        <button
          onClick={handleDispatch}
          disabled={!selectedTarget}
          aria-label="Launch dispatch"
          style={{ ...primaryStyle, opacity: selectedTarget ? 1 : 0.5 }}
        >
          Launch
        </button>
        <button
