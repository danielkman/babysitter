...
export function KanbanBoard({
  ...
  const [selectedCard, setSelectedCard] = useState<KanbanIssue | null>(null);
  const [filterAssignee, setFilterAssignee] = useState('');
  const [filterLabel, setFilterLabel] = useState('');
  const [searchText, setSearchText] = useState('');
  const [groupBy, setGroupBy] = useState('none');
  const [pendingWorkspaceItem, setPendingWorkspaceItem] = useState<KanbanIssue | null>(null);
  const [boardError, setBoardError] = useState<string | null>(null);
  const dragItemRef = useRef<KanbanIssue | null>(null);
  ...
  const handleCardClick = useCallback((item: KanbanIssue) => {
    setSelectedCard(item);
  }, []);

  const handleStartWork = useCallback(
    async (item: KanbanIssue) => {
      ...
      try {
        ...
        setBoardError(`Workspace creation failed: ${(err as Error).message || 'unknown error'}`);
      } finally {
  ...
  const handleStartWorkFromButton = useCallback((item: KanbanIssue) => {
    setPendingWorkspaceItem(item);
  }, []);

  const getColumnColor = (colId: string) => WORKFLOW_COLUMNS.find((c) => c.id === colId)?.color || '#6b7280';
