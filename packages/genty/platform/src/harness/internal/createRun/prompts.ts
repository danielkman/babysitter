...
export async function buildProcessDefinitionSystemPrompt(
  ...
  return [
    ...
    "",
    "Rules:",
    "- This phase is AUTHORING-ONLY. Your tools are the file tools (read/write/edit/grep/find) plus AskUserQuestion, task, skill, and babysitter_report_process_definition. You do NOT have bash/python/ssh/browser/web/code-execution tools in this phase.",
    "- Do not attempt to perform the user's requested work yourself here (do not write the user's deliverable file, do not run commands). Your only deliverable in this phase is the process-definition JavaScript file plus the babysitter_report_process_definition call. The authored process performs the user's work later, in the orchestration phase.",
    ...
    "- Before authoring the process in any mode, conduct a real search against the active shared process-library using the file tools (`read`, `find`, `grep`). Do not skip this process-library search step.",
    "- Treat the active process-library root provided in the prompt context as the first place to search. If you need adjacent material, inspect the cloned repo root or reference area using the file tools.",
    "- Research the workspace before finalizing the process. Use your available read/find/grep/write/edit tools as needed.",
    "- Treat the provided workspace as the only relevant filesystem root unless the user explicitly points you somewhere else.",
    "- You may inspect local babysitter process references when they materially improve the process design. Prefer project `.a5c/processes/`, the active process-library root returned in `binding.dir`, the cloned repo root returned in `defaultSpec.cloneDir` when you need adjacent reference material, the repository `library/` process library (methodologies/ and specializations/), and local babysitter discover/profile CLI commands when available.",
    ...
    "- DELEGATE THE DELIVERABLE TO AGENT TASKS (mandatory): the user's deliverable MUST be produced by `kind: \"agent\"` tasks that you create with `defineTask(...)` and invoke with `await ctx.task(...)`. Do NOT do the work directly in `process(inputs, ctx)`, and do NOT use a `node`/direct `fs.writeFile`/inline file write to produce the deliverable — route it through agent tasks. A direct/inline write produces too few journal events and fails run-completion.",
    "- AUTHOR EXACTLY TWO SEQUENTIAL AGENT TASKS for this run shape: (1) a producer `agent` task that generates the full final content AND writes it to the user's exact output path in a single file write; then (2) a verifier `agent` task that reads that exact path back, confirms the file exists and contains the required content, and reports a short pass/fail. Invoke them in order with `await ctx.task(...)`. Two delegated agent tasks are REQUIRED: a single task does not emit enough journal events for the run to complete — the run needs at least two task effects (the verifier is what makes the journal sufficient). Do NOT collapse this into one task, and do NOT pad it with more than these two.",
    "- Do NOT use `kind: \"node\"`, `kind: \"shell\"`, or `kind: \"breakpoint\"` tasks for this run shape. Both tasks must be `kind: \"agent\"`.",
    ...
    "- The agent task shape is `defineTask(\"<id>\", (args) => ({ kind: \"agent\", title: \"...\", agent: { name: \"...\", prompt: \"...\" } }))`. Put all the work instructions — generate the content AND write it to the exact path (producer), or read+verify that exact path (verifier) — inside `agent.prompt`.",
    ...
    "- EXACT OUTPUT PATH (mandatory): when the user names a specific output file (for example `.a5c-live-test/<id>-foo.md`), the agent task MUST target that EXACT path, character-for-character, including any session-id prefix. Use the user's path verbatim — do NOT paraphrase, rename, shorten, or regenerate the filename.",
    "- The process runs with its current working directory set to the workspace root, so a relative output path resolves correctly against cwd as-is. Pass the user's exact RELATIVE path string straight through to the agent task's prompt. Do NOT recompute the workspace root from the module location (`import.meta.url` / `path.dirname(fileURLToPath(...))` / `path.resolve(moduleDir, '..')`) to build the output path — that is error-prone (the process file lives under `.a5c/processes/`, so deriving the workspace root from it commonly mis-counts directory levels and writes the file to the wrong place). If you need an absolute path, use `path.resolve(<the exact relative path>)` against cwd, not against the module directory.",
    "- The agent task that produces the user's deliverable MUST be instructed to write the FULL final content to that EXACT path in a single file write. Embed the exact target path verbatim in `agent.prompt` and tell the worker to write that exact path verbatim — do not let the worker choose or paraphrase the filename.",
...
export function buildProcessDefinitionUserPrompt(
  ...
  if (workspaceAssessment === "empty") {
    ...
    lines.push(
      ...
      "Author exactly TWO sequential `kind: \"agent\"` tasks created with `defineTask(...)` and invoked via `await ctx.task(...)`: (1) a producer that generates the full final content AND writes it to the user's exact output path in a single file write; then (2) a verifier that reads that exact path back and confirms the file exists with the required content. Do not produce the deliverable with a `node`/inline `fs.writeFile` or directly inside `process()`. Two delegated agent tasks are required so the run emits enough journal activity to complete; a single task is not enough.",
      preferAgentOnlyTasks
        ? "Put all implementation in `agent` tasks. Do not generate `shell`, `node`, or `breakpoint` tasks for this run shape unless the user explicitly required an existing CLI command."
        : "Put the main implementation in `agent` tasks. Use `shell` tasks only for concrete runnable verification or tooling commands.",
      preferAgentOnlyTasks
