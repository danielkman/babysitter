...
export async function buildProcessDefinitionSystemPrompt(
  ...
  return [
    ...
    "- EXACT OUTPUT PATH (mandatory): when the user names a specific output file (for example `.a5c-live-test/<id>-foo.md`), the process MUST target that EXACT path, character-for-character, including any session-id prefix. Use the user's path verbatim — do NOT paraphrase, rename, shorten, or regenerate the filename.",
    "- The process runs with its current working directory set to the workspace root, so a relative output path resolves correctly against cwd as-is. Pass the user's exact RELATIVE path string straight through to the task/worker. Do NOT recompute the workspace root from the module location (`import.meta.url` / `path.dirname(fileURLToPath(...))` / `path.resolve(moduleDir, '..')`) to build the output path — that is error-prone (the process file lives under `.a5c/processes/`, so deriving the workspace root from it commonly mis-counts directory levels and writes the file to the wrong place). If you need an absolute path, use `path.resolve(<the exact relative path>)` against cwd, not against the module directory.",
    "- The task/worker that produces the user's deliverable MUST be instructed to write the FULL final content to that EXACT path in a single file write. Embed the exact target path in the task prompt and tell the worker to write that exact path verbatim — do not let the worker choose or paraphrase the filename.",
