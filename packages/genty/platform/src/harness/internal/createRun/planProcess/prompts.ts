...
export function buildExternalProcessConformancePrompt(args: {
  ...
  return [
    ...
    "- Agent responder tasks must use `kind: \"agent\"` with `agent: { name, prompt, responderType: \"agent\", adapter: \"...\" }`; `adapter` must be a non-empty installed adapters adapter name.",
