...
function createDeps(overrides: Partial<WorkspaceLifecycleDeps> = {}): WorkspaceLifecycleDeps {
  ...
  const execGit = vi.fn<WorkspaceLifecycleDeps["execGit"]>(async (args, cwd) => {
    ...
    const map: Record<string, { stdout: string; stderr: string }> = {
      ...
      [`${taskPath}::status --porcelain`]: { stdout: " M packages/adapters/webui/src/pages/KanbanLayout.tsx\n", stderr: "" },
      ...
      workspaces: {
        [repoPath("repo", "worktrees", "task")]: {
          ...
          rebase: {
            ...
            attemptCount: 0,
            unresolvedFiles: [
              "packages/adapters/webui/src/components/workspaces/workspaces-page.tsx",
              "packages/adapters/webui/src/lib/workspace-lifecycle.ts",
            ],
            resolvedFiles: [],
