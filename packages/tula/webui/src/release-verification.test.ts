...
function withPackageRoot(run: (packageRoot: string) => void): void {
  ...
  fs.writeFileSync(
    ...
    '# @a5c-ai/tula-webui\n\nBuilt on `@a5c-ai/tula-ui/session-flow`.\n',
  );
        ...
        verifyAgentMuxWebuiRelease({
          ...
          manifest: {
            ...
            scripts: {
              ...
              test: 'vitest run --root ../../.. --config vitest.config.ts packages/adapters/webui',
            },
          },
          packEntries: basePackEntries,
        })
      ...
      expect(() =>
        verifyAgentMuxWebuiRelease({
          ...
          manifest: {
            ...
            scripts: {
              ...baseManifest.scripts,
              'test:realtime': 'vitest run --root ../../.. --config vitest.config.ts "packages/adapters/webui/src/pages/SessionDetailPage.test.ts"',
            },
          },
          packEntries: basePackEntries,
        })
