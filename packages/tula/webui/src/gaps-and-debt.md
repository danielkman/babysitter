...
This document tracks the remaining board-, issue-, and workspace-first product gaps for the browser kanban surface after ownership moved into `packages/adapters/webui`, with shared contracts in `packages/adapters/ui` and server/runtime ownership in `packages/adapters/gateway`.
...
`packages/adapters/webui` should be understood as:
