...
export function verifyAgentMuxWebuiRelease({ packageRoot, manifest, packEntries }) {
  ...
  expect(
    manifest.name === '@a5c-ai/tula-webui',
    'packages/adapters/webui/package.json name must stay @a5c-ai/tula-webui'
  );
  expect(
    manifest.publishConfig?.access === 'public',
    'packages/adapters/webui/package.json publishConfig.access must stay public'
  );
  expect(
    scripts['build:realtime'] === 'npm run build --workspace=@a5c-ai/atlas && npm run build --workspace=@a5c-ai/adapters-comm && npm run build --workspace=@a5c-ai/tula-ui && npm run build',
    'packages/adapters/webui/package.json build:realtime must remain the package-local realtime build entrypoint'
  );
  expect(
    scripts.test === 'vitest run --config vitest.config.ts',
    'packages/adapters/webui/package.json test must keep the package-local Vitest config entrypoint stable'
  );
  expect(
    typeof scripts['test:realtime'] === 'string' &&
      scripts['test:realtime'].includes('packages/adapters/webui/src/pages/SessionDetailPage.test.ts') &&
      scripts['test:realtime'].includes('packages/adapters/webui/src/pages/SessionDetailPage.route.test.tsx') &&
      scripts['test:realtime'].includes('packages/adapters/webui/src/release-verification.test.ts'),
    'packages/adapters/webui/package.json test:realtime must keep the session-detail and release verification suite together'
  );
  expect(
    scripts['verify:release'] === 'node ./scripts/verify-release.mjs',
    'packages/adapters/webui/package.json verify:release must point at the package-local release verifier'
  );
  expect(
    scripts.prepublishOnly === 'npm run build:realtime && npm run test && npm run verify:release',
    'packages/adapters/webui/package.json prepublishOnly must exercise the package-local test and release seam directly'
  );
  expect(
    readme.includes('@a5c-ai/tula-ui/session-flow'),
    'packages/adapters/webui/README.md must keep documenting the shared session-flow dependency seam'
  );
