...
const gatewayPort = Number.parseInt(process.env.AGENT_MUX_WEBUI_E2E_PORT ?? "4175", 10);
const gatewayHost = "127.0.0.1";
const baseUrl = `http://${gatewayHost}:${gatewayPort}`;
const tempRoot = path.join(packageRoot, ".tmp", `adapters-webui-e2e-fixture-${gatewayPort}`);
const stateFile = path.join(packageRoot, ".tmp", `adapters-webui-e2e-state-${gatewayPort}.json`);
