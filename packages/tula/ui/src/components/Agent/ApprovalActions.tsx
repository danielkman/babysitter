...
export function ApprovalDecisionButtons({ org, approvalName }: ApprovalDecisionButtonsProps) {
  ...
  async function handleDecision(decision: 'approve' | 'deny') {
    ...
    try {
      const response = await fetch(
        `/api/orgs/${encodeURIComponent(org)}/agents/approvals/${encodeURIComponent(approvalName)}/decide`,
        {
          method: 'POST',
  ...
  if (status === 'approved') {
    return (
      <div
        ...
        style={{ justifyContent: 'flex-start', gap: '0.5rem', marginTop: '0.75rem' }}
      >
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={`Approve approval request ${approvalName}`}
        style={{
          background: 'var(--color-good, #22863a)',
          color: '#fff',
          border: 'none',
          padding: '0.4rem 1rem',
          borderRadius: '6px',
      ...
      <button
        ...
        disabled={status === 'loading'}
        aria-label={`Deny approval request ${approvalName}`}
        style={{
      ...
      {status === 'error' && message ? (
        <span role="alert" style={{ color: 'var(--color-danger, #cb2431)', fontSize: '0.85rem' }}>
          {message}
        </span>
      ) : null}
    </div>
