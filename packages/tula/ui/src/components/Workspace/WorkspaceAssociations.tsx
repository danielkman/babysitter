...
export interface AssociationsSectionProps {
  ...
  onAdd?: (association: WorkspaceAssociation) => void;
  onRemove?: (association: WorkspaceAssociation) => void;
}

const ASSOCIATION_KINDS = ['User', 'AgentDispatchRun', 'AgentSession'];
...
export function AssociationsSection({ associations = [], onAdd, onRemove }: AssociationsSectionProps) {
      ...
      {associations.length > 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '0.5rem' }}>
          {associations.map((a, i) => {
            const colors = KIND_COLORS[a.kind] || { bg: '#f3f4f6', fg: '#6b7280' };
            return (
              <div
                key={`${a.kind}-${a.name}-${i}`}
                style={{
        ...
        <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginBottom: '0.5rem' }}>No associations</div>
      )}
