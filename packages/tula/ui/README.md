...
```bash
npm install @a5c-ai/tula-ui react react-dom react-native react-native-web
```
...
- `@a5c-ai/tula-ui/gateway`
- `@a5c-ai/tula-ui/protocol`
- `@a5c-ai/tula-ui/session-flow`
...
import { GatewayClient, GatewayProvider } from "@a5c-ai/tula-ui";
import { buildSessionFlowModel } from "@a5c-ai/tula-ui/session-flow";
```

`@a5c-ai/tula-ui` does **not** ship a package CSS entrypoint. Browser apps such as `@a5c-ai/tula-webui` own their app CSS and any external design-system CSS they choose to use.
...
- use `@a5c-ai/tula-ui/session-flow` when a surface needs a reusable realtime execution model
- keep product-specific routing, layout, page composition, and browser-only deep links in consuming apps such as `@a5c-ai/tula-webui`
- do not treat this package as the owner of app-specific session pages or kanban workflow policy

## Validation

```bash
npm run build:realtime --workspace=@a5c-ai/tula-ui
npm run test --workspace=@a5c-ai/tula-ui
npm run test:realtime --workspace=@a5c-ai/tula-ui
npm run verify:release --workspace=@a5c-ai/tula-ui
npm run verify:metadata
npm pack --json --dry-run --workspace=@a5c-ai/tula-ui
```
...
`@a5c-ai/tula-ui` is a public shared package. Keep this README aligned with the exported client/store/hooks/session-flow surface and keep `package.json#files` limited to built artifacts plus package docs.
