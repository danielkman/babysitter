 ...
 * full adapters graph (for example when a required Node built-in is absent).
 */
...
function normalizeAmuxLoadError(error: unknown): Error {
  ...
  if (
    ...
    return new Error(
      `adapters runtime is unavailable in this Node environment: ${message}`,
      { cause: error },
    );
  }
...
/**
 * Check whether adapters client can be created.
 */
