...
|---|---|---|---|---|
| 11-layer stack as the canonical frame | universal/00-overview.md, 02-stack.md, architecture/stack-diagram.md | NodeKind `Layer` (cluster 1); 11 instances `layer:1-model` … `layer:11-presentation`; Term with kind=layer | ✅ | each layer reified as graph node; cross-cluster `realizes` edge from impl nodes |
...
| Layer 10 — Interaction | universal/02-stack.md §Layer 10 | `Layer` `layer:10-interaction`; `InteractionPattern` and `InteractionPrimitive` NodeKinds (cluster 4) | ✅ | nine categories captured (a–i) |
