...
| Layer 4 — Agent-Core | universal/02-stack.md §Layer 4 | `Layer` `layer:4-genty-core`; `AgentCoreImpl` realizes it; loop/context/tool/subagent/result/stop concerns are responsibilities/examples on the layer and attributes/edges on implementations | ✅ | seven named sub-components captured |
| Layer 5 — Agent-Runtime | universal/02-stack.md §Layer 5 | `Layer` `layer:5-genty-runtime`; `AgentRuntimeImpl` realizes it; built-in tools, session state, hooks, registry, approvals, MCP, and identity are runtime attributes/edges | ✅ | per-product built-in tool sets recorded as `ToolDescriptor` instances `provides` `AgentRuntimeImpl` |
| Layer 6 — Agent-Platform | universal/02-stack.md §Layer 6 | `Layer` `layer:6-genty-platform`; `AgentPlatformImpl` realizes it; plugin/skill/channel/identity/marketplace/update concerns are platform attributes/edges | ✅ | sub-components per row below |
...
| 22 live node kinds in `atlas-catalog` | universal/07-cross-cutting.md §1.1, a5c/00-overview.md (catalog node-kind glossary), a5c/01-component-map.md | each live node kind has a `Term` `kind=concept` `references` to its atlas schema NodeKind counterpart; atlas expanded to 99 NodeKinds across 14 clusters | ✅ | resolved 2026-04-28 — mapping table in `./schema/node-kinds/README.md` (legacy `TransportProtocol`+`TransportRuntime` → atlas `ModelTransportProtocol`+`Provider`; legacy `Harness` → atlas `AgentRuntimeImpl`+`AgentCoreImpl`+`AgentPlatformImpl`; legacy `Tool` → atlas `ToolDescriptor`+`ToolServer`; legacy `Plugin` retained, gains `NativeExtension`/`ExtensionInterface`). |
| 18 live edge kinds in atlas-catalog | universal/07-cross-cutting.md §1.1, a5c/00-overview.md | edge mapping recorded; full table in `./schema/edge-kinds.md`; atlas expanded to 139 edges | ✅ | resolved 2026-04-28 — legacy 18 absorbed (served_by/speaks/spoken_by, supported_by, bridges, replaces/replaced_by, evidenced_by); remaining 121 edges represent newly-modeled relations. |
...
| Benchmark scoping (targets Skill / Plugin / Subagent / Agent / ModelVersion / Domain / Specialization / Topic / StackProfile / stack-layer / genty-platform) | universal/07-cross-cutting.md §8 | edge `Benchmark targets <NodeKind>` | ✅ | |
...
| Discovery protocols (env-var detection, binary on PATH, Windows registry, file presence) | universal/04-protocols.md §11 | enum `DiscoverySignal.kind` (cluster 13 carry-over) | ✅ | covered by `DiscoverySignal` (live in atlas-catalog) |
...
| genty-platform (special Layer-6 entry) | a5c/00-overview.md, 01-component-map.md, ADR-001 | `AgentProduct id=agent:genty-platform` | ✅ | semantic contract per ADR-001 |
...
| Concept | Source | Schema mapping | Status | Notes |
|---|---|---|---|---|
| `@a5c-ai/atlas/catalog` | a5c/01-component-map.md | `PackageSurface` + `CiSurface` (both populated; example: graph/catalog-meta/package-surfaces/atlas-catalog.yaml) | ✅ | promoted from stub |
