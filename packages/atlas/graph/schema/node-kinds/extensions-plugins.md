...
The per-agent target spec used by the `extensions-adapter` / `extensions-adapter` to know
...
| `adapterName` | string | no | Internal adapter id used by `extensions-adapter` (e.g. `claude-code`, `codex`, `omp`). Legacy required; atlas optional pending evidence. |
...
| `pairs_with` | `PortableExtension` | N:N | Records that a `PortableExtension` compiles to this target via the extensions-adapter. |
| `sourced_from` | `SourceRef` | N:1 | Source-of-truth for the target spec (typically `packages/extensions-adapter/src/targets/<adapter>.ts`). |
...
- `NativeExtension` — the per-target compiled output the extensions-adapter produces.
