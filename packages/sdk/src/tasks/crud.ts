 ...
 *
 * SDK functions for creating, reading, listing, and updating tasks
...
/**
 * List tasks in a run directory with optional status filter.
 */
export async function listTasks(
  ...
  for (const effectId of entries) {
    ...
    try {
      ...
      continue; // Skip entries without task.json
    }
    ...
    summaries.push({
      ...
      status: taskStatus,
      labels: Array.isArray(definition?.labels)
        ? (definition.labels as string[])
        : undefined,
...
/**
 * Read a task's full definition and result.
 */
...
/**
 * Read a task's stdout output if available.
 */
export async function readTaskStdout(
  ...
  try {
    return await fs.readFile(
      path.join(runDir, "tasks", effectId, "stdout.txt"),
      "utf-8",
    );
  } catch {
    return null;
...
/**
 * Read a task's stderr output if available.
 */
export async function readTaskStderr(
  ...
  try {
    return await fs.readFile(
      path.join(runDir, "tasks", effectId, "stderr.txt"),
      "utf-8",
    );
  } catch {
    return null;
...
/**
 * Count tasks by status in a run.
 */
