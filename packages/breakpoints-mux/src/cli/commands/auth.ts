...
function createServerConfigCommand(): Command {
  const cmd = new Command("server").description("Manage the saved breakpoints-mux server URL");

  cmd
    .command("set")
    .argument("<url>", "Breakpoints-mux server URL")
    .description("Save a default server URL in ~/.breakpoints-mux/config.json")
      ...
      try {
        ...
        if (jsonMode) {
          console.log(JSON.stringify({
            ...
            source: "~/.breakpoints-mux/config.json",
          }));
        } else {
          console.log(`Saved default server URL to ~/.breakpoints-mux/config.json: ${resolveServerUrl(url)}`);
        }
      } catch (error) {
        printError(error, jsonMode);
        process.exitCode = 1;
  ...
  cmd
    .command("clear")
    .description("Remove the saved default server URL from ~/.breakpoints-mux/config.json")
    .action((_opts, command: Command) => {
      ...
      try {
        ...
        if (jsonMode) {
          console.log(JSON.stringify({ status: "cleared", key: "serverUrl", source: "~/.breakpoints-mux/config.json" }));
        } else {
          console.log("Cleared saved default server URL from ~/.breakpoints-mux/config.json");
        }
      } catch (error) {
        printError(error, jsonMode);
        process.exitCode = 1;
...
function createTokenCommand(): Command {
  ...
  cmd
    ...
    .description("Save a bearer token in ~/.breakpoints-mux/config.json")
    .action((token: string, _opts: Record<string, unknown>, command: Command) => {
      ...
      try {
        ...
        if (jsonMode) {
          console.log(JSON.stringify({ status: "saved", source: "~/.breakpoints-mux/config.json" }));
        } else {
          console.log("Saved bearer token to ~/.breakpoints-mux/config.json");
        }
      } catch (error) {
        printError(error, jsonMode);
        process.exitCode = 1;
  ...
  cmd
    .command("clear")
    .description("Remove the saved bearer token from ~/.breakpoints-mux/config.json")
    .action((_opts, command: Command) => {
      const allOpts: GlobalOpts = command.optsWithGlobals();
      const jsonMode = allOpts.json === true;
      ...
      try {
        ...
        if (jsonMode) {
          console.log(JSON.stringify({ status: "cleared", source: "~/.breakpoints-mux/config.json" }));
        } else {
          console.log("Cleared saved bearer token from ~/.breakpoints-mux/config.json");
        }
      } catch (error) {
        printError(error, jsonMode);
        process.exitCode = 1;
      ...
      try {
        ...
        if (!token && !auth) {
          if (jsonMode) {
            ...
            console.log("Not authenticated. Use `breakpoints-mux auth login`, `breakpoints-mux auth token set`, or BMUX_AUTH_TOKEN.");
          }
          return;
        }
  ...
  return new Command("keygen")
    ...
    .action((_opts, command: Command) => {
      ...
      try {
        ...
        // Save private key to ~/.breakpoints-mux/keys/{fingerprint}.pem
    ...
    .action(async (opts, command: Command) => {
      ...
      try {
        ...
        if (keyFiles.length === 0) {
          throw new Error("No keys found. Run `breakpoints-mux auth keygen` first.");
        }
      ...
      try {
        ...
        if (keyFiles.length === 0) {
          if (jsonMode) {
            ...
            console.log("No keys found. Run `breakpoints-mux auth keygen` to generate a key pair.");
          }
          return;
        }
