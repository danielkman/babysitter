 ...
 *
 * Tests that the kradle-sdk exports function correctly as an integrated unit:
...
function createMockGateway() {
  ...
  return {
    ...
    async snapshot() {
      ...
      return { namespace: 'kradle-org-sdk-test', resources: byKind };
    },
    watch() { return { close: () => {} }; },
    resourceDefinitions: [],
  };
...
test('SDK integration: create controller → apply resource → list includes it', async () => {
  ...
  const repo = createResource(
    'Repository',
    { name: 'sdk-test-repo', namespace: 'kradle-org-sdk-test' },
...
test('SDK integration: createControllerUiModel with snapshot returns valid model', () => {
  const snapshot = {
    ...
    resources: {
      Repository: [
        { kind: 'Repository', metadata: { name: 'test-repo', namespace: 'kradle-org-default' }, spec: { organizationRef: 'default', visibility: 'internal' } },
      ],
      AgentStack: [
        { kind: 'AgentStack', metadata: { name: 'review-bot', namespace: 'kradle-org-default' }, spec: { organizationRef: 'default', baseAgent: 'claude-code', adapter: 'default', runtimeIdentity: 'workspace' } },
      ],
    },
  };
...
test('SDK integration: createSessionCookie → parseSessionCookie roundtrip', () => {
  const config = createAuthProviderConfig({ KRADLE_AUTH_COOKIE_NAME: 'kradle_sdk_test' });
  ...
  assert.ok(cookie.includes('kradle_sdk_test='), 'cookie must have the configured cookie name');

  // Extract just the cookie value
  const cookieValue = cookie.match(/kradle_sdk_test=([^;]+)/)?.[1];
  ...
  assert.equal(parsed.cookieName, 'kradle_sdk_test');
});
...
test('SDK integration: createResource → clone → resourceToYaml produces valid YAML', () => {
  const repo = createResource(
    'Repository',
    { name: 'yaml-test-repo', namespace: 'kradle-org-yaml' },
