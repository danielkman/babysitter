...
test('SDK exports createKradleApiController', () => {
  assert.equal(typeof sdk.createKradleApiController, 'function');
});

test('SDK exports KRADLE_API_CONTROLLER_BOUNDARY', () => {
  assert.ok(sdk.KRADLE_API_CONTROLLER_BOUNDARY);
  assert.equal(sdk.KRADLE_API_CONTROLLER_BOUNDARY.role, 'kradle-api-controller');
});
...
test('SDK exports auth functions', () => {
  ...
  assert.equal(typeof sdk.mapLoginProfileToKradleIdentity, 'function');
...
test('SDK exports orgNamespaceName', () => {
  assert.equal(sdk.orgNamespaceName('my-org'), 'kradle-org-my-org');
});
...
test('createGiteaService returns null when no giteaUrl configured', () => {
  // Without KRADLE_GITEA_HTTP_URL set, the service must return null
  const savedUrl = process.env.KRADLE_GITEA_HTTP_URL;
  ...
  if (savedUrl !== undefined) process.env.KRADLE_GITEA_HTTP_URL = savedUrl;
});
