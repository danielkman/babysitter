...
- Make permission intent easy to manage from Kradle without hiding the native objects that enforce it.
...
| Actor | Kubernetes identity | Kradle management surface | Notes |
| --- | --- | --- | --- |
...
Kradle-managed projection to Kubernetes `Role`, `ClusterRole`, `RoleBinding`, and `ClusterRoleBinding` resources.

Important fields:

- `spec.subject`: user, group, team, agent-stack, runner-pool, service-account;
- `spec.scope`: namespace, repository, organization, cluster;
- `spec.roleRef`: native Kubernetes Role/ClusterRole or Kradle role template;
- `spec.resourceRules`: optional generated rules for Kradle resources;
...
Kradle should ship role templates that compile into native Kubernetes roles. Users may still bind pre-existing native Roles/ClusterRoles, but templates make safe paths obvious.

| Template | Intended subject | Native capabilities | Guardrails |
| --- | --- | --- | --- |
| `kradle-agent-readonly` | agent stack, subagent | get/list/watch repository, issue, PR, pipeline, job, context bundle metadata | no Secret read, no write-back, no pod exec. |
| `kradle-agent-repository-repair` | trusted repair agent | read repo graph, create dispatch artifacts, patch allowed PR/issue records | branch push and PR comments still require write-back approval. |
| `kradle-agent-ci-diagnoser` | CI diagnosis agent | read pipeline/job/log/artifact projections | no repository mutation and no privileged secrets. |
| `kradle-agent-workspace-manager` | workspace controller/agent | create/link/archive/recover agent workspaces | cannot access unrelated namespaces or runner pools. |
| `kradle-runner-untrusted` | untrusted runner pool | create own pods/jobs, stream own logs, read no Secrets | forced for forks and untrusted refs. |
| `kradle-runner-trusted` | trusted runner pool | create own pods/jobs, mount admitted Secrets/ConfigMaps | only trusted refs and approved task kinds. |
| `kradle-secret-grant-admin` | platform admin | create/update `AgentSecretGrant` and `AgentConfigGrant` resources | native RBAC must also allow binding or reading target metadata. |
| `kradle-agent-approver` | maintainer/team | approve selected `AgentApproval` classes | cannot approve grants that would escalate beyond their own bind permissions. |
...
For every stack save, trigger dry-run, and dispatch creation, Kradle should run the same deterministic resolver:
...
- Creates or updates native `ServiceAccount`, `Role`, `ClusterRole`, `RoleBinding`, and `ClusterRoleBinding` resources when Kradle owns them.
- Imports externally managed native RBAC into read-only projections when Kradle does not own them.
...
Kradle must distinguish native objects it owns from native objects it only observes:

- Owned objects carry Kradle labels/owner references and are reconciled back to desired state.
...
- Deleting a Kradle grant should remove only Kradle-owned native bindings/mount wiring, never arbitrary external RBAC objects.
...
- ConfigMap values may be shown only when native RBAC allows `get` and Kradle marks the key non-sensitive.
...
- ConfigMaps can show values only when RBAC permits `get` on the ConfigMap and Kradle policy marks the key non-sensitive.
...
- Native Kubernetes RBAC remains the enforcement point; Kradle resources are declarative management and UI projection, not a parallel authorization backend.
