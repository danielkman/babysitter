...
4. Kradle checks repository policy before a tool/skill/MCP server is attached to a stack or dispatch.
...
5. When an event matches, Kradle creates a dispatch run and links it to the source object.
...
Kradle-managed wrapper around native Kubernetes `ServiceAccount` identity for agent stacks and runner pools.
...
Kradle-managed intent that syncs to native Kubernetes `Role`, `ClusterRole`, `RoleBinding`, or `ClusterRoleBinding` resources.

Important fields:

- `spec.subject`: user, group, team, agent-stack, runner-pool, service-account;
- `spec.scope`: namespace, repository, organization, cluster;
- `spec.roleRef`: native role or Kradle role template;
- `spec.resourceRules`;
...
Before dispatch, Kradle must validate:
