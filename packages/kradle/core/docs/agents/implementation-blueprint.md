...
| UI frame | GitHub-like Kradle navigation, route hierarchy, resource forms, policy explanations | chat transcript, event timeline, runtime panels that can be embedded |
...
## Existing Kradle files to extend
...
- `src/kubernetes-controller.js`
  - Reconcile declarative agent config resources and expose them through the existing Kradle resource gateway.
  - Watch CRD-backed config changes and trigger validation refreshes.
- `src/api-controller.js`
  - Add agent API routes through the existing controller boundary; do not bypass the Kradle gateway from UI components.
...
- New `src/agent-rbac-controller.js`
  - Sync Kradle role/service-account intent to native Kubernetes ServiceAccounts, Roles, ClusterRoles, RoleBindings, and ClusterRoleBindings.
...
2. Kradle records source actor, comment/label payload, repository trust, and context label set.
...
Before implementing company brain memory, Kradle should add org scoping foundations:
