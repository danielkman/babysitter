...
- How issue/session/workspace/dispatch relationships from Agent Mux kanban should become first-class system capabilities in Kradle, not just UI widgets.
...
- How Agent Mux can provide adapters, session streaming, chat, cancellation, run events, plugin/tool discovery, and workspace/session primitives while Kradle remains the repository, policy, trigger, and audit source of truth.
...
- [Agent CRD schema spec](./crd-schema-spec.md) maps agent resources onto Kradle's existing `CONFIG_KINDS`, `AGGREGATED_KINDS`, schemas, labels, status conditions, and storage classes.
...
- [Resource relationship map](./resource-relationship-map.md) shows how agent resources connect to existing Kradle repositories, PRs, issues, pipelines, webhooks, identity, RBAC, secrets, and UI pages.
