...
The MVP is a deterministic Kubernetes-native contract implementation of the Kradle architecture. Local tests do not require a live cluster, but the modules, chart, examples, Argo CD Application surface, and Gitea backend integration model the cluster contracts described in `docs/`.
