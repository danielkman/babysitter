...
function makeRun(name, workspaceRef, phase = 'Succeeded', extra = {}) {
  const run = createResource('AgentDispatchRun', { name, namespace: 'kradle-org-default' }, {
...
test('launchCodespace returns valid pod spec with PVC mount', () => {
  ...
  assert.ok(volumes.some((v) => v.persistentVolumeClaim?.claimName === 'kradle-ws-ws-dev'), 'PVC should be mounted');
...
test('launchCodespace pod spec has correct env vars', () => {
  ...
  assert.equal(envMap.KRADLE_WORKSPACE, 'ws-env');
  assert.equal(envMap.KRADLE_ORG, 'default');
