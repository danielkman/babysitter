...
function liveSnapshot() {
  return {
    source: 'kubernetes',
    namespace: 'kradle-staging',
    kubectl: { available: true, context: 'aks-kradle-staging', errors: [] },
    apiService: { metadata: { name: 'v1alpha1.kradle.a5c.ai' } },
    crds: [{ metadata: { name: 'repositories.kradle.a5c.ai' } }],
    resources: {
      Organization: [createResource('Organization', { name: 'default', namespace: 'kradle-system' }, { slug: 'default', namespaceName: 'kradle-org-default', displayName: 'Default org' })],
      Repository: [createResource('Repository', { name: 'test2', namespace: 'kradle-org-default' }, { organizationRef: 'default', visibility: 'internal', defaultBranch: 'main' })]
    },
    events: [],
    permissions: [],
    storage: {},
...
test('fetchControllerUiModel falls back to local snapshot when remote controller returns degraded empty data', async () => {
  ...
  const model = await fetchControllerUiModel({
    controllerUrl: 'http://kradle-api.kradle-staging.svc.cluster.local',
    organization: 'default',
    useCache: false,
