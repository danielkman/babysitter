...
function makeExecution(ruleName, eventUid, phase = 'Dispatched') {
  const exec = createResource('AgentTriggerExecution', { name: `exec-${ruleName}-1`, namespace: 'kradle-org-default' }, {
    organizationRef: 'default',
...
test('processEvent: dispatches matching rules via dispatch controller', async () => {
  ...
  const result = await controller.processEvent({ event, resources, namespace: 'kradle-org-default', organizationRef: 'default' });
