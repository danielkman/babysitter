...
function makeWorkspace(name, opts = {}) {
  const ws = createResource('KradleWorkspace', { name, namespace: opts.namespace || 'default' }, {
    ...
    pvcName: opts.pvcName || `kradle-ws-${name}`,
  });
  ws.status = {
...
test('createWorkspace generates valid PVC manifest with correct labels and capacity', () => {
  ...
  assert.equal(result.pvcManifest.metadata.name, 'kradle-ws-my-ws');
  assert.equal(result.pvcManifest.metadata.labels['kradle.a5c.ai/workspace'], 'my-ws');
  assert.equal(result.pvcManifest.metadata.labels['kradle.a5c.ai/org'], 'acme');
...
test('deleteWorkspace generates PVC delete manifest', () => {
  ...
  const result = ctrl.deleteWorkspace({ name: 'del-ws', resources: { KradleWorkspace: [ws] } });

  assert.equal(result.error, false);
  assert.equal(result.workspace.status.phase, 'Terminating');
  assert.ok(result.pvcDeleteManifest);
  assert.equal(result.pvcDeleteManifest.metadata.name, 'kradle-ws-del-ws');
...
test('getWorkspaceStatus returns Pending for new workspace', () => {
  ...
  const result = ctrl.getWorkspaceStatus({ name: 'new-ws', resources: { KradleWorkspace: [ws] } });
...
test('getMountSpec returns valid K8s volume and volumeMount', () => {
  const ctrl = createAgentWorkspaceController();
  const ws = makeWorkspace('mount-ws', { pvcName: 'kradle-ws-mount-ws' });
...
test('getMountSpec uses correct PVC claim name', () => {
  const ctrl = createAgentWorkspaceController();
  const ws = makeWorkspace('pvc-test', { pvcName: 'kradle-ws-pvc-test' });

  const result = ctrl.getMountSpec({ workspace: ws });

  assert.equal(result.error, false);
  assert.equal(result.volume.persistentVolumeClaim.claimName, 'kradle-ws-pvc-test');
});
...
test('findReusableWorkspace returns matching Ready workspace', () => {
  ...
  const result = ctrl.findReusableWorkspace({
    ...
    repository: 'https://github.com/acme/app.git',
    branch: 'main',
    resources: { KradleWorkspace: [ws1, ws2] },
  });
