...
function createFakeGateway(overrides = {}) {
  ...
  return {
    ...
    gateway: {
      ...
      resourceDefinitions: [],
      async snapshot() { return { source: 'kubernetes', namespace: 'kradle-system', resources: {}, commands: [], events: [], permissions: [], storage: {} }; },
...
async function makeController(overrides = {}, controllerOptions = {}) {
  ...
  const { createKradleApiController } = await import('../src/api-controller.js');
  const controller = createKradleApiController({ resourceGateway: gateway, ...controllerOptions });
...
describe('Slice 1.1 — Org namespace derivation', () => {
  test('orgNamespaceName derives kradle-org-<slug> from org slug', () => {
    return import('../src/org-scoping.js').then(({ orgNamespaceName: scopedFn }) => {
      assert.equal(scopedFn('a5c-ai'), 'kradle-org-a5c-ai');
      assert.equal(scopedFn('my-org'), 'kradle-org-my-org');
      assert.equal(scopedFn('default'), 'kradle-org-default');
    });
  });
});
...
describe('Slice 1.1 — applyResource in org scope', () => {
  test('applyResourceForOrg creates resource in the correct org namespace', async () => {
    ...
    assert.equal(
      ...
      'kradle-org-a5c-ai',
      'resource must be placed in the org namespace kradle-org-a5c-ai'
    );
    assert.equal(calls.applied.length, 1);
    assert.equal(calls.applied[0].metadata.namespace, 'kradle-org-a5c-ai');
  });
});

// ---------------------------------------------------------------------------
// Test 3 — Cross-org reference denial (kradle-org-* namespace mismatch)
// ---------------------------------------------------------------------------
...
describe('Slice 1.1 — Org-scoped resource listing', () => {
  test('listResource with org filter returns only resources from that org namespace', async () => {
    const repoOrgA = createResource(
      'Repository',
      { name: 'repo-alpha', namespace: 'kradle-org-org-a' },
      { organizationRef: 'org-a', visibility: 'internal' }
    );
    const repoOrgB = createResource(
      'Repository',
      { name: 'repo-beta', namespace: 'kradle-org-org-b' },
...
describe('Slice 1.1 — Org-scoped audit event', () => {
  test('applyResource emits an audit event with org context after a successful apply', async () => {
    ...
    const resource = createResource(
      'Repository',
      { name: 'audited-repo', namespace: 'kradle-org-a5c-ai' },
    ...
    assert.equal(event.namespace, 'kradle-org-a5c-ai', 'event.namespace must be the org namespace');
...
describe('Slice 1.1 — deleteResourceForOrg', () => {
  test('deleteResourceForOrg deletes resource in the correct org namespace', async () => {
    const orgResource = {
      resource: createResource('Repository', { name: 'my-repo', namespace: 'kradle-org-org-a' }, {
        organizationRef: 'org-a',
        visibility: 'internal'
      })
    };
...
describe('Slice 1.1 — deleteResourceForOrg cross-org denial', () => {
  test('deleteResourceForOrg rejects when resource belongs to a different org', async () => {
    const foreignResource = {
      resource: createResource('Repository', { name: 'foreign-repo', namespace: 'kradle-org-org-b' }, {
...
describe('Slice 1.1 — getResourceForOrg cross-org denial', () => {
  test('getResourceForOrg rejects when resource belongs to a different org', async () => {
    const foreignResource = {
      resource: createResource('Repository', { name: 'stolen-repo', namespace: 'kradle-org-org-b' }, {
        organizationRef: 'org-b',
        visibility: 'internal'
      })
    };
...
describe('Slice 1.1 — Audit callback error resilience', () => {
  test('audit callback error does not crash apply operations', async () => {
    ...
    const resource = createResource('Repository', { name: 'resilient-repo', namespace: 'kradle-org-a5c-ai' }, {
      organizationRef: 'a5c-ai',
      visibility: 'internal'
    });
...
describe('Slice 1.1 — applyResourceForOrg slug normalization', () => {
  test('applyResourceForOrg with un-normalized slug ("Org-A") normalizes the stored organizationRef to "org-a"', async () => {
    ...
    assert.equal(result.resource.metadata.namespace, 'kradle-org-org-a',
...
describe('Slice 1.1 — applyResource auto-assigns namespace from organizationRef', () => {
  test('applyResource with organizationRef but no namespace auto-assigns namespace', async () => {
    ...
    assert.equal(calls.applied.length, 1);
    assert.equal(calls.applied[0].metadata.namespace, 'kradle-org-org-a',
...
describe('Slice 1.1 — listResourceForOrg multi-org filtering', () => {
  test('listResourceForOrg correctly filters across multiple orgs', async () => {
    const repoA1 = createResource('Repository', { name: 'a-repo-1', namespace: 'kradle-org-org-a' }, { organizationRef: 'org-a', visibility: 'internal' });
    const repoA2 = createResource('Repository', { name: 'a-repo-2', namespace: 'kradle-org-org-a' }, { organizationRef: 'org-a', visibility: 'internal' });
    const repoB1 = createResource('Repository', { name: 'b-repo-1', namespace: 'kradle-org-org-b' }, { organizationRef: 'org-b', visibility: 'private' });
    const repoC1 = createResource('Repository', { name: 'c-repo-1', namespace: 'kradle-org-org-c' }, { organizationRef: 'org-c', visibility: 'public' });
...
// (Regression: resources targeting system namespaces like kradle-system
//  with any organizationRef must also be blocked.)
// ---------------------------------------------------------------------------
describe('Slice 1.1 — Cross-org denial for system namespaces', () => {
  test('applyResource rejects resource targeting kradle-system with an organizationRef', async () => {
    const { controller, calls } = await makeController();
