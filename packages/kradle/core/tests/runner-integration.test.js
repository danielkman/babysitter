...
test('createRunner with workspace: generatePodSpec includes workspace volume mount', () => {
  ...
  const vol = podSpec.spec.volumes.find(
    (v) => v.persistentVolumeClaim?.claimName === 'kradle-ws-run-42'
  );
...
test('createRunner with workspace: pod spec has correct org label', () => {
  ...
  assert.equal(podSpec.metadata.labels['kradle.a5c.ai/org'], 'my-org');
...
test('generatePodSpec with workspace sets KRADLE_WORKSPACE_PATH env var', () => {
  const ctrl = createRunnerController();
  ...
  const wsPathEnv = env.find((e) => e.name === 'KRADLE_WORKSPACE_PATH');
  assert.ok(wsPathEnv, 'KRADLE_WORKSPACE_PATH env var must be present');
