...
test('applyOwnershipMode allows write in bidirectional mode', () => {
  ...
  assert.equal(result.allowed, true, 'bidirectional mode must allow kradle writes');
...
test('applyOwnershipMode blocks write in external-owned mode', () => {
  ...
  assert.equal(result.allowed, false, 'external-owned mode must block kradle writes');
...
test('applyOwnershipMode allows write in kradle-owned mode', () => {
  const controller = createSyncController();
  const result = controller.applyOwnershipMode({
    ownershipMode: 'kradle-owned',
    operation: 'write',
    origin: 'kradle'
  });
  assert.equal(result.allowed, true, 'kradle-owned mode must allow kradle writes');
});
