...
function makeGateway(overrides = {}) {
  ...
  return {
    ...
    resourceDefinitions: {},
    async snapshot() { return { resources: {}, namespace: 'kradle-system' }; },
...
function makeController(gatewayOverrides = {}) {
  return createKradleApiController({ resourceGateway: makeGateway(gatewayOverrides) });
}
...
test('syncExternalBinding normalizes the provided envelope into a K8s-style resource', async () => {
  ...
  assert.equal(result.resource.metadata.namespace, 'kradle-org-acme', 'namespace must be preserved');
