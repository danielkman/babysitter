...
test('runKubectlAsync resolves with ok=true for a succeeding command', async () => {
  const result = await runKubectlAsync(['version', '--client=true', '-o', 'json'], {
    kubectl: process.env.KRADLE_KUBECTL || 'kubectl',
    timeoutMs: 5000,
    allowFailure: true
  });
...
test('getControllerSnapshotAsync uses in-cluster service account instead of kubeconfig current-context', async () => {
  const tempDir = await mkdtemp(path.join(tmpdir(), 'kradle-sa-'));
  ...
  try {
    ...
    const snapshot = await getControllerSnapshotAsync({
      ...
      env: {
        ...
        KRADLE_SERVICE_ACCOUNT_DIR: tempDir
      }
    });
...
test('full async snapshot lists known Kradle CRDs even when CRD discovery is empty', async () => {
  const tempDir = await mkdtemp(path.join(tmpdir(), 'kradle-known-crds-'));
  const fakePath = path.join(tempDir, 'fake-kubectl.cjs');
  const fakeScript = `
const args = process.argv.slice(1);
const joined = args.join(' ');
const has = (value) => args.includes(value) || joined.includes(value);
if (has('current-context')) { console.log('kind-kradle'); process.exit(0); }
if (has('version')) { console.log(JSON.stringify({ clientVersion: { gitVersion: 'v1.test' } })); process.exit(0); }
if (has('apiservice')) { console.log(JSON.stringify({ metadata: { name: 'v1alpha1.kradle.a5c.ai' } })); process.exit(0); }
if (has('crd')) { console.error('crd discovery unavailable'); process.exit(1); }
if (has('runnerpools.kradle.a5c.ai')) {
  console.log(JSON.stringify({ items: [{ apiVersion: 'kradle.a5c.ai/v1alpha1', kind: 'RunnerPool', metadata: { name: 'default', namespace: 'kradle-org-default', labels: { 'kradle.a5c.ai/org': 'default' } }, spec: { organizationRef: 'default', image: 'ubuntu:24.04' } }] }));
  process.exit(0);
}
...
`;
  ...
  try {
    ...
    const snapshot = await getControllerSnapshotAsync({
      ...
      timeoutMs: 1000,
      env: { KRADLE_ORG: 'default', NODE_OPTIONS: `--require ${fakePath}` }
    });
