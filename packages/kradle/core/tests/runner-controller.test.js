...
function makePool(overrides = {}) {
  return {
    ...
    metadata: {
      ...
      namespace: overrides.namespace || 'kradle-org-default',
...
test('generatePodSpec returns valid K8s Pod with workspace volume mount', () => {
  ...
  const workspace = { runRef: 'run-99', mountPath: '/workspace', pvcName: 'kradle-ws-run-99' };
  ...
  assert.ok(podSpec.spec.volumes.some((v) => v.persistentVolumeClaim?.claimName === 'kradle-ws-run-99'));
...
test('generatePodSpec injects required env vars', () => {
  ...
  const podSpec = ctrl.generatePodSpec({ runnerId: 'runner-xyz', pool }, { runRef: 'run-7', mountPath: '/workspace', pvcName: 'kradle-ws-run-7' });
  const env = podSpec.spec.containers[0].env;
  assert.ok(env.some((e) => e.name === 'KRADLE_ORG' && e.value === 'myorg'));
  assert.ok(env.some((e) => e.name === 'KRADLE_RUN_ID'));
  assert.ok(env.some((e) => e.name === 'KRADLE_WORKSPACE_PATH'));
});
...
test('generatePodSpec sets pool and org labels on Pod', () => {
  ...
  const podSpec = ctrl.generatePodSpec({ runnerId: 'r', pool }, null);
  assert.equal(podSpec.metadata.labels['kradle.a5c.ai/pool'], 'my-pool');
  assert.equal(podSpec.metadata.labels['kradle.a5c.ai/org'], 'acme');
});
