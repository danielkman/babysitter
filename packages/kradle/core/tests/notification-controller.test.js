...
test('createNotification with KradleWorkspace claimed → type workspace-ready, severity info', () => {
  const ctrl = makeController();
  const notif = ctrl.createNotification({ type: 'KradleWorkspace', claimed: true, name: 'ws-1', claimedBy: 'run-abc', org: 'test-org' });
