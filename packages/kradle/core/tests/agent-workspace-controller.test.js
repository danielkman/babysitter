...
function makeWorkspace(name, repository, runRef, phase = 'InUse', extra = {}) {
  const workspace = createResource('KradleWorkspace', { name, namespace: 'kradle-org-default' }, {
    ...
    pvcName: extra.pvcName || `kradle-ws-${name}`,
  });
...
function makeRuntime(name, workspaceRef, status = 'provisioning') {
  const runtime = createResource('KradleWorkspaceRuntime', { name, namespace: 'kradle-org-default' }, {
    organizationRef: 'default',
    workspaceRef,
    status
  });
...
test('provisionWorkspace creates KradleWorkspace + KradleWorkspaceRuntime', () => {
  ...
  assert.equal(result.workspace.kind, 'KradleWorkspace');
  ...
  assert.equal(result.runtime.kind, 'KradleWorkspaceRuntime');
...
test('archiveWorkspace sets phase=Archived', () => {
  ...
  const result = controller.archiveWorkspace({
    ...
    resources: { KradleWorkspace: [existing] }
  });
...
test('recoverWorkspace sets phase=Active', () => {
  ...
  const result = controller.recoverWorkspace({
    workspaceName: 'ws-2',
    resources: { KradleWorkspace: [archived] }
  });
...
test('listWorkspacesForRepo filters by repository', () => {
  ...
  const result = controller.listWorkspacesForRepo({
    ...
    resources: { KradleWorkspace: [ws1, ws2, ws3] }
  });
...
test('recoverWorkspace on non-archived returns error', () => {
  ...
  const result = controller.recoverWorkspace({
    ...
    resources: { KradleWorkspace: [active] }
  });
