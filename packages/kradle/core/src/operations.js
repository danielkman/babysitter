...
import { createKradleComponentCatalog, createKradleLifecycleSnapshot } from './component-catalog.js';
...
export function generateInstallManifests({ namespace = 'kradle-system' } = {}) {
  const manifests = [
    { apiVersion: 'apiregistration.k8s.io/v1', kind: 'APIService', metadata: { name: 'v1alpha1.kradle.a5c.ai' }, spec: { group: 'kradle.a5c.ai', version: 'v1alpha1', service: { namespace, name: 'kradle-api' } } },
    { apiVersion: 'apps/v1', kind: 'Deployment', metadata: { name: 'kradle-api', namespace }, spec: { replicas: 2, template: { spec: { containers: [{ name: 'api', image: 'kradle/api:dev' }] } } } },
    { apiVersion: 'apps/v1', kind: 'Deployment', metadata: { name: 'kradle-gitea', namespace, labels: { 'app.kubernetes.io/component': 'gitea-backend' } }, spec: { replicas: 1, template: { spec: { containers: [{ name: 'gitea', image: 'gitea/gitea:1.22-rootless' }] } } } },
    { apiVersion: 'argoproj.io/v1alpha1', kind: 'Application', metadata: { name: 'kradle', namespace: 'argocd' }, spec: { source: { repoURL: 'https://gitea-http.kradle-system.svc.cluster.local/kradle/platform-config.git', path: 'charts/kradle', targetRevision: 'main' }, destination: { namespace, server: 'https://kubernetes.default.svc' }, syncPolicy: { automated: { prune: true, selfHeal: true } } } },
    { apiVersion: 'apps/v1', kind: 'Deployment', metadata: { name: 'kradle-web', namespace }, spec: { replicas: 1 } }
  ];
...
export function createKradleMvpDemo() {
  const platformEngineer = mapOidcIdentity({ subject: 'user:platform', email: 'platform@example.com', groups: ['kradle:platform-engineers'] });
  const developer = mapOidcIdentity({ subject: 'user:dev', email: 'dev@example.com', groups: ['kradle:developers'] });
  const repoAdmin = mapOidcIdentity({ subject: 'user:admin', email: 'admin@example.com', groups: ['kradle:repo-admins'] });
  const organizationRef = 'default';
  const namespace = 'kradle-org-default';
  ...
  const repository = git.createRepository({ name: 'kradle-demo', namespace, organizationRef }, repoAdmin);
  ...
  const subscription = webhooks.subscribe({ name: 'chatops', namespace, organizationRef, url: 'https://hooks.example.test/kradle', events: ['pullrequest.created'] }, repoAdmin);
  const pullRequest = controlPlane.create(createResource('PullRequest', { name: 'pr-1', namespace, labels: { repository: 'kradle-demo' } }, { organizationRef, repository: 'kradle-demo', sourceRef: 'refs/heads/feature', targetRef: 'refs/heads/main', title: 'Add Kubernetes-native forge smoke path' }, { phase: 'Open' }), developer);
  const pipelineRun = runners.startPipeline({ name: 'pipeline-pr-1', namespace, organizationRef, repository: 'kradle-demo', ref: 'refs/pull/1/head', actor: developer, fork: false }, developer);
  const delivery = webhooks.deliver({ subscriptionName: 'chatops', namespace, organizationRef, eventType: 'pullrequest.created', payload: { pullRequest: pullRequest.metadata.name, repository: 'kradle-demo' } }, repoAdmin);
  ...
  demo.components = createKradleComponentCatalog(demo);
  demo.lifecycle = createKradleLifecycleSnapshot(demo);
  return demo;
}

export function runSmokeAssertions(demo = createKradleMvpDemo()) {
  ...
  const assertions = [
    ...
    ['Gitea receive path is ready', demo.git.route('kradle-demo').backend === 'gitea' && demo.git.route('kradle-demo').receivePackReady],
