...
import { createKradleRuntime } from './runtime.js';
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/healthz') {
        if (url.searchParams.get('quick') === '1' || request.headers['x-readiness-probe']) {
          return send(response, 200, { ok: true, project: 'Kradle', status: 'alive' });
        }
        const probes = await collectKradleHealthProbes({ timeoutMs: 3000, eventBus: globalEventBus, ...healthProbeOptions });
      ...
      if (orgResourceCollectionMatch) {
        ...
        const scopedController = createKradleApiController({ namespace: orgNamespaceName(org) });
      ...
      if (orgResourceMatch) {
        const scopedController = createKradleApiController({ namespace: orgNamespaceName(orgResourceMatch[1]) });
      ...
      if (orgRepositoryMatch) {
        const scopedController = createKradleApiController({ namespace: orgNamespaceName(orgRepositoryMatch[1]) });
...
function scopeResource(resource, org) {
  ...
  return {
    ...resource,
    metadata: { ...(resource.metadata || {}), namespace, labels: { ...(resource.metadata?.labels || {}), 'kradle.a5c.ai/org': org, 'kradle.a5c.ai/namespace': namespace } },
