...
const CONTROLLER_REQUEST_TIMEOUT_MS = Number(process.env.KRADLE_CONTROLLER_REQUEST_TIMEOUT_MS || 5_000);

export async function fetchControllerUiModel({ controllerUrl = process.env.KRADLE_CONTROLLER_URL, fetchImpl = globalThis.fetch, controller = null, organization = process.env.KRADLE_ORG || null, localFallback = true, requestTimeoutMs = CONTROLLER_REQUEST_TIMEOUT_MS, useCache = true, swrOptions = {}, fallbackSnapshot = getControllerSnapshotAsync } = {}) {
  const revalidateFn = async () => {
    ...
    if (!localFallback) return unavailableControllerModel('KRADLE_CONTROLLER_URL is not configured', organization);
...
async function fallbackControllerModel({ controller = null, connectionError = null, organization = null, fallbackSnapshot = getControllerSnapshotAsync } = {}) {
  try {
    ...
    return createControllerUiModel({
      source: 'kubernetes',
      namespace: process.env.KRADLE_NAMESPACE || 'kradle-system',
