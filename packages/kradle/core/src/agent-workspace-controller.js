...
export function createAgentWorkspaceController() {
  return {
    ...
    deleteWorkspace({ name, namespace = 'default', resources = {} }) {
      ...
      const workspaces = resources.KradleWorkspace || [];
      const workspace = workspaces.find((w) => w.metadata?.name === name);
      if (!workspace) {
        return { error: true, reason: 'not-found', message: `KradleWorkspace not found: ${name}` };
      }

      const pvcName = workspace.spec?.pvcName || `kradle-ws-${name}`;
    ...
    claimWorkspace({ name, runRef, resources = {} }) {
      ...
      if (workspace.status?.phase === 'InUse') {
        return { error: true, reason: 'already-in-use', message: `KradleWorkspace ${name} is already in use by ${workspace.status.runRef}` };
      }
    ...
    releaseWorkspace({ name, resources = {} }) {
      ...
      if (workspace.status?.phase !== 'InUse') {
        return { error: true, reason: 'not-in-use', message: `KradleWorkspace ${name} is not in use (current phase: ${workspace.status?.phase || 'Unknown'})` };
      }
    ...
    provisionWorkspace({ repository, ref, branch, dispatchRun, policy, namespace = 'default', organizationRef = 'default' }) {
      ...
      const runtime = createResource('KradleWorkspaceRuntime', { name: runtimeName, namespace }, {
        organizationRef,
    ...
    archiveWorkspace({ workspaceName, reason, resources = {} }) {
      ...
      if (!workspace) {
        return { error: true, reason: 'not-found', message: `KradleWorkspace not found: ${workspaceName}` };
      }
    ...
    recoverWorkspace({ workspaceName, resources = {} }) {
      ...
      if (workspace.status?.phase !== 'Archived') {
        return { error: true, reason: 'not-archived', message: `KradleWorkspace ${workspaceName} is not archived (current phase: ${workspace.status?.phase || 'Unknown'})` };
      }
    ...
    launchCodespace(workspace, options = {}) {
      ...
      const pvcName = workspace.spec?.pvcName || `kradle-ws-${wsName}`;
      ...
      const podSpec = {
        ...
        metadata: {
          ...
          namespace,
          labels: {
            'kradle.a5c.ai/workspace': wsName,
            'kradle.a5c.ai/org': orgRef,
            'kradle.a5c.ai/component': 'codespace',
          },
        },
        spec: {
          containers: [
            {
              ...
              env: [
                { name: 'KRADLE_WORKSPACE', value: wsName },
                { name: 'KRADLE_ORG', value: orgRef },
                { name: 'GIT_AUTHOR_NAME', value: options.gitAuthorName || 'kradle-agent' },
                { name: 'GIT_AUTHOR_EMAIL', value: options.gitAuthorEmail || `agent@${orgRef}.kradle.local` },
              ],
              volumeMounts: [
