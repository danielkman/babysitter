...
export function createAuthProviderConfig(env = process.env) {
  const githubEnabled = env.KRADLE_AUTH_GITHUB_ENABLED !== 'false';
  const ssoEnabled = env.KRADLE_AUTH_SSO_ENABLED === 'true';
  return {
    ...
    providers: {
      github: {
        ...
        clientId: env.KRADLE_AUTH_GITHUB_CLIENT_ID || '',
        clientSecret: env.KRADLE_AUTH_GITHUB_CLIENT_SECRET || '',
        clientSecretConfigured: Boolean(env.KRADLE_AUTH_GITHUB_CLIENT_SECRET),
        authorizationUrl: env.KRADLE_AUTH_GITHUB_AUTHORIZATION_URL || 'https://github.com/login/oauth/authorize',
        tokenUrl: env.KRADLE_AUTH_GITHUB_TOKEN_URL || 'https://github.com/login/oauth/access_token',
        userInfoUrl: env.KRADLE_AUTH_GITHUB_USERINFO_URL || 'https://api.github.com/user',
        scopes: env.KRADLE_AUTH_GITHUB_SCOPES || defaultScopes.github
      },
      sso: {
        id: 'sso',
        label: env.KRADLE_AUTH_SSO_PROVIDER_NAME || 'Workspace SSO',
        type: 'oidc',
        enabled: ssoEnabled,
        issuerUrl: env.KRADLE_AUTH_SSO_ISSUER_URL || '',
        clientId: env.KRADLE_AUTH_SSO_CLIENT_ID || '',
        clientSecret: env.KRADLE_AUTH_SSO_CLIENT_SECRET || '',
        clientSecretConfigured: Boolean(env.KRADLE_AUTH_SSO_CLIENT_SECRET),
        authorizationUrl: env.KRADLE_AUTH_SSO_AUTHORIZATION_URL || '',
        tokenUrl: env.KRADLE_AUTH_SSO_TOKEN_URL || '',
        userInfoUrl: env.KRADLE_AUTH_SSO_USERINFO_URL || '',
        scopes: env.KRADLE_AUTH_SSO_SCOPES || defaultScopes.sso
      }
    }
  };
}
...
export function profileFromDelegatedHeaders(headers, config = createAuthProviderConfig(), options = {}) {
  ...
  return { provider: 'delegated', subject: user, email, displayName: user, username: normalizeName(user), groups, teams: [], admin: groups.includes('kradle:platform-engineers') || groups.includes('kradle:repo-admins'), delegatedIdentitySource: headerUser ? 'proxy-header' : 'local-development' };
}
...
export async function registerLoginProfile({ controller, namespace, profile }) {
  ...
  const orgNamespace = namespace || `kradle-org-${org}`;
  const adminUsername = process.env.KRADLE_ADMIN_USERNAME || '';
  ...
  const mapped = mapLoginProfileToKradleIdentity({ ...profile, namespace: orgNamespace, organizationRef: org, admin: isBootstrapAdmin || profile.admin });
...
export function createInviteResource({ email, role = 'member', teams = [], invitedBy = 'admin', namespace = 'kradle-org-default', organizationRef = 'default', expiresInDays = 7 }) {
...
function delegatedLocalDevelopmentEnabled(env = process.env) {
  if (env.KRADLE_AUTH_DELEGATED_LOCAL_DEVELOPMENT === 'true') return true;
  if (env.KRADLE_AUTH_DELEGATED_LOCAL_DEVELOPMENT === 'false') return false;
