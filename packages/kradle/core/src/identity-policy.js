...
export function defaultAuthorizer() {
  return new RbacAuthorizer()
    ...
    .allow('kradle:developers', { verbs: ['create', 'update'], kinds: ['PullRequest', 'Issue', 'Review', 'Pipeline', 'Job'] })
    .allow('kradle:repo-admins', { verbs: ['create', 'update', 'delete'], kinds: ['Organization', 'User', 'Team', 'Invite', 'IdentityMapping', 'AuthProvider', 'Repository', 'SSHKey', 'RepositoryPermission', 'BranchProtection', 'RefPolicy', 'WebhookSubscription', 'WebhookDelivery', 'View', 'Selector', 'PullRequest', 'Issue', 'Review', 'Pipeline', 'Job'] })
    .allow('kradle:platform-engineers', { verbs: ['*'], kinds: ['*'] });
}
...
export function serviceAccountForJob({ namespace = 'default', repository, pipeline, trustTier = 'trusted' }) {
  return {
    name: `kradle-job-${pipeline}`,
    namespace,
    groups: ['system:serviceaccounts', `system:serviceaccounts:${namespace}`, 'kradle:ci-jobs'],
    trustTier,
