...
export const RESOURCE_DEFINITIONS = Object.freeze({
  ...
  IdentityMapping: { storage: 'etcd', context: 'identity', plural: 'identitymappings', purpose: 'Mapping between Kradle users, sign-in subjects, workspace identities, and repository accounts', requiredSpec: ['organizationRef', 'user', 'provider', 'subject'] },
  ...
  KradleGeneratedView: { storage: 'etcd', context: 'web-ui', plural: 'kradlegeneratedviews', purpose: 'Durable, named HTML widget/view/screen generated by the assistant, reusable and viewable later', requiredSpec: ['organizationRef', 'content'] },
  ...
  AgentAppearance: { storage: 'etcd', context: 'agents', plural: 'agentappearances', purpose: 'Visual identity for agent profile images, theme colors, badges, avatar generation settings, and realtime avatar model (renderer, model URL, viseme set, default mood/view)', requiredSpec: ['organizationRef'] },
  ...
  AgentProcessTemplate: { storage: 'etcd', context: 'agents', plural: 'agentprocesstemplates', purpose: 'Per-taskKind process phase pipeline (ordered phase labels) that drives the run/process view for a given task kind', requiredSpec: ['organizationRef', 'taskKind', 'phases'] },
  ...
  KradleWorkspacePolicy: { storage: 'etcd', context: 'agents', plural: 'kradleworkspacepolicies', purpose: 'Git worktree provisioning, cleanup, retention, and trust tier policies', requiredSpec: ['organizationRef', 'mode', 'retentionPolicy'] },
  ...
  AgentSession: { storage: 'postgres', context: 'agents', plural: 'agentsessions', purpose: 'Kradle projection of Agent Adapter chat/session with lifecycle state', requiredSpec: ['organizationRef', 'agentMuxSessionId', 'dispatchRun'] },
  ...
  KradleArtifact: { storage: 'postgres', context: 'agents', plural: 'kradleartifacts', purpose: 'Durable agent output with kind, digest, and retention policy', requiredSpec: ['organizationRef', 'dispatchRun', 'kind', 'digest'] },
  ...
  KradleWorkspace: { storage: 'etcd', context: 'workspaces', plural: 'kradleworkspaces', purpose: 'Volume-backed git workspace with PVC lifecycle, repo binding, and runner mount spec', requiredSpec: ['organizationRef', 'repository', 'volumeSpec'] },
  ...
  KradleProject: { storage: 'etcd', context: 'agents', plural: 'kradleprojects', purpose: 'Org project grouping issues, linked repositories, kanban board config, default workflow, and backend sync refs', requiredSpec: ['organizationRef', 'displayName'] },
  AgentGatewayConfig: { storage: 'etcd', context: 'agents', plural: 'agentgatewayconfigs', purpose: 'Runtime Agent Adapter gateway connection settings with URL, auth, reconnect policy, and feature flags', requiredSpec: ['organizationRef', 'gatewayUrl'] },
  ...
  KradleWorkspaceRuntime: { storage: 'postgres', context: 'agents', plural: 'kradleworkspaceruntimes', purpose: 'Workspace runtime surface state with cwd, environment variables, process status, and preview URL', requiredSpec: ['organizationRef', 'workspaceRef', 'status'] },
  ...
  AgentMemoryAssociation: { storage: 'etcd', context: 'agents', plural: 'agentmemoryassociations', purpose: 'Bridge record linking memory content to Kradle resources by relationship type', requiredSpec: ['organizationRef', 'memoryRef', 'targetRef', 'relationship'] },
  ...
  ExternalObjectLink: { storage: 'postgres', context: 'external-backends', plural: 'externalobjectlinks', purpose: 'Stable mapping between a Kradle local resource and its external backend counterpart by external ID', requiredSpec: ['organizationRef', 'providerRef', 'externalId', 'localRef'] },
  ...
  KradleInferenceService: { storage: 'etcd', context: 'inference', plural: 'kradleinferenceservices', purpose: 'KServe InferenceService wrapper for model inference with endpoint discovery, model catalog, protocol config, and provider integration', requiredSpec: ['organizationRef', 'modelFormat', 'storageUri'] },
  KradleServingRuntime: { storage: 'etcd', context: 'inference', plural: 'kradleservingruntimes', purpose: 'KServe ServingRuntime wrapper defining container image, supported model formats, resource limits, and scheduling policy', requiredSpec: ['organizationRef', 'supportedModelFormats', 'containers'] },
  ...
  KradleModelRoute: { storage: 'etcd', context: 'inference', plural: 'kradlemodelroutes', purpose: 'Unified model routing through Envoy AI Gateway mapping logical model names to internal KServe services or external cloud LLM endpoints', requiredSpec: ['organizationRef', 'modelName', 'routeType'] },
  KradleVirtualModel: { storage: 'etcd', context: 'inference', plural: 'kradlevirtualmodels', purpose: 'Programmable model abstraction with declarative routing rules, JS hooks for request/response transformation, session lifecycle management, and observability injection', requiredSpec: ['organizationRef', 'modelName', 'routes'] },
  EvalSuite: { storage: 'etcd', context: 'agents', plural: 'evalsuites', purpose: 'Named evaluation suite with prompt test cases, scoring criteria, and model comparison config', requiredSpec: ['organizationRef', 'displayName'] },
  EvalCase: { storage: 'postgres', context: 'agents', plural: 'evalcases', purpose: 'Individual test case with input prompt, expected output, scoring rubric, and tags', requiredSpec: ['organizationRef', 'suiteRef', 'input'] },
  EvalRun: { storage: 'postgres', context: 'agents', plural: 'evalruns', purpose: 'Execution record of an eval suite against one or more models with aggregate scores', requiredSpec: ['organizationRef', 'suiteRef', 'models'] },
  EvalResult: { storage: 'postgres', context: 'agents', plural: 'evalresults', purpose: 'Per-case result within an eval run with model output, score, latency, and token usage', requiredSpec: ['organizationRef', 'runRef', 'caseRef', 'modelRef'] },
  Dataset: { storage: 'etcd', context: 'agents', plural: 'datasets', purpose: 'Named dataset of structured records for training, evaluation, or reference with schema and versioning', requiredSpec: ['organizationRef', 'displayName', 'schema'] },
  DatasetVersion: { storage: 'postgres', context: 'agents', plural: 'datasetversions', purpose: 'Immutable snapshot of a dataset at a point in time with record count and storage reference', requiredSpec: ['organizationRef', 'datasetRef', 'version'] },
  DatasetRecord: { storage: 'postgres', context: 'agents', plural: 'datasetrecords', purpose: 'Individual record within a dataset version with structured fields matching the dataset schema', requiredSpec: ['organizationRef', 'versionRef', 'data'] },
  Guardrail: { storage: 'etcd', context: 'agents', plural: 'guardrails', purpose: 'Safety guardrail rule with trigger conditions, actions (block/warn/log), and scope (per-stack or global)', requiredSpec: ['organizationRef', 'displayName', 'ruleType'] },
  GuardrailEvent: { storage: 'postgres', context: 'agents', plural: 'guardrailevents', purpose: 'Record of a guardrail activation with trigger context, action taken, and affected agent/session', requiredSpec: ['organizationRef', 'guardrailRef', 'eventType', 'action'] }
});
...
export function resourceDefinitionForKind(kind) {
  ...
  if (!definition) throw new Error(`Unknown Kradle resource kind: ${kind}`);
...
export function createResource(kind, metadata, spec = {}, status = {}) {
  if (!ALL_KINDS.has(kind)) throw new Error(`Unknown Kradle resource kind: ${kind}`);
...
const FIELD_TYPE_RULES = {
  ...
  suiteRef: 'string', input: 'string', runRef: 'string', caseRef: 'string', modelRef: 'string',
...
export function toKubernetesList(kind, items) {
  return { apiVersion: 'kradle.a5c.ai/v1alpha1', kind: `${kind}List`, items: items.map(clone) };
}
...
export function createSelector({ name, namespace = 'kradle-org-default', organizationRef = 'default', labels = {}, query = '' }) {
  return createResource('Selector', { name, namespace }, { organizationRef, labels, query });
}

export function createView({ name, namespace = 'kradle-org-default', organizationRef = 'default', selector, columns = [], sort = [] }) {
