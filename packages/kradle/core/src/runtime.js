...
import { createInviteResource, createTeamResource, createAuthProviderResources, mapLoginProfileToKradleIdentity } from './auth.js';
...
export function createDefaultKradleUsers() {
  return {
    platformEngineer: mapOidcIdentity({ subject: 'user:platform', email: 'platform@example.com', groups: ['kradle:platform-engineers'] }),
    developer: mapOidcIdentity({ subject: 'user:dev', email: 'dev@example.com', groups: ['kradle:developers'] }),
    repoAdmin: mapOidcIdentity({ subject: 'user:admin', email: 'admin@example.com', groups: ['kradle:repo-admins'] })
  };
}

export class KradleRuntime {
  constructor({ organizationRef = DEFAULT_ORG, namespace = DEFAULT_NAMESPACE, users = createDefaultKradleUsers(), controlPlane, git, runners, webhooks } = {}) {
  ...
  static fromSnapshot(snapshot, options = {}) {
    const runtime = new KradleRuntime(options);
    runtime.importSnapshot(snapshot);
    return runtime;
  }

  bootstrap({ repository = 'kradle-demo', webhookUrl = 'https://hooks.example.test/kradle' } = {}) {
    ...
    const identity = mapLoginProfileToKradleIdentity({ provider: 'sso', subject: 'user:admin', email: 'admin@example.com', displayName: 'Admin', username: 'admin', teams: ['maintainers'], admin: true, namespace: this.namespace, organizationRef: this.organizationRef });
...
export function createKradleRuntime(options = {}) {
  const runtime = new KradleRuntime(options);
