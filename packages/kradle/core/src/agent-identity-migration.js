...
export function legacyAgentStackIdentityWarning(stack) {
  ...
  return `[kradle] AgentStack "${name}" has inline prompts or skills. Consider migrating to AgentPersona + AgentDefinition.`;
}
...
export function cleanAgentStackIdentity(stack) {
  ...
  cleaned.metadata.annotations['kradle.a5c.ai/identity-migrated'] = 'true';
  return cleaned;
}
