...
const DEFAULT_SERVICE_ACCOUNT = 'kradle-runner';
  ...
  return {
    ...
    generatePodSpec({ runnerId, pool }, workspace = null) {
      ...
      const envVars = [
        ...
        { name: 'KRADLE_WORKSPACE_PATH', value: workspace?.mountPath || '/workspace' }
      ];
      ...
      if (workspace) {
        const pvcName = workspace.pvcName || `kradle-ws-${runId}`;
        volumes.push({
          name: 'workspace',
      ...
      return {
        ...
        metadata: {
          ...
          labels: {
            ...
            'kradle.a5c.ai/pool': pool?.metadata?.name || 'unknown',
