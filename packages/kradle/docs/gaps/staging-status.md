...
Current state of `kradle-staging.a5c.ai` as of 2026-05-30.
...
- CRD resource CRUD — creating, editing, deleting AgentStacks, TriggerRules, KradleProjects, etc. all write to etcd via the Kubernetes controller
...
**Error:** `Assistant API key not configured. Set ANTHROPIC_API_KEY or KRADLE_ASSISTANT_API_KEY.`

The assistant runtime calls `callModel()` from kradle-sdk which delegates to the Anthropic API. Without the key:
