/**
 * CLI Commands Tests — tests for kradle CLI commands and MCP prompts/resources
 *
...
function createMockController(overrides = {}) {
  ...
  const stacks = [
    { kind: 'AgentStack', metadata: { name: 'review-bot', namespace: 'kradle-org-default' }, spec: { organizationRef: 'default', baseAgent: 'claude-code', adapterRef: 'github' }, status: { phase: 'Ready' } },
    { kind: 'AgentStack', metadata: { name: 'deploy-agent', namespace: 'kradle-org-default' }, spec: { organizationRef: 'default', baseAgent: 'claude-opus', adapterRef: 'k8s' }, status: { phase: 'Pending' } },
  ];
...
test('kradle status: kradle_snapshot returns org info with namespace and resources', async () => {
  ...
  assert.equal(data.namespace, 'kradle-org-default', 'snapshot must include namespace');
...
test('kradle status: snapshot resource counts are correct', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', { name: 'kradle_snapshot', arguments: {} }));
  const data = JSON.parse(resp.result.content[0].text);
...
test('kradle stacks: kradle_list_stacks returns list with name/adapter/phase', async () => {
  const server = createMcpServer({ controller: createMockController() });
...
test('kradle stacks: stack items have spec with baseAgent', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', { name: 'kradle_list_stacks', arguments: {} }));
  const data = JSON.parse(resp.result.content[0].text);
...
test('kradle dispatch --stack: kradle_dispatch_agent creates a run', async () => {
  const server = createMcpServer({ controller: createMockController() });
...
test('kradle dispatch --stack: dispatch result includes input context', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', {
    name: 'kradle_dispatch_agent',
...
test('kradle apply --file: kradle_apply_resource reads and applies resource', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resource = {
    apiVersion: 'kradle.a5c.ai/v1',
    kind: 'AgentStack',
    metadata: { name: 'new-stack', namespace: 'kradle-org-default' },
    spec: { organizationRef: 'default', baseAgent: 'claude-sonnet' },
  };
  const resp = await server.handleMessage(rpc('tools/call', {
    name: 'kradle_apply_resource',
    arguments: { resource },
  }));
  assert.ok(resp.result, 'must return a result');
  const data = JSON.parse(resp.result.content[0].text);
...
test('kradle get: kradle_get_resource returns resource as object', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', {
    name: 'kradle_get_resource',
...
test('kradle list: kradle_list_resources returns resources for kind', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', {
    name: 'kradle_list_resources',
...
test('kradle delete: kradle_delete_resource removes resource by kind and name', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('tools/call', {
    name: 'kradle_delete_resource',
...
test('kradle version: getVersion logic returns a version string', () => {
...
test('kradle help: commands map includes all expected commands', () => {
  // Verify the commands object structure expected by kradle.mjs cmdHelp()
  ...
  assert.ok(toolNames.includes('kradle_list_stacks'), 'stacks command backed by kradle_list_stacks tool');
  assert.ok(toolNames.includes('kradle_dispatch_agent'), 'dispatch command backed by kradle_dispatch_agent tool');
  assert.ok(toolNames.includes('kradle_apply_resource'), 'apply command backed by kradle_apply_resource tool');
  assert.ok(toolNames.includes('kradle_get_resource'), 'get command backed by kradle_get_resource tool');
  assert.ok(toolNames.includes('kradle_list_resources'), 'list command backed by kradle_list_resources tool');
  assert.ok(toolNames.includes('kradle_delete_resource'), 'delete command backed by kradle_delete_resource tool');
...
test('MCP prompts/list returns all 3 prompts', async () => {
  ...
  assert.ok(names.includes('kradle_workspace_setup'), 'must include workspace setup prompt');
  assert.ok(names.includes('kradle_stack_config'), 'must include stack config prompt');
  assert.ok(names.includes('kradle_troubleshoot'), 'must include troubleshoot prompt');
});
...
test('MCP prompts/get kradle_workspace_setup returns messages', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('prompts/get', { name: 'kradle_workspace_setup' }));
  assert.ok(resp.result, 'must return a result');
...
test('MCP prompts/get kradle_troubleshoot returns diagnostic messages', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('prompts/get', { name: 'kradle_troubleshoot' }));
  assert.ok(resp.result, 'must return a result');
  assert.ok(Array.isArray(resp.result.messages), 'must include messages array');
  const assistantMsg = resp.result.messages.find((m) => m.role === 'assistant');
  assert.ok(assistantMsg, 'must have assistant message');
  assert.ok(assistantMsg.content.text.includes('kradle status'), 'troubleshoot message must mention kradle status');
});
...
test('MCP resources/list returns all 3 resources', async () => {
  ...
  assert.ok(uris.includes('kradle://snapshot'), 'must include snapshot resource');
  assert.ok(uris.includes('kradle://stacks'), 'must include stacks resource');
  assert.ok(uris.includes('kradle://models'), 'must include models resource');
});
...
test('MCP resources/read kradle://snapshot returns workspace snapshot data', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('resources/read', { uri: 'kradle://snapshot' }));
  assert.ok(resp.result, 'must return a result');
  ...
  assert.equal(content.uri, 'kradle://snapshot', 'content uri must match');
...
test('MCP resources/read kradle://stacks returns agent stacks data', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('resources/read', { uri: 'kradle://stacks' }));
  assert.ok(resp.result, 'must return a result');
  assert.ok(Array.isArray(resp.result.contents), 'must return contents array');
  const content = resp.result.contents[0];
  assert.equal(content.uri, 'kradle://stacks', 'content uri must match');
  const data = JSON.parse(content.text);
...
test('MCP resources/read unknown URI returns JSON-RPC error', async () => {
  const server = createMcpServer({ controller: createMockController() });
  const resp = await server.handleMessage(rpc('resources/read', { uri: 'kradle://nonexistent' }));
  assert.ok(resp.error, 'must return an error for unknown URI');
  assert.equal(resp.error.code, -32602);
  assert.ok(resp.error.message.includes('kradle://nonexistent'));
});
