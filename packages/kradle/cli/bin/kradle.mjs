...
async function getController() {
  const { createKradleApiController } = await import('../../core/src/api-controller.js');
  const { createKubernetesResourceGateway } = await import('../../core/src/kubernetes-resource-gateway.js');
  return createKradleApiController({
...
async function cmdGet(positional, flags) {
  // Supports: kradle get AgentStack my-stack  OR  kradle get --kind AgentStack --name my-stack
  const kind = positional[0] || flags.kind;
  const name = positional[1] || flags.name;
  if (!kind || !name) {
    console.error('Error: kind and name are required. Usage: kradle get <Kind> <name>');
    process.exit(1);
  }
  const controller = await getController();
...
async function cmdList(positional, flags) {
  // Supports: kradle list AgentStack  OR  kradle list --kind AgentStack
  const kind = positional[0] || flags.kind;
  if (!kind) {
    console.error('Error: kind is required. Usage: kradle list <Kind>');
    process.exit(1);
  }
  const controller = await getController();
...
async function cmdDelete(positional, flags) {
  // Supports: kradle delete AgentStack my-stack  OR  kradle delete --kind AgentStack --name my-stack
  const kind = positional[0] || flags.kind;
  const name = positional[1] || flags.name;
  if (!kind || !name) {
    console.error('Error: kind and name are required. Usage: kradle delete <Kind> <name>');
    process.exit(1);
  }
...
async function cmdModels() {
  ...
  try {
    ...
    if (models.length === 0) {
      console.log('No models in catalog. Deploy models via the web console or create KradleModelRoute resources.');
      return;
    }
...
async function cmdRoutes() {
  const controller = await getController();
  const result = await controller.listResource('KradleModelRoute');
  const items = result?.items || [];
  if (items.length === 0) {
...
async function cmdVirtualModels() {
  const controller = await getController();
  const result = await controller.listResource('KradleVirtualModel');
  const items = result?.items || [];
  if (items.length === 0) {
...
function cmdVersion() {
  console.log(`kradle v${getVersion()}`);
}

function cmdHelp() {
  console.log('Usage: kradle <command> [options]\n');
...
if (command === 'serve' || !command) {
  ...
  console.error('Run "kradle help" for usage.');
  process.exit(1);
}
