...
test('model route creation uses applyModelRoute so generated gateway routes are applied', () => {
  ...
  assert.doesNotMatch(
    ...
    'model route POST must not persist only KradleModelRoute'
  );
  assert.match(
    route,
    /controller\.applyModelRoute\(\s*resource\s*\)/,
    'model route POST must apply KradleModelRoute and generated AIGatewayRoute together'
  );
});
...
test('inference infer route lists virtual models with the org-scoped controller API', () => {
  ...
  assert.doesNotMatch(route, /controller\.listResources\(/, 'createKradleApiController does not expose listResources');
  assert.match(
    route,
    /controller\.listResourceForOrg\(\s*org\s*,\s*['"]KradleVirtualModel['"]\s*\)/,
    'inference infer must load KradleVirtualModel resources through listResourceForOrg(org, kind)'
  );
});
...
// ── Contract: project create form sends required KradleProject fields ──────

test('agent pages reference required KradleProject fields', () => {
  const source = readFile('app', 'pages', 'agent-project-pages.jsx');
  assert.match(source, /KradleProject/);
...
test('inference service manager emits required KradleInferenceService fields', () => {
  ...
  const requiredFields = RESOURCE_DEFINITIONS.KradleInferenceService.requiredSpec;
  for (const field of requiredFields) {
    if (field === 'organizationRef') continue;
    assert.ok(
      source.includes(field),
      `inference-service-manager.jsx does not reference required KradleInferenceService field: ${field}`
    );
  }
});
...
test('model route form emits required KradleModelRoute spec fields', () => {
  const source = readFile('app', 'components', 'inference', 'inference-service-manager.jsx')
    + readFile('app', 'components', 'inference', 'model-route-manager.jsx');
  const requiredFields = RESOURCE_DEFINITIONS.KradleModelRoute.requiredSpec;
  for (const field of requiredFields) {
    if (field === 'organizationRef') continue;
    assert.ok(
      source.includes(field),
      `inference-service-manager.jsx does not reference required KradleModelRoute field: ${field}`
    );
  }
});
...
test('model routes API route creates KradleModelRoute with required fields', () => {
  const route = readFile('app', 'api', 'orgs', '[org]', 'inference', 'routes', 'route.js');
  assert.match(route, /KradleModelRoute/);
...
// ── Contract: virtual model form references required KradleVirtualModel fields ──

test('virtual model form references required KradleVirtualModel fields', () => {
  const source = readFile('app', 'components', 'inference', 'inference-service-manager.jsx')
    + readFile('app', 'components', 'inference', 'virtual-model-manager.jsx');
  const requiredFields = RESOURCE_DEFINITIONS.KradleVirtualModel.requiredSpec;
  for (const field of requiredFields) {
    if (field === 'organizationRef') continue; // added by API route
    assert.ok(
      source.includes(field),
      `inference-service-manager.jsx does not reference required KradleVirtualModel field: ${field}`
    );
  }
});
...
test('virtual model API route creates KradleVirtualModel with required fields', () => {
  const route = readFile('app', 'api', 'orgs', '[org]', 'inference', 'virtual-models', 'route.js');
  assert.match(route, /KradleVirtualModel/);
  assert.match(route, /modelName/);
