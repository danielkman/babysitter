...
test('web smoke: ui-shell.jsx exports key page components', () => {
  ...
  const modulePaths = ['app/lib/kradle-ui.jsx', 'app/lib/page-frame.jsx', 'app/pages/agent-pages.jsx', 'app/pages/repo-pages.jsx', 'app/pages/manage-pages.jsx', 'app/pages/settings-pages.jsx', 'app/pages/external-pages.jsx'];
...
test('web smoke: next.config.mjs has turbopack root and SDK alias', () => {
  ...
  // Verify SDK alias
  assert.ok(
    content.includes('@a5c-ai/kradle-sdk'),
    'next.config.mjs must contain @a5c-ai/kradle-sdk alias'
  );
...
test('web smoke: Dockerfile exists or build config is present', () => {
  ...
  if (fs.existsSync(dockerfilePath)) {
    ...
    assert.ok(
      content.includes('sdk') || content.includes('kradle') || content.includes('node_modules') || content.includes('COPY'),
      'Dockerfile must reference the SDK or kradle packages'
    );
  } else {
