...
test('pipeline logs formatter returns useful text for linked jobs', () => {
  ...
  assert.match(log, /Kradle pipeline log: deploy-test-1/);
...
test('degraded Kradle UI renders recovery loader only for controller fetch failures', () => {
  ...
  assert.match(ui, /KradleControllerRecovery/);
  assert.match(ui, /ORG_HYDRATED_RESOURCE_KINDS = \[/);
  for (const kind of ['Repository', 'RunnerPool', 'Pipeline', 'Job', 'KradleProject', 'Issue']) {
  ...
  assert.match(controllerRoute, /'KradleProject'/);
  ...
  assert.doesNotMatch(ui, /Kradle workspace degraded or empty/);
  ...
  assert.doesNotMatch(loader, /product === 'Kradle' \|\| body\??\.controller/);
...
test('recovery overlay progresses without covering normal route navigation', () => {
  ...
  assert.doesNotMatch(loading, /KradleRouteLoadingOverlay/);
  assert.doesNotMatch(loading, /return null/);
  assert.match(css, /\.kradleRecoveryOverlay\s*\{[\s\S]*position:\s*fixed[\s\S]*inset:\s*0/);
  assert.match(css, /kradleSpinnerRotate/);
  assert.match(loader, /export function KradleControllerRecovery/);
  assert.doesNotMatch(loader, /export function KradleDelayedRouteLoading/);
  assert.doesNotMatch(loader, /export function KradleRouteLoadingOverlay/);
  assert.doesNotMatch(loader, /kradle-route-loading-refresh/);
  assert.doesNotMatch(loader, /orgFromPathname/);
  assert.match(loader, /KRADLE_LOADING_MESSAGES/);
...
test('theme setting applies across full page loads', () => {
  ...
  assert.match(layout, /kradle-theme/);
  assert.match(layout, /suppressHydrationWarning/);
  assert.match(runtime, /THEME_STORAGE_KEY = 'kradle-theme'/);
...
test('agents events stream route exists for EventSource consumers', () => {
  ...
  assert.match(route, /KRADLE_CONTROLLER_URL/);
...
test('snapshot route provides health data for insights page', () => {
  ...
  assert.match(route, /collectKradleHealthProbes/);
