...
// ---------------------------------------------------------------------------
// 9. Page metadata titles all include "Kradle"
// ---------------------------------------------------------------------------

test('all page metadata titles include "Kradle"', () => {
  const allPages = [
    ...orgPageFiles,
    path.join(webRoot, 'app', 'page.jsx'),
    path.join(webRoot, 'app', 'orgs', 'page.jsx'),
  ...
  for (const file of allPages) {
    ...
    if (titleMatch && !titleMatch[1].includes('Kradle')) {
      violations.push(`${rel}: title "${titleMatch[1]}" does not include "Kradle"`);
    }
  }
  assert.deepEqual(
    violations, [],
    `All page titles should include "Kradle":\n${violations.join('\n')}`,
  );
});
