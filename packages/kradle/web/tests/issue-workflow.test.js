...
test('issue PATCH merge preserves scoped metadata while updating comments and refs', () => {
  const existing = {
    ...
    kind: 'Issue',
    metadata: { name: 'issue-1', namespace: 'kradle-org-default', labels: { keep: 'yes' } },
  ...
  const merged = mergeResourcePatch(existing, {
    metadata: { annotations: { 'kradle.a5c.ai/repositories': 'repo-a, repo-b' } },
  ...
  assert.equal(merged.metadata.namespace, 'kradle-org-default');
  assert.equal(merged.metadata.labels.keep, 'yes');
  assert.equal(merged.metadata.annotations['kradle.a5c.ai/repositories'], 'repo-a, repo-b');
...
test('issue detail UI exposes shared editor and comment patch affordances', () => {
  ...
  assert.match(editor, /kradle\.a5c\.ai\/repositories/);
  assert.match(editor, /kradle\.a5c\.ai\/projects/);
