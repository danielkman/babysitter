    ...
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem' }}>
      <button
        ...
        style={{
          ...
          background: currentEnabled ? 'var(--color-good)' : 'var(--color-muted)',
      ...
      >
        <span style={{
          ...
          width: '1rem', height: '1rem', borderRadius: '9999px', background: 'var(--surface)',
          transition: 'left 0.2s', boxShadow: '0 1px 2px rgba(0,0,0,0.2)'
        }} />
      </button>
      <small style={{ color: currentEnabled ? 'var(--color-good)' : 'var(--text-muted)', fontSize: '0.75rem' }}>
        {currentEnabled ? 'Enabled' : 'Disabled'}
      </small>
      {error ? <small style={{ color: 'var(--color-danger)', fontSize: '0.75rem' }}>{error}</small> : null}
    </span>
  );
}
...
export function DeleteRuleButton({ org, ruleName, onDeleted }) {
  ...
  if (status === 'deleted') {
    return <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Deleted</span>;
  }
      ...
      {status === 'confirm' ? (
        <>
          <button
            ...
            style={{ background: 'var(--color-danger)', color: 'var(--surface)', border: 'none', padding: '0.25rem 0.625rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600 }}
          >
            Confirm delete
          </button>
          <button
            ...
            style={{ background: 'transparent', border: '1px solid var(--border)', padding: '0.25rem 0.5rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem' }}
          >
            Cancel
          </button>
        </>
        ...
        <button
          ...
          style={{ background: 'transparent', border: '1px solid var(--color-danger)', color: 'var(--color-danger)', padding: '0.25rem 0.625rem', borderRadius: '4px', cursor: status === 'deleting' ? 'wait' : 'pointer', fontSize: '0.75rem', opacity: status === 'deleting' ? 0.6 : 1 }}
        >
          {status === 'deleting' ? 'Deleting...' : 'Delete'}
        </button>
      )}
      {status === 'error' && error ? <small style={{ color: 'var(--color-danger)', fontSize: '0.75rem' }}>{error}</small> : null}
    </span>
  );
}
