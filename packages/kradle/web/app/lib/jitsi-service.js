...
const {
  ...
  globalEventBus,
  orgNamespaceName,
} = await import('@a5c-ai/kradle-sdk').catch(() => import('../../../sdk/src/index.js'));
...
export function jitsiController(org) {
  return createKradleApiController({ namespace: orgNamespaceName(org) });
}
...
export function createMeetingResource(org, body = {}) {
  ...
  return orgResource(org, 'JitsiMeeting', name, {
    ...
    roomUrl: body.roomUrl || `https://meet.kradle.local/${roomId}`,
...
export function createJoinPayload(meeting, args = {}) {
  ...
  const claims = {
    ...
    context: {
      user: {
        name: args.participantName || args.displayName || 'Kradle user',
        id: args.participantRef || 'kradle-user',
      },
    },
  };
...
function jitsiJwtSecret() {
  const secret = process.env.KRADLE_JITSI_JWT_SECRET || process.env.JITSI_JWT_SECRET;
  if (secret) return secret;
  return 'dev-jitsi-secret';
}

export function verifyJitsiWebhookSignature(rawBody, signatureHeader, secret = process.env.KRADLE_JITSI_WEBHOOK_SECRET || process.env.JITSI_WEBHOOK_SECRET) {
