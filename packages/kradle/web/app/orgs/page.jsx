import { loadKradleUi, PageFrame, StatusPill, orgHref } from '../ui-shell.jsx';

export const metadata = { title: 'Organizations | Kradle' };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const { model } = await loadKradleUi(null);
  const orgs = model.orgs?.length ? model.orgs : [{ slug: 'default', displayName: 'Default org', namespace: 'kradle-org-default' }];
  const activeOrg = orgs[0]?.slug || process.env.KRADLE_ORG || 'default';
  return <PageFrame org={activeOrg} orgs={orgs} eyebrow="organizations" title="Choose an organization" text="Each organization has its own workspace, repositories, people, runs, and deployments." actions={[[`/orgs/${activeOrg}`, 'Open current'], ['/api/orgs', 'Advanced organization API']]} breadcrumbs={[[ '/orgs', 'Organizations' ]]}> 
    <section className="routeGrid two" aria-label="Organizations">
      ...
      <div className="card">
        ...
        <p>Organization resources stay isolated from each other while the Kradle experience remains one flow.</p>
      </div>
    </section>
  </PageFrame>;
}
