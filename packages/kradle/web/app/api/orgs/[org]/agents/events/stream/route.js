...
async function proxyControllerStream(org, request) {
  if (!process.env.KRADLE_CONTROLLER_URL) {
    console.warn('[agent-events-stream] KRADLE_CONTROLLER_URL not set, falling back to local event bus');
    return null;
  }
  const requestUrl = new URL(request.url);
  const cursor = request.headers.get('Last-Event-ID') || requestUrl.searchParams.get('cursor') || '';
  const target = new URL(`/api/orgs/${org}/agents/events/stream`, process.env.KRADLE_CONTROLLER_URL);
