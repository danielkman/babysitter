...
All notable changes to `@a5c-ai/adapters` are recorded here. Dates are ISO-8601.
...
- The `@a5c-ai/adapters` meta package currently re-exports `core` + `adapters` from `packages/adapters/src/index.ts`. The CLI ships from `@a5c-ai/adapters-cli`.
