...
adapter.run(options: RunOptions): RunHandle
adapter.adapters         // AdapterRegistry — discovery, capabilities, install
adapter.models           // ModelRegistry — per-agent model lists and metadata
adapter.sessions         // SessionManager — read native session files
adapter.config           // ConfigManager — read/write agent config files
adapter.auth             // AuthManager — detect and surface auth state
adapter.profiles         // ProfileManager — named RunOptions presets
adapter.plugins          // PluginManager — install, list, remove plugins per agent
```
...
interface RunHandle extends AsyncIterable<AgentEvent> {
  ...
  // Promise interface — await adapter.run() gives RunResult
...
npx @a5c-ai/adapters       # Zero-install CLI
```
...
- Explicit parallel dispatch orchestration (consumers call `adapter.run()` concurrently)
