...
const migrationDoc = read('packages/adapters/transport/migration.md');
const readmeDoc = read('packages/adapters/transport/README.md');
const architectureDoc = read('packages/adapters/transport/architecture.md');
const packageJson = JSON.parse(read('packages/adapters/transport/package.json'));
const packageEntrypoint = read('packages/adapters/transport/src/index.ts');
const launchCommand = [
  read('packages/adapters/cli/src/commands/launch.ts'),
...
const jsContractTests =
  ...
  countFiles('packages/adapters/transport/tests/e2e', '.ts');
...
const scorecard = [
  ...
  },
  {
    ...
    evidence: packageJson.scripts['scorecard:migration']
      ? `scorecard:migration script is present and ${jsContractTests} JS test files exist under packages/adapters/transport/tests`
      : 'scorecard:migration script is missing from packages/adapters/transport/package.json',
