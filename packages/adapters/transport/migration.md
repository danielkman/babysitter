...
- `packages/adapters/transport/package.json` remains publishable and aligned with the package docs map.
...
- `packages/adapters/transport/README.md` describes this workspace as the published runtime seam.
- `packages/adapters/transport/architecture.md` remains the design reference for the runtime boundary.
...
- `packages/adapters/cli/src/commands/launch.ts` resolves into the runtime surface exported by this package.
- `packages/adapters/core/src/provider-resolver.ts` and `packages/adapters/adapters/src/translate-for-harness.ts` stay aligned with that runtime.
