...
function expectExport(exportsMap, subpath, typesPath, defaultPath) {
  ...
  expect(
    ...
    `packages/adapters/core/package.json must keep exporting ${subpath} from ${defaultPath} and ${typesPath}`
  );
  expect(
    requireEntry.types === typesPath && requireEntry.default === defaultPath,
    `packages/adapters/core/package.json must keep require parity for ${subpath}`
  );
  expect(
    exportEntry.default === defaultPath,
    `packages/adapters/core/package.json must keep the default condition for ${subpath}`
  );
}
...
export function verifyAgentMuxCoreRelease({ packageRoot, manifest, packEntries }) {
  ...
  expect(
    ...
    'packages/adapters/core/package.json publishConfig.access must stay public'
  );
  expect(
    ...
    'packages/adapters/core/package.json files must keep dist, README.md, and LICENSE'
  );
  expect(
    manifest.main === './dist/index.js' &&
      manifest.module === './dist/index.js' &&
      manifest.types === './dist/index.d.ts',
    'packages/adapters/core/package.json main/module/types must keep pointing at dist/index.*'
  );
  expect(
  ...
  expect(
    ...
    'packages/adapters/core/package.json verify:release must point at the package-local release verifier'
  );
  expect(
    scripts.prepublishOnly === 'npm run build && npm run test && npm run verify:release',
    'packages/adapters/core/package.json prepublishOnly must build, test, and verify the release surface'
  );
  ...
  for (const exportedSurface of REQUIRED_README_EXPORTS) {
    expect(
      ...
      `packages/adapters/core/README.md must keep documenting ${exportedSurface}`
    );
  }
  ...
  for (const command of REQUIRED_README_COMMANDS) {
    expect(
      ...
      `packages/adapters/core/README.md must keep documenting ${command}`
    );
  }
}
