...
describe('scaffoldPlugin', () => {
  it.each([
    ...
    const outputDir = path.join(createTempDir('extensions-adapter-init-'), template);
    ...
    const compileResult = compile({
      source: outputDir,
      target: 'claude-code',
      output: path.join(createTempDir('extensions-adapter-compile-'), template),
    });
  ...
  it('runs the public init CLI flow', () => {
    const outputDir = path.join(createTempDir('extensions-adapter-cli-'), 'scaffold');
    const stdout: string[] = [];
    const stderr: string[] = [];
