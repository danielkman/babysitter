...
describe('adapters trigger evaluation', () => {
  ...
  it('matches changed file globs and diff mentions', () => {
    const event = normalizeEvent('github', 'push', {
      ref: 'refs/heads/main',
      repository: { full_name: 'a5c-ai/babysitter' },
      commits: [{ modified: ['packages/adapters/cli/src/index.ts'] }],
    });
    event.changes[0]!.patch = '+ @develop-this wire this action';

    const result = evaluateTrigger(event, { paths: 'packages/adapters/**', diff: '@develop-this' });

    expect(result.matched).toBe(true);
    expect(matchesGlob('packages/adapters/cli/src/index.ts', 'packages/adapters/**')).toBe(true);
  });
