...
/**
 * Creates the Antigravity CLI adapter capability descriptor.
 *
 ...
 * workflow-driven hook model (not shell-hook) and a SKILL.md plugin
 * system. It is model-agnostic, supporting Gemini, Claude, and GPT.
 */
const DEFAULT_ADAPTER_NAME = 'antigravity';

export function createAdapter(name: string = DEFAULT_ADAPTER_NAME): AdapterCapabilities {
  const target = getPluginTargetDescriptor(name === 'antigravity' ? 'antigravity-cli' : name);
  return {
    name,
    family: (target?.hooksMuxFamily as AdapterCapabilities['family']) ?? 'workflow-hook',
    ...
    notes: [
      'Workflow-driven hook model (successor to Gemini CLI shell-hook)',
      'SKILL.md plugin system with workflow, rules, and MCP integration',
      'Model-agnostic: Gemini 3.5 Flash (default), Claude Sonnet 4.6, GPT-OSS-120B',
      'Logs must go to stderr; final JSON to stdout only',
      'Session ID derived from ANTIGRAVITY_SESSION_ID env or workspace + timestamp',
    ],
  };
}
