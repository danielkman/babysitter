...
/**
 * Antigravity CLI native hook output formats.
 *
 * Antigravity CLI hooks communicate results via JSON on stdout only.
 * All diagnostic/log output MUST go to stderr.
 *
 * The output format is inherited from Gemini CLI, with the same
 * field structure per event type. The dispatch mechanism differs
 * (workflow-driven vs shell scripts) but the wire protocol is identical.
 */

/** BeforeToolSelection hook output: tool filtering via union aggregation. */
export interface AntigravityBeforeToolSelectionOutput {
...
export interface AntigravityBeforeModelOutput {
...
export interface AntigravityAfterModelOutput {
  /** Additional context to inject. */
  additionalContext?: string;
}

/** BeforeAgent hook output. */
export interface AntigravityBeforeAgentOutput {
  /** Additional context to inject. */
  additionalContext?: string;
...
export interface AntigravityAfterAgentOutput {
...
export interface AntigravityBeforeToolOutput {
  /** Permission decision. */
  decision?: 'allow' | 'deny';
  /** Reason for the decision. */
  reason?: string;
...
export interface AntigravityAfterToolOutput {
  /** Additional context to inject. */
  additionalContext?: string;
}

/** SessionStart hook output. */
export interface AntigravitySessionStartOutput {
  /** Additional context to inject. */
  additionalContext?: string;
}

/**
 * Render a UnifiedHookResult back to Antigravity's native JSON output format.
 *
 ...
 * @param nativeEventName - The original Antigravity event name.
 * @returns The native JSON output to write to stdout.
 */
export function renderAntigravityOutput(
...
function renderBeforeToolSelectionOutput(
  ...
  const output: AntigravityBeforeToolSelectionOutput = {};
...
function renderBeforeModelOutput(result: UnifiedHookResult): AntigravityBeforeModelOutput {
  const output: AntigravityBeforeModelOutput = {};
...
function renderAfterModelOutput(result: UnifiedHookResult): AntigravityAfterModelOutput {
  const output: AntigravityAfterModelOutput = {};
...
function renderBeforeAgentOutput(result: UnifiedHookResult): AntigravityBeforeAgentOutput {
  const output: AntigravityBeforeAgentOutput = {};
...
function renderAfterAgentOutput(result: UnifiedHookResult): AntigravityAfterAgentOutput {
  const output: AntigravityAfterAgentOutput = {};
...
function renderBeforeToolOutput(
  ...
  const output: AntigravityBeforeToolOutput = {};
  ...
  // 'ask', 'continue', 'noop' produce no decision — Antigravity defaults to allow
...
function renderAfterToolOutput(result: UnifiedHookResult): AntigravityAfterToolOutput {
  const output: AntigravityAfterToolOutput = {};
...
function renderSessionStartOutput(result: UnifiedHookResult): AntigravitySessionStartOutput {
  const output: AntigravitySessionStartOutput = {};
 ...
 *
 * Antigravity CLI requires that only the final JSON result goes to stdout.
