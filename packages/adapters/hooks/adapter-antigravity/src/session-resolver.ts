...
/**
 * Resolve a session ID for Antigravity CLI.
 *
 * Antigravity CLI does not provide a native session ID on stdin.
 * The session ID is derived from available signals:
 *
 *   1. Explicit env: AGENT_SESSION_ID or HOOKS_PROXY_SESSION_ID
 *   2. Antigravity-specific env: ANTIGRAVITY_SESSION_ID
...
export function resolveSessionId(
  ...
  // 2. Antigravity-specific session env
  if (env['ANTIGRAVITY_SESSION_ID']) {
    return env['ANTIGRAVITY_SESSION_ID'];
  }
