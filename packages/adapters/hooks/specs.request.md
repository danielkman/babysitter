...
runtime event callback
    -> @a5c-ai/hooks-adapter-cli library
        -> normalize event
...
```bash
npx -y @a5c-ai/hooks-adapter-cli invoke --adapter claude
```
...
```bash
npx -y @a5c-ai/hooks-adapter-cli exec --session-id "$AGENT_SESSION_ID" -- npm test
```
