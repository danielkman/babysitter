...
1. `@a5c-ai/adapters-hooks-cli` — npm library
...
runtime event callback
    -> @a5c-ai/adapters-hooks-cli library
        -> normalize event
...
```bash
npx -y @a5c-ai/adapters-hooks-cli invoke --adapter claude
```
...
```bash
npx -y @a5c-ai/adapters-hooks-cli exec --session-id "$AGENT_SESSION_ID" -- npm test
```
