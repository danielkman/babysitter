...
describe('adapters remote install/update', () => {
  it('runs all four steps via ssh when adapters missing on remote', async () => {
    ...
    const { spawner, calls } = makeSpawner([
      { code: 127 }, // probe: adapters missing
      { code: 0 },   // npm install adapters
      { code: 0 },   // adapters install claude
  ...
  it('uses docker transport when --mode=docker --image=...', async () => {
    const client = createClient();
    const args = parseArgs(
      ['remote', 'install', '--mode', 'docker', '--image', 'ghcr.io/a5c-ai/adapters:latest'],
      REMOTE_FLAGS,
    );
    const { spawner, calls } = makeSpawner([
    ...
    expect(calls[0]!.args).toContain('ghcr.io/a5c-ai/adapters:latest');
  });
