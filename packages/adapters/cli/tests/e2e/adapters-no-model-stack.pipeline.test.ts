...
describe('adapters no-model stack matrix', () => {
  ...
  matrixIt('installs and launches a no-model stack lane through transport-adapter', async () => {
    ...
    if (lane.runtime === 'adapters-mocks') {
      ...
      const runOutput = await callMain([
        ...
        'Say hello through the no-model transport adapter.',
      ]);
      ...
      const launchOutput = await callMain([
        ...
        '--no-proxy',
        '--no-interactive',
        '--prompt', 'Say hello through the no-model transport adapter.',
      ]);
...
function readMatrixLane(): MatrixLane {
  ...
  if (hooks !== 'hooks-adapter' && hooks !== 'none') {
...
for (const line of eventsFor(agent, body.choices?.[0]?.message?.content || 'Hello from transport adapter')) {
...
async function fireHooksMuxBridge(repoRoot: string, evidenceDir: string, agent: string): Promise<Record<string, unknown>> {
  ...
  const evidencePath = path.join(evidenceDir, `${agent}-hooks-adapter.jsonl`);
...
`, 'utf8');
  ...
  let output = await callMain(['hooks', agent, 'add', hook.hookType, '--handler', 'command', '--target', `${quoteForShell(process.execPath)} ${quoteForShell(bridgePath)}`, '--id', 'no-model-hooks-adapter', '--json']);
