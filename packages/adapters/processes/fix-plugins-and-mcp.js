...
const investigateTask = defineTask('investigate', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: 'Check `packages/adapters/core`, `packages/adapters/cli`, `packages/adapters/tui`, and adapters like `packages/adapters/adapters/src/claude-adapter.ts` to figure out why plugins and MCP servers are not showing, and why global vs repo local distinction is missing or broken. Identify how to fix this by using `findProjectRoot` or adapter-specific logic. Write your plan to result.json.',
      outputFormat: 'JSON'
    },
