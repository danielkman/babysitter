...
const deterministicGateTask = defineTask('deterministic-gate', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      `cd "${args.repoRoot}"`,
      'npm test -- packages/adapters/adapters/tests/codex-adapter.test.ts packages/adapters/adapters/tests/claude-adapter.test.ts packages/adapters/core/tests/spawn-runner.test.ts packages/adapters/cli/tests/commands/run.test.ts packages/adapters/gateway/tests/e2e.test.ts',
      'npm run build',
    ].join(' && '),
    expectedExitCode: 0,
    timeout: 600000,
...
const smokeCheckTask = defineTask('interactive-smoke-check', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      `cd "${args.repoRoot}"`,
      'node packages/adapters/cli/dist/index.js help > NUL',
      'node packages/adapters/cli/dist/index.js models --help > NUL',
      'node packages/adapters/cli/dist/index.js gateway --help > NUL',
    ].join(' && '),
    expectedExitCode: 0,
    timeout: 120000,
  },
