...
replaceInFile('packages/adapters/core/src/plugin-manager.ts',
...
replaceInFile('packages/adapters/core/src/plugin-manager-impl.ts',
...
replaceInFile('packages/adapters/core/src/adapter.ts',
  'uninstallPlugin?(pluginId: string): Promise<void>;',
  'uninstallPlugin?(pluginId: string, options?: { global?: boolean }): Promise<void>;');

replaceInFile('packages/adapters/core/src/adapter-types.ts',
  'uninstallPlugin?(pluginId: string): Promise<void>;',
  'uninstallPlugin?(pluginId: string, options?: { global?: boolean }): Promise<void>;');

const adapters = fs.readdirSync('packages/adapters/adapters/src').filter(f => f.endsWith('-adapter.ts'));
for (const a of adapters) {
  let code = fs.readFileSync(`packages/adapters/adapters/src/${a}`, 'utf8');
  ...
  fs.writeFileSync(`packages/adapters/adapters/src/${a}`, code);
}
