  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        ...
        'The repo layout is authoritative. If the playbook names older paths like packages/adapters/core/src/client/types.ts or packages/adapters/core/src/capabilities/types.ts, map the work onto the actual current files in this repo.',
...
const qualityGateTask = defineTask('quality-gate', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      `npm run build`,
      `npm run lint`,
      `npx vitest run packages/adapters/core/tests/adapter-registry.test.ts packages/adapters/core/tests/capabilities.test.ts packages/adapters/core/tests/client-run.test.ts packages/adapters/core/tests/model-registry.test.ts packages/adapters/core/tests/plugin-manager.test.ts`,
    ].join(' && '),
    expectedExitCode: 0,
    timeout: 30 * 60_000,
  },
