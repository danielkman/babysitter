...
const refactorTypesTask = defineTask('refactor-types', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: `Refactor the flat booleans \`supportsSteering\`, \`supportsQueueing\`, \`supportsNonInteractiveStream\`, \`supportsInteractiveStream\`, \`supportsJsonlStream\`, \`supportsJsonlNonStream\` in \`packages/adapters/core/src/capabilities.ts\` into nested objects. Add:
...
const updateAdaptersTask = defineTask('update-adapters', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      role: 'Senior Software Engineer',
      task: `Update all adapters in \`packages/adapters/adapters/src/*-adapter.ts\` to use \`executionModes\`, \`steering\`, \`queueing\`, and \`supportsAsyncLoopTools\` instead of the old flat properties. Build the new nested objects with sensible defaults (like the old flat ones, but map them across interactive/nonInteractive and stream/jsonl). Generally, only claude/codex/gemini had true for some of these. You can write a node script or do it manually.`,
      outputFormat: 'JSON'
    },
    outputSchema: { type: 'object', required: ['status', 'changes'] }
  },
