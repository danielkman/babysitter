...
const verifyTask = defineTask('verify-claude-server-mode', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'npx tsc -b packages/adapters/adapters packages/adapters/core packages/adapters/gateway packages/adapters/ui packages/adapters/webui --pretty false',
      'npm exec vitest run packages/adapters/adapters/tests/claude-agent-sdk-adapter.test.ts packages/adapters/gateway/tests/e2e.test.ts --reporter=dot'
    ].join(' ; '),
    expectedExitCode: 0,
    timeout: 1800000
  },
