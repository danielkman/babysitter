...
This process is scoped to the single observability backlog item. It starts from the current unfinished `packages/adapters/observability` package and partial `core` wiring, then delivers the remaining logging and OpenTelemetry work through phased implementation, verification, and review loops.
