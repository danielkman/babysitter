...
let mcpPlugins = fs.readFileSync('packages/adapters/adapters/src/mcp-plugins.ts', 'utf8');
...
fs.writeFileSync('packages/adapters/adapters/src/mcp-plugins.ts', mcpPlugins);

const adapters = ['claude', 'cursor', 'gemini', 'opencode', 'openclaw'];
for (const a of adapters) {
  const p = `packages/adapters/adapters/src/${a}-adapter.ts`;
