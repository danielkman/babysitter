...
const analyzeRefTask = defineTask('analyze-reference', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: `Read every source file under ${args.url}. Extract formats, syntaxes, patterns, performance/security considerations, error handling, edge cases. Compare against our adapters in packages/adapters/adapters/src/. Return gap list.`,
...
export default async function referenceParity(ctx) {
  ...
  for (const ref of REFS) {
    perRef.push(await ctx.run(analyzeRefTask, { ...ref, ourAdapters: 'packages/adapters/adapters/src/' }));
  }
