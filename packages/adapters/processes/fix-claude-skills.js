...
const investigateTask = defineTask('investigate', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: 'Check `packages/adapters/core`, `packages/adapters/cli`, and `packages/adapters/adapters/src/claude-adapter.ts` to figure out why Claude Code adapter is not loading repo specific skills, and why it is not showing the correct skills. Write the finding and the plan to result.json.',
      outputFormat: 'JSON'
    },
