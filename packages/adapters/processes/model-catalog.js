...
const captureRepoStateTask = defineTask('capture-repo-state', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      `rg -n "readonly models:|defaultModelId|modelAlias|inputPricePerMillion|outputPricePerMillion" packages/adapters/adapters/src -S || true`,
      `echo ""`,
...
const qualityGateTask = defineTask('quality-gate', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      `npm run build`,
      `npm run lint`,
      `npx vitest run packages/adapters/core/tests/model-registry.test.ts packages/adapters/core/tests/config-manager.test.ts packages/adapters/cli/tests/commands-audit.test.ts packages/adapters/tui/tests/info-views.test.tsx packages/adapters/tui/tests/models-view.test.tsx`,
    ].join(' && '),
    expectedExitCode: 0,
    timeout: 45 * 60_000,
  },
