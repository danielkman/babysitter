 ...
 * @description Research, scaffold, test, and document a new agent-harness adapter for @a5c-ai/adapters.
 */
...
const scaffoldTask = defineTask('scaffold-adapter', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: `Create packages/adapters/adapters/src/${args.harness}-adapter.ts extending BaseAgentAdapter.`,
      context: { harness: args.harness, research: args.research, projectRoot: args.projectRoot },
      instructions: [
        'Read packages/adapters/adapters/src/claude-adapter.ts as the canonical template.',
        ...
        'Register the adapter in packages/adapters/adapters/src/index.ts.',
      ],
    },
  },
  io: {
...
const testTask = defineTask('test-adapter', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: `Write packages/adapters/adapters/tests/${args.harness}-adapter.test.ts with full coverage.`,
      ...
      instructions: [
        ...
        'If MCP plugin support, add the adapter to packages/adapters/adapters/tests/mcp-plugins-parity.test.ts.',
