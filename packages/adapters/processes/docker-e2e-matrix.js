 ...
 *
 * Status: scaffold complete (packages/adapters/docker/e2e/Dockerfile.mock, Dockerfile.real, docker-compose.yml, --use-mock-harness flag).
...
const expandMatrixTask = defineTask('expand-matrix', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: 'Extend packages/adapters/docker/e2e/docker-compose.yml with one mock-e2e and one real-e2e service per adapter.',
      ...
      instructions: [
        ...
        'Update packages/adapters/docker/e2e/README.md with the full matrix.',
...
const verifyTask = defineTask('verify-matrix', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      role: 'QA',
      task: 'Run `docker compose -f packages/adapters/docker/e2e/docker-compose.yml up --exit-code-from <service>` for every mock-e2e-<name>. Report pass/fail per adapter.',
    },
  },
  io: {
    inputJsonPath: `tasks/${taskCtx.effectId}/input.json`,
