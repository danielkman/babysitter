    ...
    prompt: {
      ...
      context: {
        ...
        sdkEntrypoints: [
          ...
          'packages/adapters/core/src/events.ts',
          'packages/adapters/core/src/session-manager.ts',
          'packages/adapters/sdk/src/index.ts',
          'packages/adapters/cli/src/index.ts',
        ],
        docsRoots: ['docs/', 'docs/tutorials/'],
      },
      instructions: [
        'Read packages/adapters/tui/src/** to understand what the scaffold currently provides.',
        'Read packages/adapters/core/src/events.ts and enumerate every AgentEvent variant.',
        'Read packages/adapters/core/src/client.ts and packages/adapters/core/src/session-manager.ts to enumerate every public API surface (run, sessions list/read/resume/fork/watch, plugins install/list/uninstall, hooks install, detect, doctor, profiles, remote bootstrap, cost helpers, capabilities).',
...
const writeTestsTask = defineTask('write-failing-tests', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      context: { gap: args.gap, tuiPackage: 'packages/adapters/tui' },
      instructions: [
        'Use vitest + ink-testing-library (install as devDep if missing) for component tests.',
        'Write test files under packages/adapters/tui/tests/plugins/<gap-id>.test.ts or tests/<gap-id>.test.ts.',
        'Tests MUST fail initially — they encode behavior that does not exist yet.',
        'Run `npx vitest run packages/adapters/tui` and confirm the new tests fail with an expected assertion, not a syntax/import error.',
...
const implementTask = defineTask('implement-gap', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        'Implement the plugin/view/command strictly through the registerView / registerEventRenderer / registerCommand APIs exported by packages/adapters/tui/src/plugin.ts — do NOT add new extension points without updating both plugin.ts and the README.',
        ...
        'Add the new plugin to the builtinPlugins array in packages/adapters/tui/src/index.ts if appropriate.',
...
const testGate = defineTask('test-gate', (args, taskCtx) => ({
  ...
  shell: {
    command: `cd ${args.projectRoot} && npx vitest run packages/adapters/tui --reporter=default`,
    expectedExitCode: 0,
    timeout: 300000,
  },
  io: {
...
const docsSyncTask = defineTask('sync-docs', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      instructions: [
        'Update packages/adapters/tui/README.md with the new plugin and any new extension-point API.',
