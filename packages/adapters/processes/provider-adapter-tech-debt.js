/**
 * @process provider-adapter-tech-debt
 * @description Complete remaining tech debt for the provider-adapter/launcher feature across TypeScript and Python packages.
 */
...
const tsQualityTask = defineTask('ts-quality-improvements', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: 'Implement 4 quality improvements in the adapters provider-adapter system.',
      context: {
      ...
      instructions: [
        ...
        'In packages/adapters/cli/src/commands/launch.ts, the forwardSignal function uses child.kill(sig).',
        ...
        'In packages/adapters/cli/src/commands/launch.ts, register the proxy PID with ProcessTracker',
        '(imported from @a5c-ai/comm-adapter) so it gets cleaned up on uncaught exceptions.',
        'Add: processTracker.register(proxyProcess.pid, "adapters-proxy") after spawning.',
        'Import processTracker from core.',
        '',
        '## Item 3: translateProvider as adapter method (P4.2)',
        'Add an optional translateProvider method to the adapter interface in packages/adapters/core/src/adapter.ts:',
        ...
        'In packages/adapters/adapters/src/translate-for-harness.ts, check if the adapter has translateProvider',
        ...
        'In packages/adapters/core/src/provider-profiles.ts, after reading ~/.adapters/providers.json,',
...
const pythonImprovementsTask = defineTask('python-improvements', (args, taskCtx) => ({
  kind: 'agent',
  title: 'Python adapters-proxy improvements: /v1/models endpoint, Ollama server lifecycle, token count',
  execution: { model: 'claude-sonnet-4-6' },
  agent: {
    name: 'general-purpose',
    prompt: {
      role: 'Senior Python engineer',
      task: 'Implement 3 new features in the adapters-proxy Python package.',
      context: {
        workspaceRoot: args.workspaceRoot,
        feedback: args.feedback ?? null,
        attempt: args.attempt ?? 1,
      ...
      instructions: [
        ...
        'Add a /v1/models endpoint to packages/adapters/adapters-proxy/src/adapters_proxy/server.py that queries',
        ...
        'Create packages/adapters/adapters-proxy/src/adapters_proxy/providers/ollama_server.py with OllamaServerManager',
        'that can start/stop the Ollama server as a subprocess when AGENT_MUX_PROXY_OLLAMA_MANAGE_SERVER=true.',
...
const cliExtensionsTask = defineTask('cli-extensions', (args, taskCtx) => ({
  kind: 'agent',
  title: 'CLI extensions: adapters models --provider, Ollama model pull in launch, root scripts',
  execution: { model: 'claude-sonnet-4-6' },
  agent: {
    name: 'general-purpose',
    prompt: {
      role: 'Senior TypeScript engineer',
      task: 'Implement 3 CLI improvements for the adapters provider-adapter system.',
      context: {
        workspaceRoot: args.workspaceRoot,
        feedback: args.feedback ?? null,
        attempt: args.attempt ?? 1,
      ...
      instructions: [
        '## Item 1: adapters models --provider extension (P3.1)',
        'In packages/adapters/cli/src/commands/models.ts, add --provider and --harness flags.',
        ...
        'Import from @a5c-ai/comm-adapter: PROVIDER_DEFAULTS, MODEL_TRANSLATION_TABLE.',
        '',
        '## Item 2: Ollama model pre-check in adapters launch (P3.10)',
        'In packages/adapters/cli/src/commands/launch.ts, when the resolved provider is "ollama",',
        ...
        'If --prompt IS set, auto-pull if AGENT_MUX_OLLAMA_AUTO_PULL env is set, else error.',
        '',
        '## Item 3: Root package.json scripts for adapters-proxy (P2.2)',
        'Add to the root package.json scripts:',
        '  "test:adapters-proxy": "cd packages/adapters/adapters-proxy && python -m pytest tests/ -v"',
        '  "lint:adapters-proxy": "cd packages/adapters/adapters-proxy && ruff check ."',
        '',
...
const verifyAllTask = defineTask('verify-all', (args, taskCtx) => ({
  ...
  shell: {
    command: [
      ...
      'npx vitest run packages/adapters/core/tests/ packages/adapters/adapters/tests/ packages/adapters/cli/tests/launch-resolution.test.ts --reporter=dot',
      'cd packages/adapters/adapters-proxy && python -m pytest tests/ -v',
    ].join(' && '),
    expectedExitCode: 0,
    timeout: 300000,
  },
...
const reviewTask = defineTask('review-tech-debt', (args, taskCtx) => ({
  ...
  agent: {
    ...
    prompt: {
      role: 'Code reviewer',
      task: 'Review the provider-adapter tech debt implementation for correctness.',
      context: {
        workspaceRoot: args.workspaceRoot,
