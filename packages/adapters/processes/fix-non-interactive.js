...
const fixAdaptersTask = defineTask('fix-adapters', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      role: 'Senior Software Engineer',
      task: 'Modify all adapters in `packages/adapters/adapters/src/*-adapter.ts` to only append `-p` or `--prompt` to `buildSpawnArgs` when `options.nonInteractive` is true. If false, rely on stdin/stdout handling. Make sure not to break existing adapters.',
      outputFormat: 'JSON'
    },
    outputSchema: { type: 'object', required: ['status', 'changes'] }
  },
