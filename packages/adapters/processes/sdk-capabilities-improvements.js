...
const extendCapabilitiesTask = defineTask('extend-capabilities', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      ...
      task: 'Update `packages/adapters/core/src/capabilities.ts` to include the new capability properties: `supportsSteering`, `supportsQueueing`, `supportsAsyncLoopTools`, `supportsNonInteractiveStream`, `supportsInteractiveStream`, `supportsJsonlStream`, and `supportsJsonlNonStream`.',
      outputFormat: 'JSON'
    },
...
const updateAdaptersTask = defineTask('update-adapters', (args, ctx) => ({
  ...
  agent: {
    ...
    prompt: {
      role: 'Senior Software Engineer',
      task: 'Research and update all existing adapters in `packages/adapters/adapters/src/*-adapter.ts` to correctly report these new capabilities. Research `claude` (Claude Code), `gemini`, `codex`, `cursor`, `opencode`, etc. Usually `gemini` and `codex` might support steering/queueing. `claude-code` supports async loop tools.',
      outputFormat: 'JSON'
    },
    outputSchema: { type: 'object', required: ['status', 'changes'] }
  },
