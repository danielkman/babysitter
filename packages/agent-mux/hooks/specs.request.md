...
1. `@a5c-ai/agent-mux-hooks-cli` — npm library
...
runtime event callback
    -> @a5c-ai/agent-mux-hooks-cli library
        -> normalize event
...
```bash
npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude
```
...
```bash
npx -y @a5c-ai/agent-mux-hooks-cli exec --session-id "$AGENT_SESSION_ID" -- npm test
```
