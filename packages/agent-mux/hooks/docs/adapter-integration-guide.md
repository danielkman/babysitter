...
Per-harness setup instructions for integrating `@a5c-ai/agent-mux-hooks-cli` with each supported agent harness.

---

## Overview

The hooks-mux system normalizes hook execution across agent harnesses. Each harness has a dedicated adapter package (`@a5c-ai/agent-mux-hooks-adapter-<name>`) that translates between harness-native hook contracts and the canonical event model.
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-claude`
**Family:** Shell-hook
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude --native-event SessionStart"
      }
    ],
    "PreToolUse": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude --native-event PreToolUse"
      }
    ],
    "PostToolUse": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude --native-event PostToolUse"
      }
    ],
    "Stop": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude --native-event Stop"
      }
    ]
  }
}
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter claude --native-event SessionStart --bootstrap-only"
      }
    ]
  }
}
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "npx -y @a5c-ai/agent-mux-hooks-cli bootstrap --adapter claude"
      }
    ]
  }
}
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-codex`
**Family:** Shell-hook
**Session ID:** Native
...
```bash
npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter codex
```
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-gemini`
**Family:** Shell-hook
...
{
  "hooks": {
    "onSessionStart": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter gemini",
    "onToolUse": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter gemini"
  }
}
```
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-copilot`
**Family:** Shell-hook
...
```bash
npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter copilot
```
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-cursor`
**Family:** Shell-hook
...
{
  "hooks": {
    "onFileChange": "npx -y @a5c-ai/agent-mux-hooks-cli invoke --adapter cursor --bootstrap-only"
  }
}
```
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-pi`
**Family:** In-process
**Session ID:** Native
...
```typescript
import { createAdapter, registerHandler, runNormalized } from '@a5c-ai/agent-mux-hooks-core';
import { createPiAdapter } from '@a5c-ai/agent-mux-hooks-adapter-pi';
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-oh-my-pi`
**Family:** In-process
**Session ID:** Native
**Env persistence:** Runtime hook
...
```typescript
import { createOhMyPiAdapter } from '@a5c-ai/agent-mux-hooks-adapter-oh-my-pi';
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-opencode`
**Family:** In-process
**Session ID:** Derived
**Env persistence:** Wrapper only
...
```typescript
import { createOpenCodeAdapter } from '@a5c-ai/agent-mux-hooks-adapter-opencode';
...
**Adapter package:** `@a5c-ai/agent-mux-hooks-adapter-openclaw`
**Family:** In-process
**Session ID:** Derived
**Env persistence:** Wrapper only
...
```typescript
import { createOpenClawAdapter } from '@a5c-ai/agent-mux-hooks-adapter-openclaw';
...
```bash
# Check all adapters
npx -y @a5c-ai/agent-mux-hooks-cli doctor

# Check a specific adapter
npx -y @a5c-ai/agent-mux-hooks-cli doctor --adapter claude

# JSON output for scripting
npx -y @a5c-ai/agent-mux-hooks-cli doctor --json

# Include session health with custom stale threshold
npx -y @a5c-ai/agent-mux-hooks-cli doctor --stale-threshold 48
```
