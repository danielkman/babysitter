...
const migrationDoc = read('packages/agent-mux/transport/migration.md');
const readmeDoc = read('packages/agent-mux/transport/README.md');
const architectureDoc = read('packages/agent-mux/transport/architecture.md');
const packageJson = JSON.parse(read('packages/agent-mux/transport/package.json'));
const packageEntrypoint = read('packages/agent-mux/transport/src/index.ts');
const launchCommand = [
...
const jsContractTests =
  countFiles('packages/agent-mux/transport/tests', '.ts') +
  countFiles('packages/agent-mux/transport/tests/transports', '.ts') +
  countFiles('packages/agent-mux/transport/tests/e2e', '.ts');
...
const scorecard = [
  ...
  },
  {
    ...
    evidence: packageJson.scripts['scorecard:migration']
      ? `scorecard:migration script is present and ${jsContractTests} JS test files exist under packages/agent-mux/transport/tests`
      : 'scorecard:migration script is missing from packages/agent-mux/transport/package.json',
  ...
  },
  {
    ...
    status:
      launchCommand.includes('@a5c-ai/agent-mux-transport') &&
